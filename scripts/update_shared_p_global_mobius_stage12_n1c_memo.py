...
def render(report: dict) -> str:
    ...
    table = "\n".join(
        f"| {r['B']:,} | {r['raw_all_oriented']:,} | {r['raw_distinct_oriented']:,} | "
        f"{r['raw_repeated_side_oriented']:,} | {r['global_mobius_of_distinct_raw']:,} | "
        f"{r['stage11_primitive_oriented']:,} |"
        for r in rows
    )
    return rf"""{START}
...
を得る。有限監査では$A_k$を{checks['A_identity_cases']:,}ケース、$B_{{k,B}}$を{checks['B_identity_cases']:,}ケース検算し、相違は0件だった。
