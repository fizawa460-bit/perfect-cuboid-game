...
In addition, every `StageX-audit` must update the audited pull request's **main PR description/body** so that the authoritative post-audit state is visible at the top-level PR surface without requiring the operator to inspect Conversation comments. The PR body must visibly record at least the audit verdict, whether repair is required, `MERGE_ALLOWED`, advancement status / next checkpoint, and `NEXT_EXPECTED_COMMAND`. Submission-time fields such as `AUDIT_REQUIRED=true`, `ADVANCE_ALLOWED=false`, or `MERGE_ALLOWED=false` must be replaced or clearly superseded in the PR body after audit completion. A Conversation comment may also be added as an audit trail, but a comment alone is not sufficient.
