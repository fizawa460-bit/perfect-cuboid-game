...
| NUM-R06 | Matched Stage13 one-face to Stage14 two-face comparison | `DERIVED_EXACT_FINITE` | `14-num-alpha11-diag6`–`diag9` | Stage22 transition diagnostics; Stage24, Stage25 and Stage28 causal diagnostics |
...
| 29 | Endpoint finite regression / negative control only | NUM-R01, NUM-R02, NUM-R03 | Match the primitive/canonical integral-space cutoff and triple-face mask exactly; `T=0` through `B=500,000,000` is bounded evidence only and cannot support global nonexistence |

## Consumer protocol

Before new numerical computation, every Stage16–29 research item must inspect this index and emit:
