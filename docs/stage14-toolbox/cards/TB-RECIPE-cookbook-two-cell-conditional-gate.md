...
TYPE: RECIPE
STATUS: CURRENT
TITLE: Conditional adjacent two-cell mixed-character checklist beyond the one-cell architecture
SCOPE: BOTH
...
- historical s7-09 conditional whole-family target = `16/17`.
- later proved ingredients may update the conditional target without proving the missing theorem.

## USED BY

- Designing later stages that attack the first genuinely joint two-cell receiver.
- Checking exact hypotheses of candidate several-variable character-sum theorems.

## DO NOT USE FOR

- Do not call any conditional target current.
