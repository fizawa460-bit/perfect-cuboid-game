...
ここで `a<b<c` は辺の置換を一意に代表させる canonical ordering、`gcd(a,b,c)=1` は拡大コピーを除く primitive 条件である。狭義不等号を採用するため、`a=b` または `b=c` となる等辺の境界ケースは本稿の研究対象から除外する。
...
2. **canonical unordered representative** — `a<b<c` により辺の置換を一つにまとめる。
