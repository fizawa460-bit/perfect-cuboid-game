...
>
> **CANONICAL_WORKING_FILE:** `docs/stage13-main.md`
...
数学的な誤り、記述不足、定義変更、補題修正が見つかった場合は、原則として `docs/stage13-main.md` の該当 section を **直接修正する**。
...
```text
current mathematical truth = latest merged stage13-main.md
...
ただし、**主張を成立させるために必要な数学的論証そのもの**は、外部レビュー時に canonical file だけでは追えなくならないよう、原則 `stage13-main.md` に置く。

## 4. 例外として別 mathematical file を作る条件
...
は完成済みの初期 source / history として残す。

`docs/stage13-main.md` を開始するときに、この二つの active content を §1, §2 として統合する。その後の修正は `stage13-main.md` 側を canonical とし、旧 13-1 / 13-2 文書は原則更新しない。

## 6. Task と section の対応
...
レビューが必要になった時点で、原則として `docs/stage13-main.md` をそのまま渡す。

補助資料が必要な場合のみ、

```text
canonical stage13-main.md
...
3. stage13-main.md の該当 section を直接修正する
4. 必要な補助資料だけ更新する
5. Git/PR に修正理由を残す
...
将来論文化する場合も、`stage13-main.md` を技術原稿の母艦として保持する。
...
## 10. Stage12 freeze boundary

Stage12-N1-2 は R09 で freeze する。
...
STAGE13_1_2_LEGACY_FILES=HISTORICAL_SOURCE_AFTER_MAIN_IMPORT
STAGE12_R09=FROZEN
```
