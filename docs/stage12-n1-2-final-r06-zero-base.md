# Stage12-N1-2 Final R06 — physically self-contained zero-base proof bundle
...
>
> **DOCUMENT_STATUS:** `FULL_ZERO_BASE_REREVIEW_CANDIDATE_WITH_PROVENANCE`
>
> **COUNTING_TARGET:** primitive oriented count `C_prim(B)` only

## Purpose

This file is a physical, single-document zero-base review target for
...
Unlike R06, this bundle includes the body of Stage12-N1-3a and the historical derivations of `G`, `kappa`, `A_rs`, `beta`, `gamma`, and `eta`. It also includes an explicit vertical-growth argument and an explicit treatment of the lower limits `x,y>=1` in the radial integral.

## Precedence and supersession

Historical sources are included so their derivations can be audited, not so that every historical conclusion becomes active. Apply the following rules exactly.
...
A reviewer must verify both that the required derivations are present and that no superseded claim is silently reactivated.
...
# EMBEDDED SOURCE 1/8 — Active consolidated proof through Stage12-N1-3g
...
# EMBEDDED SOURCE 2/8 — Active rectangle-error derivation
...
# EMBEDDED SOURCE 3/8 — Historical origin of the parameter sum and multiplicity G
...
# EMBEDDED SOURCE 4/8 — Historical exact divisor expansion of G
...
# EMBEDDED SOURCE 5/8 — Historical derivation of kappa and the three-variable local factors
...
# EMBEDDED SOURCE 6/8 — Historical primitive-first derivation of A_rs, beta and gamma
...
# EMBEDDED SOURCE 7/8 — Historical derivation of eta and its comparison with kappa
...
# EMBEDDED SOURCE 8/8 — Active zero-base provenance, vertical-growth and radial-boundary closure
