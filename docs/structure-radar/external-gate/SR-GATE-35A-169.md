...
## 1. Exact Hilbert-space reformulation without a false descended-Parseval identity

On one retained MAIN packet keep the original batch34 modulus `q` and normalized coefficients

```text
c_b = W_hat(b)/q,
sum_{b mod q} |c_b|^2 = (1/q) sum_{f mod q} |W(f)|^2.
```

For a fixed gcd stratum `d=(a,q)`, the Gauss sum vanishes unless `d|b`. Write the stratum contribution schematically as

```text
S_d = sum_{b mod q : d|b} c_b T_{d,b},
```

where `T_{d,b}` contains the exact descended primitive Gauss/inverse-frequency kernel, including the factor generated by `G_q(a,b)=d G_{q/d}(a/d,b/d)`, together with all retained physical/common-parent variables.

Restriction to `d|b` can only decrease the original Fourier coefficient energy, so
...
Cauchy therefore gives the exact dual reduction
...
This is the safe operator formulation. We do **not** assert that the descended coefficient vector automatically satisfies the fresh equality `(1/q') sum|W|^2` with `q'=q/d`; such an equality would require an additional descended-weight identification. The gcd factor and any reindexing are kept inside the operator kernel instead.

Equivalently, if the remaining variables are written as a linear operator `T_d` acting on the restricted original `b`-coefficient vector, it is enough to prove an `ell^2 -> ell^2` or `ell^2 -> L^2(H_phys^MAIN)` operator-norm deficit for the completed inverse-frequency family. A bounded/subpolynomial rank-one factorization of the full physical coefficient is sufficient, but it is not necessary.
...
- the exact gcd-descent factor inside the operator kernel;
...
- the batch34 original normalized Fourier energy as the only charge for the `b` coefficients.
...
> Prove a uniform positive-power operator-norm bound for the primitive completed inverse-frequency kernel family on the exact MAIN wall measure, with coefficient norms charged to the original batch34 Fourier `L2` energy and all correlated-modulus/common-parent masks and gcd-descent factors retained. It is enough to realize the family inside a published bilinear/trilinear Kloosterman-fraction, dispersion, or Kuznetsov operator estimate; explicit rank-one separation is not separately required if the theorem is already formulated as a norm inequality.
...
DESCENDED_FRESH_PARSEVAL_IDENTITY_CLAIMED=false
GCD_DESCENT_FACTOR_RETAINED_IN_OPERATOR=true
