...
Stage17-10 has passed fresh audit. Its population contract is frozen as the Stage16 primitive/canonical exactly-one population with the additional requirement that `R` be integral. On the target `d=R` exactly, so the common `R<=B` cutoff agrees with `d<=B` without a comparison loss.

Stage17-20 now submits the first deterministic finite-data baseline on that audited population. The exact census is generated by the Stage16 Pythagorean-face coverage path, followed by global primitive/canonical filtering, exact-one face multiplicity rechecking, and the additional integral-`R` test.
...
An independent direct canonical-triple enumeration agrees with the optimized path through `B=200`. The frozen CSV SHA-256 is

`2f066143090713c25eec2e8ecef7a31d5c5ec169dc008380577757b34674168a`.

Matched Stage16 counts at the same cutoffs are retained only as diagnostics for the finite ratios `N_1(B)/M_1(B)`. Stage17-20 makes no asymptotic order, limiting ratio, decay exponent, upper/lower bound, independence claim, or causal conclusion.
...
Evidence level is `COMPUTED` for the finite census. Checkpoint 30 remains blocked until a fresh `Stage17-audit` certifies the enumerator, replay, frozen data, and evidence boundary.
