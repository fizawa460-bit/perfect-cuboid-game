...
外部再検算では、Stage12-N1-3a の長方形誤差そのもの

\[
R^{3/4+\varepsilon}S+RS^{3/4+\varepsilon}
\]

は独立に再計算されて一致した。一方、3a が入力としていた
...
b_q-2=-\frac4{q+1},
\qquad
0<b_q-1=\frac{q-3}{q+1}<1,
\]

したがって `sigma=Re(s)` に対し

\[
J_{\beta,q}(s)-1
=(b_q-2)q^{-s}-(b_q-1)q^{-2s}
\]
...
\[
|J_{\beta,q}(s)-1|
\ll q^{-1-\sigma}+q^{-2\sigma}.
\]

`p≡3 mod4` では `O(p^{-2sigma})` であるから、任意の固定

\[
\sigma\ge\frac12+\delta_0
\quad(\delta_0>0)
\]
...
\[
\boxed{
\sum_{n\ge1}\frac{|j_\beta(n)|}{n}<\infty.
}
\]
...
\[
U_q(s):=\sum_{k\ge1}\frac{\beta(q^k)}{q^{ks}}
=\frac{b_q q^{-s}}{1-q^{-s}}.
\]

coprime 条件により、二変数 local factor は

\[
D_q(s_1,s_2)=1+U_q(s_1)+U_q(s_2).
\]

一変数 local factorは `1+U_q(s)` なので

\[
D_q(s_1,s_2)
=(1+U_q(s_1))(1+U_q(s_2))C_q(s_1,s_2),
\]
...
V_q(s)
=b_q q^{-s}
\sum_{j\ge0}(-1)^j(b_q-1)^jq^{-js}.
\]

`V_q(s)=sum_{k>=1}v_{q,k}q^{-ks}` と書くと
...
従って `sigma_i>=1/2+delta` なら、`C_q-1=-V_q(s_1)V_q(s_2)` の二変数係数の weighted `l^1` norm は

\[
\ll_\delta q^{-\sigma_1-\sigma_2}
\le q^{-1-2\delta}O_\delta(1).
\]

素数和

\[
\sum_q q^{-1-2\delta}<\infty
\]

より Euler product の係数 `l^1` norm も収束する。すなわち
...
Selberg--Delange法そのものを本稿内で再証明はしない。外部定理として採用するのは次の有限次数展開だけである。
...
について、係数が固定次数の divisor majorant で支配され、`H` が `s=1` を含む標準 Selberg--Delange 領域
...
- coefficient majorant:
  ...
  \]
- analytic factor: §1.1 により `J_beta` は `Re s>1/2` の任意の閉部分半平面で絶対収束し正則。`L(s,chi_4)` は非主指標の Dirichlet L-function なので正則である。
- zero-free-shaped SD region: `c0>0` を十分小さく固定すれば
  \[
  \sigma>1-c_0/\log(2+|t|)
  \]
  は `J_beta` の正則領域内に入る。
- vertical growth: Stage12-N1-3h §2 の関数等式、Stirling、Phragmen--Lindelof による固定 strip の polynomial bound を使用する。
- nonzero leading factor: `L(1,chi_4)=pi/4` かつ各 `J_beta(1)` local factor は正で、絶対収束するため `H_beta(1)>0`。
...
で coefficient majorant も満たす。

従って `z=2` の有限次数 Selberg--Delange input を適用できる。
...
\boxed{
\sum_{n\le x}\beta(n)
=c_\beta x+O_A\bigl(x(\log(2x))^{-A}\bigr)
}
\]
...
\sum_{n\le x}g(n)
=x(c_g\log x+d_g)
+O_A\bigl(x(\log(2x))^{-A}\bigr)
}
\]

を使用する。

なお3aの粗い入力 `B_beta(X)<<X` は §1 で Selberg--Delange に依存せず直接証明済みである。
...
\[
a\le R^{1/2},\qquad b\le S^{1/2}
\]

とする。Selberg--Delange expansion を
