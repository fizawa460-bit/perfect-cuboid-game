...
def tangent_direction(local_row, side: int, parameter: str):
    ...
    pivot = int(local_row["affine_patch_pivot_coordinate_0based"])
    p0 = [clean(x.subs(h, 0)) for x in P]
    if clean(p0[pivot]) == 0:
...
def rees_surface(local_row, chart_p: int):
    ...
    return d, e, u, nonp, sub, pullbacks, strict, orders
...
def choose_nonzero_minor(jacobian, point_sub):
    for cols in itertools.combinations(range(jacobian.cols), jacobian.rows):
        ...
        if value != 0:
            return list(cols), det, value
    raise SystemExit("no nonzero Jacobian minor on crossing")
...
def main():
    ...
    for side_row in b2a["side_rows"]:
        ...
        for crossing in side_row["crossings"]:
            ...
            point, affine_pivot, affine_nonpivot, d, side_forms = affine_node_and_side_forms(local_row, side)
            ...
            d2, e, u, nonp, sub, pull_surface, strict_surface, orders = rees_surface(local_row, chart_p)
            if list(d2) != list(d):
            ...
            variables = [e, *u]
            surface_rank, surface_J = rank_at(strict_surface, variables, crossing_sub)
            if surface_rank != 4:
            ...
            for idx, s in enumerate(strict_side):
                ...
                if side_rank == 5:
                    ...
                    break
            if chosen is None:
                raise SystemExit(f"no principal side uniformizer candidate at {eid} side {side}")
            sidx, s_uniformizer, side_J = chosen
            minor_cols, delta, delta_value = choose_nonzero_minor(side_J, crossing_sub)

            Jsat = saturation_basis(strict_surface + [s_uniformizer], delta, variables)
            localized_full_side_ok = all(
                clean(Jsat.reduce(sp.expand(f))[1]) == 0 for f in strict_side
            )
            if not localized_full_side_ok:
                raise SystemExit(f"selected s does not generate strict side locally {eid} side {side}")
            ...
            cross_rank, cross_J = rank_at(strict_surface + [s_uniformizer, e], variables, crossing_sub)
            if cross_rank != 6:
                raise SystemExit(f"side/exceptional nontransverse crossing {eid} side {side}: rank {cross_rank}")
            cross_det = clean(cross_J.det())
            cross_det_value = clean(cross_det.subs(crossing_sub))
            if cross_det_value == 0:
                raise SystemExit("rank-6 determinant unexpectedly zero")
            transverse_pass += 1
            ...
            crossings_out.append({
                ...
                "surface_plus_side_uniformizer_jacobian_rank_at_crossing": 5,
                "surface_plus_side_plus_exceptional_jacobian_rank_at_crossing": cross_rank,
                "distinguished_open": {
                    "condition": "delta != 0",
                    "jacobian_minor_column_indices_0based": minor_cols,
                    "delta_Qi": encode_poly(delta, variables),
                    "delta_at_crossing_Qi": encode_element(delta_value),
                },
                "exact_local_checks": {
                    ...
                    "on_D_delta_surface_plus_selected_s_equals_full_strict_side_ideal": True,
                ...
                "cross_overlap_map": {
                    "source": f"{stored_chart['chart_id']}_SIDE_{side:03d}_DDELTA",
                    "target": stored_chart["chart_id"],
                    "open_embedding": "localize the same Rees chart at delta",
    ...
    cert = {
        "schema": "stage33.e3.v91c1x_r5b2b3a.a2_02_24_side_exceptional_crossing_uniformizer_charts.v1",
        "stage": "33-12",
    ...
    print(json.dumps({
        "success": True,
        "marker": "V91C1X_R5B2B3A_24_SIDE_EXCEPTIONAL_CROSSING_UNIFORMIZER_CHARTS",
        "side_count": 4,
