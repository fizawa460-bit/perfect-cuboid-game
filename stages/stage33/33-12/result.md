...
An elementary transform can preserve the same generic splitting data while changing determinant parity by the nonzero common Kc fiber class. Therefore generic splitting matrices alone cannot justify

```text
actual ct Pic/2 defect = 0.
```

The actual local rank-two lattices and their overlap transition matrices are load-bearing.
