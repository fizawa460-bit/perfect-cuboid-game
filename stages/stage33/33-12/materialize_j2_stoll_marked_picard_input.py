...
MQ:=ChangeRing(M,Rationals()); cq:=Vector(Rationals(),Eltseq(qPicK(BigK.22))); eq:=Vector(Rationals(),Eltseq(qPicK(BigK.e)));
cm:=cq*MQ^-1; em:=eq*MQ^-1; assert forall{x:x in Eltseq(cm)|Denominator(x) eq 1} and forall{x:x in Eltseq(em)|Denominator(x) eq 1};
printf "BEGINJ2\nPT=%o\nEX=%o\nMUL=%o\nDET=%o\nCQ=%o\nEQ=%o\nCM=%o\nEM=%o\nCI=%o\nEI=%o\nENDJ2\n",k,e,Multiplicity(CsK[22],t),d,[Integers()!x:x in Eltseq(cq)],[Integers()!x:x in Eltseq(eq)],[Integers()!x:x in Eltseq(cm)],[Integers()!x:x in Eltseq(em)],[pairingmatK[22,indlistK[r]]:r in [1..20]],[pairingmatK[e,indlistK[r]]:r in [1..20]];
'''
