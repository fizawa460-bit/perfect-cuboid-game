...
The exceptional divisor is identified semantically by the exact incidence
signature of its singular point against every pinned Stoll CsK curve.  The
ptsK enumeration is used only after this signature has a unique match; its
position is not treated as generator identity.
"""
...
extra=r'''
t := Pr5![1,0,0,0,-1,-1];
tsig := [t in CptsK[j] select 1 else 0 : j in [1..#CsK]];
matches := [k : k in [1..#ptsK] | [MatCPK[j,k] : j in [1..#CsK]] eq tsig];
assert #matches eq 1;
k := matches[1];
assert ptsK[k] eq t and #CsK eq 62 and t in CsK[22];
...
printf "BEGINJ2\nPT=%o\nMATCH=%o\nTSIG=%o\nEX=%o\nMUL=%o\nDET=%o\nCQ=%o\nEQ=%o\nCM=%o\nEM=%o\nCI=%o\nEI=%o\nENDJ2\n",k,#matches,tsig,e,Multiplicity(CsK[22],t),d,[Integers()!x:x in Eltseq(cq)],[Integers()!x:x in Eltseq(eqv)],[Integers()!x:x in Eltseq(cm)],[Integers()!x:x in Eltseq(em)],[pairingmatK[22,indlistK[r]]:r in [1..20]],[pairingmatK[e,indlistK[r]]:r in [1..20]];
'''
code='SetColumns(0);\nquick:=true;\nL<i,s>:=ext<Rationals()|Polynomial([1,0,1]),Polynomial([-2,0,1])>;\n'+text[ks:ke]+extra
out,ma=run_magma(code,180,'Stage33-12 J2 marked Kc Picard input',user_agent='perfect-cuboid-stage33/3.4-j2-marked-kc-picard-input')
...
pt=int(grab(out,'PT')); match=int(grab(out,'MATCH')); tsig=[int(x) for x in grab(out,'TSIG')]; ex=int(grab(out,'EX')); mul=int(grab(out,'MUL')); det=int(grab(out,'DET'))
...
assert match==1 and len(tsig)==62 and ex==62+pt and abs(det)==1 and mul>0 and all(len(x)==20 for x in (cq,eqv,cm,em,ci,ei))
cert={'schema':'STAGE33_12_J2_STOLL_MARKED_PICARD_INPUT_V2','source_locks':{'stoll_repository':'MichaelStollBayreuth/Verification','stoll_commit':'51233ed5ef2bf228fac9416c66db9adc0ebcaadd','stoll_path':'Cuboids/cuboids.magma','stoll_git_blob_sha1':blob,'j2_ruled_to_stoll_support_sha256':SUPSHA,'submitted_magma_code_sha256':hashlib.sha256(code.encode()).hexdigest()},'execution':{'source_fetch_attempt':fa,'magma_request_attempt':ma,'remote_cas_role':'pinned Kc Picard quotient and named J2 carrier/exceptional marking only','smith_form_computed':False,'full_surface_picard_recomputed':False,'automorphism_computed':False,'gersten_data_computed':False},'stoll_kc_marked_basis':{'basis':'indlistK','indlistK_1based':[2,4,5,7,9,10,20,21,26,35,39,42,44,47,49,52,54,64,67,72],'rank':20,'determinant':det,'unimodular':True},'named_J2_carrier':{'CsK_index_1based':22,'equations':['B1=0','i*A2-A3=0'],'contains_infinity_singularity':True,'multiplicity_at_infinity_singularity':mul,'magma_qPicK_coordinates':cq,'marked_indlistK_coordinates':cm,'intersection_signature_against_indlistK':ci},'named_J2_infinity_exceptional':{'singular_point':['1','0','0','0','-1','-1'],'semantic_identification':'UNIQUE_CSK_INCIDENCE_VECTOR_MATCH','incidence_signature_against_all_CsK':tsig,'incidence_signature_length':len(tsig),'unique_match_count':match,'derived_internal_ptsK_index_1based':pt,'ptsK_order_used_as_semantic_identity':False,'BigK_exceptional_index_1based':ex,'exceptional_tangent_attachment_direction':['1','i','0'],'magma_qPicK_coordinates':eqv,'marked_indlistK_coordinates':em,'intersection_signature_against_indlistK':ei},'exact_scope':{'J2_carrier_marked_PicK_class_materialized':True,'infinity_exceptional_marked_PicK_class_materialized':True,'infinity_exceptional_identity_order_independent':True,'finite_support_points_are_not_promoted_to_PicK_divisor_classes':True,'branch_jacobian_2torsion_to_picard_discriminant_kummer_glue_materialized':False,'J2_kc_discriminant_coordinate_materialized':False,'J2_q1_kc_adapter_unique':False,'GL2_F2_adapter_survivors':6,'finite_v4_kummer_defect_columns_materialized':0},'next_exact_leaf':'APPLY_NAMED_BRANCH_JACOBIAN_2TORSION_TO_KC_PICARD_TRANSCENDENTAL_KUMMER_GLUE_USING_MARKED_CARRIER_AND_EXCEPTIONAL_INPUT_THEN_FIX_J2_KERNEL_LINE','promotion_firewall':{'arithmetic_hs_d2_computed':False,'global_q_residue_lifts_complete':False,'stage33_12_closed':False,'stage33_07_closed':False,'stage33_08_released':False,'theorem_credit':False,'endpoint_credit':False,'receiver_credit':False,'perfect_cuboid_existence_claim':False,'perfect_cuboid_nonexistence_claim':False}}
cert['canonical_sha256']=sh(cert); OUT.write_text(json.dumps(cert,indent=2,sort_keys=True)+'\n')
print('STAGE33_12_J2_STOLL_MARKED_PICARD_INPUT=PASS_EXACT_ORDER_INDEPENDENT'); print('SEMANTIC_MATCH=UNIQUE_CSK_INCIDENCE_VECTOR'); print('INTERNAL_PTSK_INDEX_1BASED='+str(pt)); print('EXCEPTIONAL_BIGK_INDEX_1BASED='+str(ex)); print('J2_CARRIER_MARKED='+json.dumps(cm,separators=(',',':'))); print('J2_INFINITY_EXCEPTIONAL_MARKED='+json.dumps(em,separators=(',',':'))); print('CERTIFICATE_SHA256='+cert['canonical_sha256'])
