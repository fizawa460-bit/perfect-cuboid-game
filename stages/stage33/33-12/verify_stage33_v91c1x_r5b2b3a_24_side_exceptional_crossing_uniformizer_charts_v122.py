...
MAT = HERE / "materialize_e3_v91c1x_r5b2b3a_24_side_exceptional_crossing_uniformizer_charts_v2.py"
...
assert cert["schema"] == "stage33.e3.v91c1x_r5b2b3a.a2_02_24_side_exceptional_crossing_uniformizer_charts.v2"
...
for side_row, side in zip(cert["crossing_neighborhood_rows"], TARGET_SIDES):
    ...
    for crossing in side_row["crossing_neighborhoods"]:
        ...
        assert op["lambda_construction"] == "crossing transversality determinant times every side-Jacobian maximal minor nonzero at the crossing"
        assert op["crossing_transversality_determinant_at_crossing_Qi"] != ZERO_QI
        assert len(op["nonzero_side_jacobian_maximal_minors"]) >= 1
        for m in op["nonzero_side_jacobian_maximal_minors"]:
            assert len(m["column_indices_0based"]) == 5
            assert m["minor_at_crossing_Qi"] != ZERO_QI
...
print(json.dumps({
    "success": True,
    "marker": "V122_V91C1X_R5B2B3A_24_SIDE_EXCEPTIONAL_CROSSING_UNIFORMIZER_CHARTS",
