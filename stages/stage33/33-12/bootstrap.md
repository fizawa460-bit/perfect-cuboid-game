...
The boundary-function coefficient adapter is also now exact: all 134 packages in the 14-generator basis have `cc/ct` scalar ratio one, and the audited Stage33-11f span transports this to all 26 directions. Thus no constant-cokernel correction is introduced at the boundary-function scalar level. The correct full-surface Stage-B receiver is the cokernel of the proper adjustment map `d2_S|Br(Sbar)[2]^{G_Q}`; it is not the literal value on one arbitrary lift. The audited nonzero `q1` obstruction on the coordinate K3 is not promoted to the full surface without a pullback/glue adapter. Next, materialize that full-surface proper Kummer adjustment map and the first finite obstruction coset, or produce an exact Q-defined residue lift directly.
