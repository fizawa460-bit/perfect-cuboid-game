...
The boundary-function coefficient adapter is also now exact: all 134 packages in the 14-generator basis have `cc/ct` scalar ratio one, and the audited Stage33-11f span transports this to all 26 directions. Thus no constant-cokernel correction is introduced at the boundary-function scalar level. The correct full-surface Stage-B receiver is the cokernel of the proper adjustment map `d2_S|Br(Sbar)[2]^{G_Q}`; it is not the literal value on one arbitrary lift. The audited nonzero `q1` obstruction on the coordinate K3 is not promoted to the full surface without a pullback/glue adapter. The finite-V4 Kummer-defect interface is now a literal `75 x 10` matrix using exact quotient bases in the retained Picard and proper-Br coordinates. Next, fill its first column from a full-surface `mu_2` lift or equivalent unimodular glue datum, then apply the integral Bockstein and proper adjustment quotient.
