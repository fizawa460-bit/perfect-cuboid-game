...
MAT = HERE / "materialize_e3_v91c1x_r5b2b3a_24_side_exceptional_crossing_uniformizer_charts.py"
...
assert cert["schema"] == "stage33.e3.v91c1x_r5b2b3a.a2_02_24_side_exceptional_crossing_uniformizer_charts.v1"
...
for side_row, side in zip(cert["crossing_neighborhood_rows"], TARGET_SIDES):
    ...
    for crossing in side_row["crossing_neighborhoods"]:
        ...
        assert crossing["distinguished_open"]["condition"] == "delta != 0"
        assert len(crossing["distinguished_open"]["jacobian_minor_column_indices_0based"]) == 5
        assert crossing["distinguished_open"]["delta_at_crossing_Qi"] != ZERO_QI
        assert all(v is True for v in crossing["exact_local_checks"].values())
        overlap = crossing["cross_overlap_map"]
        assert overlap["target"] == crossing["canonical_rees_chart_id"]
        assert overlap["open_embedding"] == "localize the same Rees chart at delta"
...
# Full deterministic replay: reconstruct all 24 side/node crossings, the complete
# side proper-transform ideal in the selected Rees chart, the localized principal
# side equation, and the rank-6 side/exceptional transversality check.
...
print(json.dumps({
    "success": True,
    "marker": "V121_V91C1X_R5B2B3A_24_SIDE_EXCEPTIONAL_CROSSING_UNIFORMIZER_CHARTS",
