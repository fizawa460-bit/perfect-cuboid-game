...
NP, NB, H1, NSRC = 64, 14, 75, 10
PHI_SIZE = NB * NP
PHI_C_OFF, PHI_T_OFF = 0, PHI_SIZE
NVARS = 2 * PHI_SIZE
MASK64, MASK75 = (1 << NP) - 1, (1 << H1) - 1
...
def source_raw_pair_int(solution: int, source: list[int]) -> int:
    crows, trows = phi_rows(solution, PHI_C_OFF), phi_rows(solution, PHI_T_OFF)
    cc = ct = 0
...
def decode_coordinate(bit: int) -> dict[str, int]:
    return {"global_zero_based": bit, "source_zero_based": bit // H1,
            "source_one_based": bit // H1 + 1, "h1_zero_based": bit % H1,
            "h1_one_based": bit % H1 + 1}


pic, proper, target = locked(PIC, PIC_SHA), locked(PROPER, PROPER_SHA), locked(TARGET, TARGET_SHA)
Pc, Pt = expand_sparse(pic["objects"]["cc"]), expand_sparse(pic["objects"]["ct"])
Bc, Bt = proper["proper_Br2_cc_action_f2"], proper["proper_Br2_ct_action_f2"]
I64, I14 = identity(NP), identity(NB)
...
for source in retained:
    assert len(source) == NB and rowmul(source, Bc) == source and rowmul(source, Bt) == source
...
for a in range(NB):
    for p in range(NP):
        bits = 0
        for q in range(NP):
            if Pc[q][p]: bits ^= 1 << vindex(PHI_C_OFF, a, q)
        for b in range(NB):
            if Bc[a][b]: bits ^= 1 << vindex(PHI_C_OFF, b, p)
        equations.append(bits)
        bits = 0
        for q in range(NP):
            if Pt[q][p]: bits ^= 1 << vindex(PHI_T_OFF, a, q)
        for b in range(NB):
            if Bt[a][b]: bits ^= 1 << vindex(PHI_T_OFF, b, p)
        equations.append(bits)
for a in range(NB):
    for p in range(NP):
        bits = 0
        for q in range(NP):
            if Pt[q][p]: bits ^= 1 << vindex(PHI_C_OFF, a, q)
            if Pc[q][p]: bits ^= 1 << vindex(PHI_T_OFF, a, q)
        for b in range(NB):
            if Bc[a][b]: bits ^= 1 << vindex(PHI_T_OFF, b, p)
            if Bt[a][b]: bits ^= 1 << vindex(PHI_C_OFF, b, p)
        equations.append(bits)
...
variable_support = 0
for vector in matrix_image_basis.values(): variable_support |= vector
...
result = {
  "success": True, "schema": "STAGE33_KUMMER_E_75X10_ABSTRACT_V4_AMBIGUITY_V1",
  "scope": "ALL_F2_V4_MODULE_EXTENSIONS_COMPATIBLE_WITH_LOCKED_PIC_AND_PROPER_BR2_ACTIONS",
  "source_locks": {"parent_exact_head": "fa7949deee5e134eddc45aff7bc1d6e608e1f286",
  ...
  "revoked_relation_firewall": {"named_j2_target_used": False, "named_j2_source_target_relation_used": False},
  "extension_solution_space": {"variables_phi_cc_phi_ct": NVARS, "equations": len(equations),
    "rank_f2": len(pivots), "nullity_f2": len(free), "zero_extension_present": True},
  "matrix": {"rows_h1": H1, "columns_retained_sources": NSRC, "scalar_entries": H1 * NSRC,
    "residual_ambiguity_dimension_f2": ambiguity_dim, "residual_candidate_count": 1 << ambiguity_dim,
    "column_ambiguity_dimensions_f2": column_dims, "forced_columns_one_based": forced_columns,
    "forced_entry_count": len(forced_bits), "forced_entries_all_zero": True,
    "forced_entries": [decode_coordinate(bit) for bit in forced_bits]},
  "minimal_coordinate_measurement_plan": {"minimum_scalar_entry_measurements": ambiguity_dim,
    "reason": "pivot-coordinate projection is injective on D; fewer than dim(D) F2 scalar measurements cannot distinguish all states",
    "entries": [decode_coordinate(bit) for bit in measurement_pivots]},
  "firewall": {"abstract_module_extension_only": True, "actual_geometric_extension_identified": False,
    "geometric_existence_credit": False, "Q_defined_descent_credit_added": False,
