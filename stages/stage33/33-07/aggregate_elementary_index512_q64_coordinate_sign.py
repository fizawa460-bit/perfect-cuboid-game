...
D=HERE/'coordinate-sign-shards';rows=[];locks=[];endpoint_locks=set()
for i in range(16):
    ...
    rows.extend(x['results']);locks.append(x['canonical_sha256']);endpoint_locks.add(x['endpoint_coordinate_sign_sha256'])
if len(endpoint_locks)!=1:raise SystemExit('coordinate-sign shards used different endpoint packages')
...
cert={'schema':'STAGE33_07_ELEMENTARY_INDEX512_Q64_COORDINATE_SIGN_CENSUS_V1','candidate_count_before':64,'simultaneous_q_cc_ct_7sign_survivor_count':len(surv),'rejected_count':64-len(surv),'survivor_indices':surv,'endpoint_coordinate_sign_sha256':next(iter(endpoint_locks)),'shard_certificate_sha256':locks,'results':rows,'actual_index512_glue_identified':False,'next_exact_leaf':'L33-07-CLASSIFY-COORDINATE-SIGN-SURVIVORS-AGAINST-INTEGRAL-ENDPOINT-T' if surv else 'L33-07-ELEMENTARY-ORDER512-GLUE-REJECTED-BY-FULL-RATIONAL-SIGN-ACTION','unit_status':'RUNNING_REPAIR','stage33_progress':'6/11','stage33_08_released':False,'stage33_09_released':False,'theorem_credit':False,'endpoint_credit':False,'perfect_cuboid_nonexistence_claim':False}
