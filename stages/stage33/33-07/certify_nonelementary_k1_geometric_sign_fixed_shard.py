...
checks=[(0,4,14),(0,2,12)]
for piece in range(1,7):checks.extend(((piece,4,14),(piece,2,12)))
...
for ridx,record in owned:
    ...
    if not all_sections_stable_under_signs(record):
        unstable_records+=1;reject['H_STABILITY']+=64;continue
    stable_records+=1
    for solution in range(64):
        H=reconstruct_H(record,solution);verify_isotropic_H(H)
        for piece,power,want in checks:
            if fixed_Qpower_log2_direct(H,piece,power)!=want:
                reject[f'S{piece}_Q{power}']+=1;break
        else:
