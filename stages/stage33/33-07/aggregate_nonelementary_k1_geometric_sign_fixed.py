...
for path in SHARDS.glob('nonelementary-k1-geometric-sign-fixed-p*-s*.json'):
    x=json.loads(path.read_text());stored,got=rehash(x)
    if stored!=got:raise SystemExit(f'subshard hash regression: {path.name}')
    if x.get('schema')!='STAGE33_07_NONELEMENTARY_K1_GEOMETRIC_SIGN_FIXED_SUBSHARD_V1':raise SystemExit('subshard schema regression')
    p=int(x['p_orbit_index']);s=int(x['record_shard_index']);n=int(x['record_shard_count'])
    by_p[p].append((s,n,x))

if set(by_p)!=set(range(P_COUNT)):raise SystemExit(f'P-orbit subshard coverage regression: {sorted(by_p)}')
source_by_p={};reject=Counter();survivors=[];subshards_by_p={}
total_records=total_pairs=checked=weighted_checked=surv=weighted_surv=0
stable_records=unstable_records=0
for p in range(P_COUNT):
    entries=by_p[p];counts={n for _,n,_ in entries}
    if len(counts)!=1:raise SystemExit(f'inconsistent subshard count for P orbit {p}')
    scount=counts.pop();subshards_by_p[str(p)]=scount
    if {s for s,_,_ in entries}!=set(range(scount)) or len(entries)!=scount:raise SystemExit(f'missing/duplicate exact subshard for P orbit {p}')
    p_records=p_pairs=p_checked=p_weighted=0
    for s,_,x in sorted(entries):
        if int(x['p_orbit_index'])!=p or int(x['record_shard_index'])!=s:raise SystemExit('subshard coordinate regression')
        ...
        source_by_p[p]=meta
        nrec=int(x['owned_record_count']);pairs=int(x['owned_pair_skeleton_count']);c=int(x['representative_lift_sections_checked']);wc=int(x['weighted_H_checked'])
        if c!=64*nrec or wc!=64*pairs:raise SystemExit('owned coverage arithmetic regression')
        ...
        p_records+=nrec;p_pairs+=pairs;p_checked+=c;p_weighted+=wc
        total_records+=nrec;total_pairs+=pairs;checked+=c;weighted_checked+=wc;surv+=r;weighted_surv+=wr
...
cert={
 ...
 'p_orbit_count':P_COUNT,'record_subshards_by_p_orbit':subshards_by_p,'exact_support_skeleton_orbit_count':total_records,
...
print(json.dumps({'success':True,'support_orbits':total_records,'sections_checked':checked,'survivors':surv,'weighted_survivors':weighted_surv,'k1_rejected':rejected,'subshards_by_p':subshards_by_p,'rejections':dict(sorted(reject.items())),'sha256':cert['canonical_sha256'],'next':cert['next_exact_leaf']},indent=2,sort_keys=True))
