#!/usr/bin/env python3
"""Aggregate the exact 15x16 k=1 endpoint-group-type + seven-sign census."""
import hashlib,json
from collections import Counter,defaultdict
from pathlib import Path
HERE=Path(__file__).resolve().parent;D=HERE/'k1-geometric-sign-fixed-subshards';files=sorted(D.glob('nonelementary-k1-geometric-sign-fixed-p*-s*.json'))
...
print(json.dumps({'success':True,'support_orbits':support_orbits,'representative_sections_checked':checked,'weighted_H_checked':weighted,'group_type_matches':group_match,'group_type_rejections':group_rejections,'survivors':surv,'weighted_survivors':wsurv,'k1_rejected':zero,'rejections':dict(sorted(rej.items())),'sha256':cert['canonical_sha256'],'next':cert['next_exact_leaf']},indent=2,sort_keys=True))
