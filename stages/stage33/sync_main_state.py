...
def load(p,h):
    ...
    assert q==h==csha(b),(p,q,h,csha(b)); return o
...
def main():
    ...
    ag=s['authority_audit_gate']; assert ag['hostile_audit_verdict']=='PASS' and ag['merged'] is True
    r3a=s['continuation_provenance']['x_r3_grouped_hostile_audit']
    assert r3a['pr']==1678 and r3a['verdict']=='PASS' and r3a['exact_audited_head']=='cea554e0fc3cad4f745ed8c4a0582a34c54f1c98'
    assert r3a['merge_commit']=='3dc69788084fd58960997959aedf04660c187078'
    ...
    assert r2['canonical_sha256']==R2_SHA and r3['canonical_sha256']==R3_SHA and r4['canonical_sha256']==R4_SHA
    ...
    assert gate['next_expected_command']==AUDIT
    assert gate['stop_semantics']=='STOP_FOR_GROUPED_HOSTILE_AUDIT_NO_R5_OR_DOWNSTREAM_CREDIT'
    ...
    if a.check or not a.write:
        print(json.dumps({'success':True,'marker':'V114_V91C1X_R4_NEGATIVE_CHECKPOINT_GROUPED_AUDIT_STOP','state_sha256':STATE_SHA,'candidate_sha256':R4_SHA,'advance_allowed':False,'next_expected_command':AUDIT,'stage33_progress':'6/11'},sort_keys=True))
