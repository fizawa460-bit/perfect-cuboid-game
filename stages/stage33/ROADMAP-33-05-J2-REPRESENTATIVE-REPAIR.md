...
| R1 | Does the abstract `J2` basis element remain genuinely nonzero independently of the bad representative? | Recompute the quotient/presentation nonzero statement without using the revoked `ell_J2`; identify exactly what object the symbol `J2` denotes | **IN_PROGRESS** |
| R2 | Can a correct concrete representative of abstract `J2` be constructed? | Produce `ell_J2_corrected` and certify `[ell_J2_corrected] != 0` in `L^*/(K^*L^{*2})`, with branch/ruling dictionary source-locked | **BLOCKED_BY_R1** |
...
- R1 has only two legitimate outcomes: `ABSTRACT_J2_NONZERO_CONFIRMED` or `ABSTRACT_J2_SURVIVAL_REVOKED`. Do not search for new marked-coordinate bridges before R1 is resolved.
...
R1 = IN_PROGRESS: re-audit abstract J2 nonzero survival independently of ell_J2
R2 = BLOCKED
R3 = BLOCKED
R4 = BLOCKED
R5 = BLOCKED
