...
SOURCES = [
    Source("Active consolidated proof through Stage12-N1-3g", "docs/stage12-n1-2-final-r05.md", "ccdf97313c361862535f3aab59ed207caae38e08"),
    Source("Active rectangle-error derivation", "docs/stage12-n1-3a-rectangular-error-repair.md", "fb4a7a52fcb2cf55005c155b0534abf3a986fe42"),
    Source("Historical origin of the parameter sum and multiplicity G", "docs/stage12-n1-2b-average.md", "0430d010472314f0c88e78189493f652c261b3ef", HISTORICAL_PROVENANCE_COMMIT),
    Source("Historical exact divisor expansion of G", "docs/stage12-n1-2e-divisor-dyadic.md", "e8783ee5577419b9f8aeff1e93a3c4aaf44a7feb", HISTORICAL_PROVENANCE_COMMIT),
    Source("Historical derivation of kappa and the three-variable local factors", "docs/stage12-n1-2f-main-term.md", "5ba2f0844446a57dd524034fd9cd186f7a00fcb1", HISTORICAL_PROVENANCE_COMMIT),
    Source("Historical primitive-first derivation of A_rs, beta and gamma", "docs/stage12-n1-2j-boundary-layers.md", "111107ce0346606cb8a73b4c50e1841386f4cf23", HISTORICAL_PROVENANCE_COMMIT),
    Source("Historical derivation of eta and its comparison with kappa", "docs/stage12-n1-2k-final-remainder.md", "48b28e84034c17e242998ab313775b0894908515", HISTORICAL_PROVENANCE_COMMIT),
    Source("Active zero-base provenance, vertical-growth and radial-boundary closure", "docs/stage12-n1-3h-zero-base-provenance-closure.md", "7683303f47ef8d6d4d10f1bb1501657020956cfd"),
    Source("Active final reference closure: beta bound, cross norm, Selberg--Delange map", "docs/stage12-n1-3i-final-reference-closure.md", "c1279b3a9510077196cd5f4726512c964751e921"),
]
...
def read_source(source: Source) -> tuple[str, str]:
    if source.ref is None:
        ...
        text = subprocess.check_output(["git", "show", object_name], text=True, encoding="utf-8")
    if actual_blob != source.expected_blob:
        raise SystemExit(f"source blob mismatch for {source.locator}:{source.path}: expected {source.expected_blob}, got {actual_blob}")
...
def read_verified_sources() -> list[tuple[Source, str, str]]:
    ...
    if ledger_hash != SOURCE_LEDGER_SHA256:
        raise SystemExit(f"source ledger mismatch: expected {SOURCE_LEDGER_SHA256}, got {ledger_hash}")
    return verified
...
|---|---|---|---|
"""
    rows = "\n".join(f"| {source.label} | `{source.locator}` | `{source.path}` | `{blob}` |" for source, blob, _ in sources)
    sections: list[str] = []
    total = len(sources)
