...
def render(report: dict) -> str:
    ...
    table = "\n".join(
        f"| {r['B']:,} | {r['raw_oriented_chains']:,} | "
        f"{r['coprime_joint_oriented_before_distinctness']:,} | "
        f"{r['repeated_side_oriented_correction']:,} | "
        f"{r['primitive_distinct_oriented']:,} | {r['stage11_primitive_oriented']:,} |"
        for r in rows
    )
    return rf"""{START}
