...
> STATUS: `STAGE13_13F_R05_REPAIR_GATES_A_B_C_COMPLETE_GATE_D_NEXT`
>
...
The audit combines retained exact-one checkpoints from `B=1000` through `B=5000000` and retraces the non-circular common-`Theta` proof. The endpoint data move away from exact `2:1:1` and modestly toward the claimed limit from `B=100000` to `B=5000000`. No q-dependent leading arithmetic factor was found at the current proof level, while the absence of an effective convergence rate remains explicitly recorded.
...
The split-prime mixed correction is expanded coefficient by coefficient. The exact constant chain is
...
The exceptional split prime `p=5` is also given an explicit phase-uniform bound `<432`.
...
### 13-13fc — Gate C: curved-region / box-error accumulation

Status: `[x] Complete`.

The core mesh now has an explicit size. At `eta=(log B)^-8`, each coordinate requires `O((log B)^9)` multiplicative intervals, hence

```text
BOX_COUNT=O((log B)^27).
```

Choosing the fixed zero-mode Perron remainder order `N=64` gives `O(B(log B)^-62)` per box and therefore

```text
FINITE_REMAINDER_AFTER_ALL_BOXES=O(B(log B)^-35).
```

With `epsilon=1/16`, the rectangle power tails have saving

```text
exp(-(3/16)(log B)^(1/4)),
```

which still beats every fixed logarithmic power after the `27` box powers are inserted.

The remaining zero-mode global ledger is
...
and every row is `o(B(log B)^3)`. Boundary-intersecting boxes are placed in the explicit shell `2B e^(-3eta) <= h(r^2+s^2) <= 2B e^(3eta)`; their analytic rectangle errors are not hidden inside the shell estimate but charged to the already accumulated remainder ledger.
