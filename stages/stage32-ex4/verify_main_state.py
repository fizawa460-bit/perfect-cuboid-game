...
assert STATE["schema"]=="STAGE32EX4_MAIN_COMPACT_STATE_V7_KUUSALO_R4_SCRATCH"
...
assert STATE["current"]["leaf"]=="EX4-R4_KUUSALO_F2_BRANCH_ACTION_TO_CECOTTI_B8_ALGEBRAIC_BINDING_PREFLIGHT"
assert STATE["current"]["next_leaf"]=="EX4-R5_RECOVER_FULL_KUUSALO_FIGURE9_10_BRANCH_TO_H1_LABEL_OR_EXPLICIT_UNIFORMIZATION_MAP"
...
assert re["f2_branch_permutation_multiplicity"]==2
assert re["f2_branch_orbit"]=="r^k*B8*r^-k, k=0..3"
assert re["literal_B8_admissible"] is True
...
for k in [
 ...
 "second_generator_pair_transport_exact","f2_branch_action_finite_orbit_computed",
 "f2_branch_action_candidate_count_4","literal_B8_admissible"
]: assert front[k] is True,k
for k in [
 "period_equivalence_prunes_W_line","simultaneous_pair_data_prunes_W_line","literal_B8_source_bound",
...
credit=STATE["credit"]
assert credit["current_reentry_level"]=="SCRATCH_F2_BRANCH_ACTION_FOUR_CONJUGATES_NONPRUNING"
...
assert route["r2_unmarked_period_matching_classified_nonpruning"] is True
assert route["r3_unmarked_second_generator_pair_classified_nonpruning"] is True
...
for k in [
 ...
 "literal_B8_selected_from_admissibility_only","Cecotti_B8_used_to_choose_source_H1_embedding",
...
assert R4["finite_weierstrass_embedding_reconstruction"]["compatible_full_branch_embeddings"]==8
assert R4["finite_weierstrass_embedding_reconstruction"]["distinct_f2_branch_permutations"]==4
assert R4["group_crosscheck"]["literal_B8_is_admissible_but_not_unique"] is True
assert R4["decision"]["literal_Kuusalo_f2_equals_Cecotti_B8_proved"] is False
...
print("Stage32EX4 MAIN Kuusalo R4 scratch state: PASS")
print("R2 adapters=48 line=16/16/16; R3 pairs=24 line=8/8/8")
print("R4 branch embeddings=8 -> four r^k B8 r^-k candidates; literal B8 not selected")
print("residue97 conditional only; absolute=false")
