#!/usr/bin/env python3
"""Replay persisted V59 Goal4V state while repaired V60 Goal4W is live."""
...
assert real['hostile_audit_repair']['verdict']=='FAIL_SCOPE'
assert real['claims']['goal4w_executed'] is True
assert real['claims']['full_picard_galois_module_computed'] is True
assert real['claims']['proper_surface_full_Picard_H1_computed'] is True
assert real['claims']['proper_surface_full_Picard_H1_trivial'] is True
assert real['claims']['proper_surface_algebraic_brauer_quotient_trivial'] is True
assert real['claims']['open_receiver_Picard_group_computed'] is False
...
print(f'PASS V60_REPAIRED_PERSISTED_V59_REPLAY_{target}')
