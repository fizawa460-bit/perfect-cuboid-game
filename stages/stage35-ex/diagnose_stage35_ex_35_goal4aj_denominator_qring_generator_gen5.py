#!/usr/bin/env python3
"""Goal4AJ diagnostic: denominator degree-19 residual via native Singular qring products, generation5.
...
Diagnostic only: no Q-normalized degree31 denominator/F_B/E1 credit.
"""
...
def emit(m,ids):
 ...
 parts.append(f'print("GOAL4AJ_DEN_QRING_GROUP_BASE_DONE={m}:"+string(size(G)));\n')
 ...
 parts.append(f'print("GOAL4AJ_DEN_QRING_GROUP_POWER_DONE={m}:"+string(size(D{m})));\n')
for m in [13,9,7,3,2,1]: emit(m,GROUPS[m])
parts.append(r'''
ideal A=qprod(D13,D9); ideal B=qprod(D7,D3); ideal C=qprod(D2,D1); ideal AB=qprod(A,B); ideal QRAW=qprod(AB,C);
print("GOAL4AJ_DEN_QRING_RAW_GENERATORS="+string(size(QRAW)));
setring r;
ideal L=imap(qr,QRAW); ideal RAW=std(surf+L); list FL=sat(RAW,Sing); ideal D=std(FL[1]);
print("GOAL4AJ_DEN_QRING_FINAL_HULL_DONE="+string(size(D)));
...
print("GOAL4AJ_DEN_QRING_FINAL_GENERATORS="+string(size(D))); print("GOAL4AJ_DEN_QRING_NONSURF_GENERATORS="+string(nonSurf)); print("GOAL4AJ_DEN_QRING_MIN_DEG="+string(minDeg)); print("GOAL4AJ_DEN_QRING_DEG19_COUNT="+string(d19));
if(minDeg!=19){ERROR("minimum nonsurface generator degree is not 19");} if(d19<1){ERROR("no degree19 candidate");}
print("GOAL4AJ_DEN_QRING_CANDIDATE_BEGIN"); print(string(candidate)); print("GOAL4AJ_DEN_QRING_CANDIDATE_END"); print("GOAL4AJ_DEN_QRING_BUILD=PASS"); quit;
''')
script=''.join(parts)
with tempfile.TemporaryDirectory() as td:
 p=Path(td)/'goal4aj-den-qring-gen5.sing'; p.write_text(script,encoding='utf-8')
 try: cp=subprocess.run(['Singular','-q',str(p)],text=True,capture_output=True,timeout=1500); timed_out=False
 except subprocess.TimeoutExpired as exc:
  stdout=exc.stdout or ''; stdout=stdout.decode('utf-8',errors='replace') if isinstance(stdout,bytes) else stdout; marks=[x for x in stdout.splitlines() if x.startswith('GOAL4AJ_DEN_QRING_')]; print('GOAL4AJ_DEN_QRING_TIMEOUT_MARKERS='+json.dumps(marks[-80:])); raise SystemExit('Goal4AJ denominator qring gen5 timed out; no mathematical obstruction credit')
...
try: i0=lines.index('GOAL4AJ_DEN_QRING_CANDIDATE_BEGIN'); i1=lines.index('GOAL4AJ_DEN_QRING_CANDIDATE_END'); candidate='\n'.join(lines[i0+1:i1]).strip()
except ValueError: candidate=''
completion='GOAL4AJ_DEN_QRING_BUILD=PASS' in lines
def val(p): return int(next(x for x in lines if x.startswith(p)).split('=',1)[1])
out={'schema':'STAGE35_EX_GOAL4AJ_DENOMINATOR_QRING_GENERATOR_DIAGNOSTIC_V5','source_locks':{'active_divisor_condition_packet_canonical_sha256':ACTIVE_SHA,'parent_generator_blob_sha1':PARENT_BLOB,'degree31_divisor_packet_sha256':DIVISOR_PACKET_SHA,'denominator_rigidity_canonical_sha256':DEN_RIGIDITY_SHA,'qring_product_preflight_canonical_sha256':QRING_PREFLIGHT_SHA,'peeled_q_factor_canonical_sha256':PEELED_FACTOR_SHA},'constructor':'native Singular qring products; one ambient lift and one final A1 saturation','strict_prime_count':22,'strict_total_multiplicity':102,'multiplicity_groups':{str(m):GROUPS[m] for m in sorted(GROUPS,reverse=True)},'singular_returncode':cp.returncode,'singular_error_text_present':error_text,'singular_timed_out':timed_out,'completed_group_base_count':sum(x.startswith('GOAL4AJ_DEN_QRING_GROUP_BASE_DONE=') for x in lines),'completed_group_power_count':sum(x.startswith('GOAL4AJ_DEN_QRING_GROUP_POWER_DONE=') for x in lines),'final_hull_completed':any(x.startswith('GOAL4AJ_DEN_QRING_FINAL_HULL_DONE=') for x in lines),'final_standard_basis_generator_count':val('GOAL4AJ_DEN_QRING_FINAL_GENERATORS=') if completion else None,'nonsurface_standard_basis_generator_count':val('GOAL4AJ_DEN_QRING_NONSURF_GENERATORS=') if completion else None,'minimum_nonsurface_generator_degree':val('GOAL4AJ_DEN_QRING_MIN_DEG=') if completion else None,'degree19_candidate_count':val('GOAL4AJ_DEN_QRING_DEG19_COUNT=') if completion else None,'candidate_materialized_over_split_field':bool(candidate),'candidate_text_sha256':hashlib.sha256(candidate.encode()).hexdigest() if candidate else None,'candidate_text_bytes':len(candidate.encode()),'candidate_contains_extension_symbol_u':('u' in candidate),'candidate_text':candidate if candidate and len(candidate.encode())<=60000 else None,'candidate_text_persisted_inline':bool(candidate) and len(candidate.encode())<=60000,'peeled_degree12_q_factor':'c*b1^11','q_coefficient_normalization_completed':False,'peeled_degree12_q_factor_reattached':False,'literal_degree31_denominator_materialized':False,'literal_F_B_materialized':False,'E1_proved':False,'stage35_closed':False,'theorem_credit':False,'endpoint_credit':False}
out['canonical_sha256']=hashlib.sha256(json.dumps(out,sort_keys=True,separators=(',',':')).encode()).hexdigest(); print('GOAL4AJ_DEN_QRING_JSON='+json.dumps(out,sort_keys=True,separators=(',',':')))
if cp.returncode!=0 or error_text or not completion or not candidate:
 if stdout: print('GOAL4AJ_DEN_QRING_STDOUT_TAIL='+json.dumps(stdout[-12000:]));
 if stderr: print('GOAL4AJ_DEN_QRING_STDERR='+json.dumps(stderr[-12000:]));
 raise SystemExit('Goal4AJ denominator qring gen5 failed closed')
print('GOAL4AJ_DEN_QRING=PASS')
