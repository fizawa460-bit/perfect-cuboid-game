...
BASE = '2949391d032a9014b0e9ffebce78be84a93d1d8a'
...
assert state['parent_authority']['merge_sha'] == BASE
...
assert art['parent_goal4d_authority']['merge_sha'] == BASE

# Small-prime exact projective census.  On the all-edge-nonzero branch scale A=1.
def qset(p: int) -> set[int]:
...
def nonzero_projective_counts(p: int) -> tuple[int,int]:
    ...
    for B,C in itertools.product(range(1,p), repeat=2):
        ...
        if not all(v in Q for v in vals):
            continue
        face += 1
        if (1+B*B+C*C)%p in Q:
            full += 1
...
assert forced == art['all_odd_prime_zero_support_theorem']['exact_forced_prime_set']

# Exact exceptional projective zero-edge patterns, using first-nonzero normalization.
def normalize_projective(v: tuple[int,int,int], p: int) -> tuple[int,int,int]:
    for x in v:
        if x % p:
            inv = pow(x,-1,p)
            return tuple(t*inv % p for t in v)
    raise AssertionError('zero vector')

def full_projective_classes(p: int):
    Q = qset(p)
    classes = set()
    for A,B,C in itertools.product(range(p), repeat=3):
        ...
        if all(v in Q for v in vals):
            classes.add(normalize_projective((A,B,C),p))
    return classes

expected_class_counts = {3:3,5:9,7:9,11:15,19:27}
for p in forced:
    cls = full_projective_classes(p)
    assert len(cls) == expected_class_counts[p]
    zero_sizes = {sum(x==0 for x in v) for v in cls}
    assert zero_sizes == ({2} if p == 3 else {1,2})
    assert all(any(x==0 for x in v) for v in cls)

# Saunderson specialization. All identities and smoothness data are verified symbolically.
s = sp.symbols('s')
a = 6*s**5 - 20*s**3 + 6*s
...
disc = int(sp.discriminant(F,s))
assert sp.factorint(disc) == {2:72,5:4}
assert sp.degree(F,s) == 8
# Smooth y^2=F(s), degree 2g+2=8 => genus 3 for odd p !=5.
assert art['large_prime_completion']['smooth_genus'] == 3
# Bad s are roots of a*b*c: at most deg(a)+deg(b)+deg(c)=16; each has <=2 curve points.
assert sp.degree(a,s)+sp.degree(b,s)+sp.degree(c,s) == 16
assert art['large_prime_completion']['bad_affine_points_upper_bound'] == 32
# Two points at infinity for even degree with square leading coefficient.
assert art['large_prime_completion']['points_at_infinity'] == 2
# Hasse-Weil lower bound p+1-2g sqrt(p); verify the stated uniform threshold.
def lower(p: int) -> float:
    return p + 1 - 6*math.sqrt(p)
assert lower(89) <= 34 < lower(97)
assert all(lower(p) > 34 for p in sp.primerange(97,1000))
# Standard Hasse-Weil then supplies >34 total points for every prime p>=97;
# after removing <=2 infinity and <=32 edge-zero points, a good all-edge-unit point remains.

# Small nonexceptional primes are already witnessed directly by the census.
assert all(full > 0 for p,face,full in rows if p not in forced)

# Global integer consequence.
assert art['global_integer_consequence']['combined_divisor'] == 65835
assert 3**2 * 5 * 7 * 11 * 19 == 65835

# Finite joint edge-support receiver and exact S3 orbit census.
edges = (0,1,2)
single = [frozenset([i]) for i in edges]
double = [frozenset(set(edges)-{i}) for i in edges]
opts3 = double
opts = single + double
assignments = list(itertools.product(opts3,opts,opts,opts,opts))
assert len(assignments) == 3888
perms = list(itertools.permutations(edges))
def act(S,p): return frozenset(p[i] for i in S)
def canon(asg):
    return min(tuple(tuple(sorted(act(S,p))) for S in asg) for p in perms)
orbits = {canon(a0) for a0 in assignments}
assert len(orbits) == 656
assert art['finite_global_forced_prime_support_receiver']['labeled_joint_support_assignments'] == 3888
...
assert boundary['finite_global_forced_prime_support_receiver_obtained'] is True
...
assert state['claims']['goal4e_executed'] is True
assert state['claims']['all_odd_primes_classified_for_universal_edge_zero_obstruction'] is True
assert state['claims']['forced_odd_prime_set'] == [3,5,7,11,19]
assert state['claims']['forced_ABC_divisor'] == 65835
assert state['claims']['finite_global_forced_prime_support_receiver_obtained'] is True
