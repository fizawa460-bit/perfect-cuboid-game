...
UPSTREAM_R401A_PR=1026
UPSTREAM_R401A_MERGE_COMMIT=05f460c6df069f9b6da58409bc19378920a5666f
...
Stage27-r401a hostile intermediate audit passed and PR #1026 merged at `05f460c6df069f9b6da58409bc19378920a5666f`. Its accepted continuation order puts `R401-NEXT-MAIN-CRT2` first. This route remains inside checkpoint40 and does not authorize checkpoint50.
...
Indeed, if `N_rec<=B^{epsilon_B}` with `epsilon_B->0`, then `N_rec^r<=B^{r epsilon_B}=B^{o(1)}` for fixed `r`.

Therefore

\[
\boxed{S_r\text{ and }\#T_{\rm rec}\text{ have the same fixed-power exponent for every fixed }r.}
\]
...
\[
\boxed{\#\{(u,v)\in R_{\rm prim}^{\rm wall}:N_{\rm rec}(u,v)>0\}\ll B^{\kappa-\delta+o(1)}}
\]
...
Equivalent acceptable input: an upper bound for any one fixed moment `S_r` with the same fixed-power deficit. The lemma shows that such a moment theorem is useful only because it proves a support deficit; changing `r` itself supplies no saving.
