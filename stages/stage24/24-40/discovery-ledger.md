...
Checkpoint40 policy requires:

- fresh Stage19 upper surgeon first;
- no stopping at the inherited half-power theorem;
- explicit strict sub-square-root attack;
- Q06 moving-family boundary recheck;
- local-sieve growing-modulus uniformity recheck;
- actual source-level attack opening.

All six were executed.
...
| U40-S5 | centered character / dispersion | current-input negative certificate `kappa=1`; Stage15 internal routes exhausted | NO |
| U40-S6 | million-scale / B500m numerical pressure | finite slopes below/away from stable 1/2, diagnostic only | NO |

## New rigorous localization from checkpoint40

The fixed-curve branch can be sharpened using the merged Stage14-4ak void.
...
Thus the residual square-root obstruction is simultaneously:
...
The exact split-prime acceptance satisfies `rho_p=1-4/p+O(p^-2)`. The fixed-prime zero-density theorem has no uniform error for a prime set growing with `B`.

Even granting a hypothetical legal range `p<=B^theta`, the natural product is only `(log B)^(-2+o(1))`. Therefore this precise local parity tensor cannot explain a fixed `B^-delta` gain by itself. A strict sub-square-root theorem needs a genuinely global correlation/dispersion/small-point input, not merely a longer block of the same local factors.

## Q06/moving-family audit
...
Exact finite data are used only as diagnostics:

- `N2(1m)=255` in the matched r202 census;
- the 100k->1m finite `N2` exponent is about `0.457`;
- Stage14's larger B500m census fails the predeclared `N2/sqrt(B)` stability gate.
...
FRESH_UPPER_SURGEON_FIRST=true
