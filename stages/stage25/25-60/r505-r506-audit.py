...
reaudit2=(root/'stages/stage25/25-60/r505-r506-audit-recheck2.md').read_text() if (root/'stages/stage25/25-60/r505-r506-audit-recheck2.md').exists() else ''
...
for marker in ['R504_STANDARD_Q_KUMMER_IDENTIFICATION=false','R504_SAFE_KUMMER_CLASS=Q_FORM_OR_TWIST_OF_PRODUCT_KUMMER','R504_KUMMER_MODEL_EXACT_OVER_Q=false']:
    assert marker in r504
...
for marker in ['R504_QUARTIC_REAL_X_RANGE=[-2F,0]','R504_REAL_COMPONENT_GROUP=Z/2','R504_GENERATOR_COMPONENT=NONIDENTITY','R504_EVEN_MULTIPLES_COMPONENT=IDENTITY','R504_EVEN_MULTIPLES_ARE_QUARTIC_IMAGES=false','R504_ALL_PHYSICAL_MULTIPLES_ODD_LEMMA_MATERIALIZED=true','R504_FIRST_NONDEGENERATE_PHYSICAL_MULTIPLE_INDEX=3','R504_ALL_MULTIPLES_COUNT_UPPER=O(B^(1/9)*sqrt(log B))']:
    assert marker in r504, marker

for marker in ['STATUS=NORMATIVE_FOR_STAGE25_60_CONTINUATION','remaining open items require genuinely new external mathematics, not another unexecuted repo-native mutation.','R504_EXCEPTIONAL_BASE_CHANGE_RESIDUAL=LIVE_EXPLICIT_CURVE_SEARCH','CHECKPOINT60_DEEP_STOP_RULE_SATISFIED=false','STAGE70_ALLOWED=false']:
...
assert ctl['next_checkpoint']==60 and ctl['stage70_allowed'] is False
...
if ctl['audit_status']=='PENDING':
    ...
    assert ctl['advance_allowed'] is False and ctl['merge_allowed'] is False
    assert ctl['next_expected_command']=='Stage25-audit'
elif ctl['audit_status']=='PASS':
    assert ctl['status']=='AUDITED_PASS_CONTINUE_CHECKPOINT60'
    assert ctl['advance_allowed'] is True and ctl['merge_allowed'] is True
    assert ctl['repair_2']['all_physical_multiples_odd_lemma_accepted'] is True
    assert ctl['repair_2']['growing_multiple_count_upper_accepted'] is True
    assert ctl['r504_residual']['growing_multiple_count_upper_audit_status']=='PASS'
    assert ctl['r504_residual']['growing_multiple_route']=='CLOSED_NO_QUARTER_UPGRADE_WITH_HEIGHT_CERTIFICATE_AUDITED_PASS'
    for marker in ['AUDIT_VERDICT=PASS','R504_ALL_MULTIPLES_COUNT_UPPER_ACCEPTED=true','R504_EXCEPTIONAL_BASE_CHANGE_RESIDUAL=LIVE_EXPLICIT_CURVE_SEARCH','STAGE70_ALLOWED=false']:
        assert marker in reaudit2, marker
else:
    raise AssertionError(ctl['audit_status'])
...
print(f"STAGE25_60_REPAIR_2_AUDIT_STATUS={ctl['audit_status']}")
