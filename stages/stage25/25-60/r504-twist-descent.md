...
Direct substitution gives `v^2=u^3-4u`, and the inverse has `k=2u/v` away from the usual exceptional points.
...
Because `psi` identifies `J(C)` with `E_0`, this is `End_Q(E_0)`. The curve `E_0` has `j=1728`; its nonintegral CM endomorphisms require `i`, hence
