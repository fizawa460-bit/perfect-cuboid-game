...
# Exact truth-table identities.  A triple-face object lies in every raw pair;
# an exactly-two object lies in exactly one pair determined by its shared edge.
for ia in (0, 1):
    for ib in (0, 1):
        for ic in (0, 1):
            # indicators correspond to faces ab, ac, bc
            faces = ia + ib + ic
            triple = int(faces == 3)
            pa = ia * ib
            pb = ia * ic
...
# Exponent/log arithmetic.
M2j = (Fraction(1), Fraction(5))
M1 = (Fraction(2), Fraction(1))
M3upper = (Fraction(1), Fraction(5))
ratio = (M2j[0]-M1[0], M2j[1]-M1[1])
assert ratio == (Fraction(-1), Fraction(4))
# M3 carries an extra negative fixed log exponent eta relative to M2,j.
...
assert controller["current_phase"] == 40
...
if controller["status"] == "PHASE40_SUBMITTED_PENDING_FRESH_AUDIT":
    ...
    # Audit may later promote this immutable theorem bundle; no bypass to phase50.
