...
Taking the difference of the squares,
...
a power of two fixed by the 2-primary refinement. Using `n_-n_+=oddpart(alpha*delta)`,
...
Divide the first coupled equation by `m_-^2` and the second by `n_-^2`:

```text
a_+^2 x^2-a_-^2 = c*(n_-^2/m_-^2)*y,

b_+^2 y^2-b_-^2 = d*(m_-^2/n_-^2)*x.
```
...
kappa := 16*epsilon_k*epsilon_xi*r*s*X*Y.
```
...
```text
ChargedOnceReciprocalRatioBiquadraticIncidence.
```

It consists of positive rational points `(x,y)` on the ratio equation satisfying:
...
- integrality of the absolute scales recovered from either normalized equation;
...
X5 does not multiply that support count by a separate curve saving. Instead it uses the short cofactors as coefficient data in the ratio curve. For fixed residual/2-primary decoration, only `B^o(1)` factor-pair choices of their odd parts occur.
...
- the ratio identity holds as an exact Fraction identity;
...
The next obstruction is now an explicit low-degree algebraic curve family. Before any external elliptic/large-sieve theorem audit, X6 should classify the singular locus of the ratio curve and determine the exact physical height map. Only after that theorem shape is frozen would an H-line applicability audit be useful.
