...
Therefore the external theorem does **not** need to supply:

```text
moving modulus averaging,
fixed-U projective coefficient removal,
two-frequency reduction,
Hecke conductor d^2 compression,
fixed-power fractional/mismatch support savings.
```
...
Recent improvements, including Dong–Robles–Zeindler (arXiv:2601.00292), remain in moving-denominator Kloosterman-fraction geometry.
...
Blomer–Pascadi, *Bilinear forms with Kloosterman sums via quadratic characters* (arXiv:2607.24311), proves all-moduli bilinear Kloosterman bounds and in the critical square-root range obtains a `c^-1/32` saving over trivial size.
...
1. the Kloosterman arguments fall into a theorem range;
2. no second independent frequency or modulus family is recreated;
3. the canonical Gaussian-prime weight is admissible;
...
The t82 refinement narrows the obstruction from a generic affine single-frequency problem to
