...
Equivalently, the lattice determinant is

```text
det Lambda = uv.
```

This is the medium-range replacement for the sparse occupancy argument of s5l.
...
There is a second bound from the linear condition.
...
Combining that exact line with `m-rn == 0 (mod v)` forces the scalar parameter on the line to be divisible by `v`.  Indeed `r`, `1-r`, and `1+r` are all units modulo every odd prime divisor of `v`.  Therefore
...
with an absolute constant `c>0`, uniformly in all four linear columns and all norm-root patterns.

This lower bound is the main new geometric input of s5m.
...
At an odd prime, a fixed projective root has primitive local density `1/(p+1)`.  Therefore put
...
where `c_Omega` is the primitive opposite-parity area constant used in s5i--s5l.

Define the signed-root discrepancy
...
Apply ordinary rational Möbius inversion to the primitive condition.  For each Möbius variable `d`, after separating the finitely many factors sharing `uv`, the remaining lattice is a dilation of `Lambda(i;u,v,r)` by `d`.  Its determinant is `d^2 uv` and its shortest vector is at least

```text
d * c*K(u,v).
```

For a convex planar region, the elementary lattice-point estimate is
...
The area terms reproduce the primitive Euler product and hence the factor

```text
lambda(u)lambda(v).
```

The boundary terms sum as

```text
sum_d P_Omega/(d*K) << B^epsilon P_Omega/K.
```

The number of relevant dilations is `O(B^epsilon(1+P_Omega/K))`; the factors of `d` meeting `uv` cost only divisor factors.  This proves the stated pointwise bound.

The proof uses only lattice index, shortest-vector geometry, Möbius inversion, and the same fixed parity bookkeeping already used in s5k.
...
Since `2^omega(v) <<_epsilon B^epsilon`, the root multiplicity is harmless at the `B^epsilon` level, and
...
Thus the mixed-sign `D*S` representation is not required for a pointwise medium estimate: retaining the signed roots converts the problem to ordinary lattices first, and the root sum is taken only afterwards.
...
Let `G` denote the regular two-dimensional geometric scale (`G` comparable to area) and let `P` dominate `P_Omega`.  If a dyadic block satisfies
...
Thus whenever the cutoff is chosen so that
...
Therefore the medium norm-root problem is no longer an unstructured `D*S` obstruction.  Its central dyadic region is controlled by a signed-root lattice theorem; only boundary corridors remain quantitatively delicate.
...
Because distinct linear columns are odd-coprime on a primitive Euclid pair, `(x,v)=1`.  Therefore the Jacobi symbol rewrites exactly as
...
If the sign of `x` is not fixed by the chosen box, the missing sign is only the one-variable factor `(-1/v)` and is absorbed into the already allowed mod-4 state character.

After the standard mod-4 reciprocity partition, the same rewrite applies to an edge originally oriented as `(v/u)`.

Hence every large linear boundary strip is converted to
...
This switching is exact; no estimate has yet been used.
...
So the remaining obstruction is no longer a geometric incidence matrix or a mixed-sign `D*S` collision.  It is an **assembly/boundary problem**: one small state/cofactor variable must be combined with physical quadratic-character sums and the simultaneous finite state-polynomial expansion without losing the target exponent.
...
- exact index `uv` of every tested fixed-root/linear congruence lattice by residue enumeration;
...
- exact Jacobi rewrite `(u/v)=(|L_i|/v)(k/v)` on all tested primitive linear-edge states.
