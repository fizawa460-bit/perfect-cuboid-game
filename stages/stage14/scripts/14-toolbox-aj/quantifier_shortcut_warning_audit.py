...
def main() -> None:
    ...
    atlas_locks = [
        "local state",
        "coordinate incidence -> packet existence",
        "local admissibility -> global rational point",
        "fixed genus-one point bound -> moving-family bound",
        "sector exponent -> whole-family exponent",
        "deterministic root-sign divisor allocation -> Bernoulli density",
        "automatic square factor -> fresh second `1/q` saving",
        "fixed-fiber `B^o(1)` multiplicity -> active-direction sparsity",
        "historical threshold -> current whole-family gap",
        "SOURCE LEVEL",
        "TARGET LEVEL",
    ]
