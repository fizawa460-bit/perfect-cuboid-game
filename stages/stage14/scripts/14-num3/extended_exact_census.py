...
This is a finite numerical observatory tool, not an asymptotic argument.

The num2 production kernel kept full hypotenuse + leg indexes and an exact mate
set in RAM.  Num3 changes only the engineering architecture for larger bounds:

* partition the *shared face hypotenuse* p by p mod C;
* in each chunk retain only the Pythagorean faces whose hypotenuse belongs to
  that residue class;
* stream every outer Pythagorean triple p^2+z^2=d^2 instead of storing a global
  leg index;
* use exact integer-square tests for the two possible second faces;
* union the deterministic chunk ledgers and canonical-deduplicate.

Every retained object is revalidated with exact integer arithmetic.  The subset
with d<=2,000,000 must reproduce the four Stage14-num1 SHA-256 locks exactly.
"""
...
def primitive_pythagorean_triples(bound: int):
    """Yield primitive (u<v,w) with u^2+v^2=w^2 and w<=bound."""
    m = 2
    while m * m + 1 <= bound:
        for n in range(1, m):
...
def build_hypotenuse_chunk(bound: int, chunk_index: int, chunk_count: int):
    """Build only face triples whose hypotenuse p satisfies p % C == chunk."""
    ...
    for u, v, w in primitive_pythagorean_triples(bound):
        ...
        max_k = bound // w
        scaled_seen += max_k
        # First k in this arithmetic progression satisfying (k*w) mod C = idx.
        # C is tiny (8 in the canonical run), so the simple loop is clearer and
        # keeps this code auditable.
...
def enumerate_chunk(bound: int, chunk_index: int, chunk_count: int):
    ...
    for u, v, w in primitive_pythagorean_triples(bound):
        ...
        max_k = bound // w
        for k_outer in range(1, max_k + 1):
            l1 = k_outer * u
            l2 = k_outer * v
            d = k_outer * w
            ...
            for p, z in ((l1, l2), (l2, l1)):
                ...
                for x, y, face_scale in faces:
                    ...
                    # gcd(x,y,z)=gcd(face_scale,z) because (x,y) is a scaled
                    # primitive Pythagorean pair.
                    if gcd(face_scale, z) != 1:
                        profile["early_nonprimitive_rejects"] += 1
                        continue
                    # x-y is already an integral face. Test x-z and then y-z
                    # exactly; no floating-point integrality test is used.
...
def summarize(objects):
    ...
    for rec in sorted(objects):
        ...
        for edge in object_edges(rec):
            if edge in edges:
                # A raw-pair edge should identify one physical pair incidence.
                # Reaching the same object through both integral faces does not
                # duplicate the edge after canonical object deduplication.
                continue
            edges.add(edge)
            u, v = edge
...
def merge_reports(bound: int, paths):
    ...
    if seen_chunks != list(range(chunk_count)):
        raise ArithmeticError(f"missing/duplicate chunks: {seen_chunks}, expected 0..{chunk_count-1}")
    ...
    num1 = load_num1()
    baseline_objects = {r for r in objects if r[3] <= REGRESSION_BOUND}
    baseline = summarize(baseline_objects)
    h = num1["hashes"]
    four_hashes_match = (
    ...
    counts_match = baseline["counts"] == num1["counts"]
    graph_match = all(baseline["graph"][k] == num1["graph"][k]
                      for k in ("raw_pair_edges","active_oriented_face_vertices","max_degree"))
    if not (four_hashes_match and counts_match and graph_match):
        raise ArithmeticError("num3 memory-bounded kernel failed num1 B=2m regression")
    ...
    combined_profile = {
        ...
        "chunk_build_seconds": [r["index_profile"]["build_seconds"] for r in sorted(reports,key=lambda x:x["metadata"]["chunk_index"])],
        "chunk_kernel_seconds": [r["kernel_profile"]["kernel_seconds"] for r in sorted(reports,key=lambda x:x["metadata"]["chunk_index"])],
    }
    return {
        "metadata": {
            "stage": "14-num3",
        ...
        "num1_regression": {
            ...
            "all_four_sha256_match": four_hashes_match,
...
def main():
    ...
    if args.chunk_index is not None:
        ...
        paths = sorted(args.merge_dir.rglob("chunk-*.json"))
        report = merge_reports(args.bound, paths)
    else:
