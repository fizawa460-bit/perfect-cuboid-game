...
def canonical_gaussian_rep(ell: int):
    # For ell == 1 mod 4 prime, choose x>y>0.
...
def main():
    ...
    for ell in range(5, LIMIT_ELL + 1):
        ...
        # Synthetic primitive covers with the exact t65 separation ell > 2 N(V).
        ...
        for p in range(1, lim + 1):
            for q in range(p + 1, lim + 1):
                ...
                for sigma in (1, -1):
                    ...
                    # Exact inverse transform from the switched coordinates.
                    pp_num = x * T + y * D
                    ...
                    ratio = n / math.sqrt(N / 2)
                    max_short_cofactor_ratio = max(max_short_cofactor_ratio, ratio)

                    # t83 D=d*j switch: every exact divisor satisfies the
                    # elementary quarter-scale min(d,|j|)<=sqrt(|D|).
                    for d in divisors(abs(D)):
                        ...
                        qr = min(d, abs(j)) / math.sqrt(abs(D))
                        max_quarter_ratio = max(max_quarter_ratio, qr)
