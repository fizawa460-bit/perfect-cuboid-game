#!/usr/bin/env python3
"""Stage14-s7-37 deterministic audit.

New exact arithmetic on the proportional common-z branch:

  T0 = oddpart(gcd(z1,z2)) = Kx*Ky*HS*HT,

with the four root-gcd cells pairwise coprime.  The same-side part
K=Kx*Ky is coprime to q_xi, while T0^2|q_k=C*u_res and C|q_xi.
Consequently K^2|u_res.

On L_-=0 one has T0=B^(1/8+o(1)).  If K=B^kappa and H=HS*HT=B^eta,
then kappa+eta=1/8.  Since u_res<=B^(2theta-2phi+o(1)), K^2|u_res
gives eta>=1/8-theta+phi.  The merged fourth-power xi count therefore
satisfies E_H<=3theta-1/2, so the proportional branch is <=7/16.
The already-merged nonproportional 19/34 branch becomes global.
"""
...
# Finite physical audit of the four root-gcd cells and the new K^2|u_res theorem.
...
for states in groups.values():
    for i in range(len(states)):
        for j in range(i + 1, len(states)):
            ...
            # Same-side primes cannot divide q_xi.  For p|Kx both x-roots
            # carry p, so in the S-host the opposite coordinate
            # J*y1^2*omega2 is a p-unit; p is also outside S.  Ky is the
            # symmetric statement using R*x2^2*omega1.  The theorem proof is
            # recorded in result.md; this finite check freezes the interface.
            assert gcd(K, q_xi) == 1
            assert gcd(K, C) == 1

            # Merged 4ci gives T0^2|q_k=C*u_res.  Since K|T0 and (K,C)=1,
            # the full same-side square must lie in the reduced k residual.
            assert u_res % (K * K) == 0
...
# Exhaustive small-root algebra for the exact four-cell product.
root_checks = 0
    ...
    for y1 in range(1, 13):
        ...
        for x2 in range(1, 13):
            for y2 in range(1, 13):
                ...
                assert lhs == Kx * Ky * HS * HT
                root_checks += 1


# Exact exponent ledger.
# Nonproportional bound is imported from merged s7-36.
# On the proportional branch T0 has exponent tau=1/8.  Write
#   kappa=log_B K, eta=log_B H, kappa+eta=1/8.
# K^2|u_res and u_res<=B^(mu), mu=2theta-2phi, imply
#   eta >= 1/8 - mu/2 = 1/8-theta+phi.
# The fourth-power xi complete count is
#   E_H<=3phi-1/8-3eta <= 3theta-1/2.
D = 1632  # divisible by 16, 34, 48 and 68.
...
# Equality bookkeeping for the proportional bound.
theta = F(5, 16)
...
print("Stage14-s7-37 same-side residual transfer audit: PASS")
...
print("whole-family exponent:", F(19, 34))
print("saving over 9/16:", F(1, 272))
