...
def check_lattice_index_and_shortest_barrier():
    ...
    for col in COLUMNS:
        for u in U_SAMPLES:
            ...
            assert linear_residues == u, (col, u, linear_residues)
            ...
            for v in V_SAMPLES:
                ...
                for r in roots_minus_one(v):
                    ...
                    assert root_residues == v, (v, r, root_residues)

                    # CRT: the two independent congruence indices multiply.
                    assert linear_residues * root_residues == u * v
                    index_checks += 1
    ...
    for col in COLUMNS:
        for U in U_BLOCKS:
            ...
            for V in V_BLOCKS:
                ...
                for u in us:
                    for v in vs:
                        ...
                        for r in roots_minus_one(v):
                            ...
                            local_K = max(sqrt(v), min(u, v))
                            envelope_r = 1.0 + P / local_K
                            signed_max_ratio = max(
                                signed_max_ratio,
                                delta_r / envelope_r,
                            )
                            signed_checks += 1
                ...
                unsplit_l2_max_ratio = max(
                    unsplit_l2_max_ratio,
                    l2 / envelope,
                )
                blocks += 1

    # Regression ceilings only; the finite box has strong boundary effects.
