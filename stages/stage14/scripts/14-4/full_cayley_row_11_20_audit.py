...
def check_nested_row_column_crt():
    ...
    assert 2*((Lp+Lm)//2)==Lp+Lm
    assert 2*((Lp-Lm)//2)==Lp-Lm

    M=123456789
    N0=crt2(M%Cm,Cm,(-M)%Cp,Cp)
    assert (N0-M)%Cm==0 and (N0+M)%Cp==0
...
def check_equality_profile():
    ...
    assert chi==F(3,10) and d==F(1,20)
    assert rho==F(1,20)
    assert j==F(1,5) and cy==F(1,4)
    assert col==F(1,20) and row==0
...
def check_predecessors():
    ...
    s37=(ROOT/'stages/stage14/14-s7-37/result.md').read_text()
    ...
    assert 'PROPORTIONAL_BRANCH_UPPER_BOUND_EXPONENT=7/16' in s37
