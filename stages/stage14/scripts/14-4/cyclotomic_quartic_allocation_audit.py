...
unique three-way cyclotomic allocation, 4x4 good-prime allocation matrix, and
absence of the proportional branch. The asymptotic proportional elimination is
proved by the endpoint exponent gap, not inferred from this finite scan.
"""
...
def allocation_matrix(left, right, k_left: int, k_right: int):
    ...
    all_left = left[0] * left[1] * left[2] * left[3]
    good_primes = {p for p in factorint(all_left) if exceptional % p != 0}
...
def audit_pair(a: dict[str, int], b: dict[str, int]):
    ...
    # Physical cross-role labels are coprime.
    assert gcd(R * J, alpha * delta) == 1

    # The proportional branch U/A=V/D is absent in every finite physical pair.
    assert U * D != V * A

    # Exact switched-product identities and reciprocal divisibilities.
    ...
    # Moving gcds are supported on fixed root/small data.
    gU = gcd(U, V)
    gA = gcd(A, D)
    assert (X * Y) % gU == 0
    assert (r * s) % gA == 0
    ...
    allocation_matrix(left, right, k_left, k_right)

    assert G * qk * r * s * ck.F(U, V) == 2 * qxi * X * Y * ck.F(A, D)
    return triple, xi_branches, k_branches
...
def main() -> None:
    ...
    # Endpoint exponent ledgers.
    proportional_cell_min = Fraction(3, 8)
    fixed_small_max = Fraction(1, 8)
    proportional_gap = proportional_cell_min - fixed_small_max
    assert proportional_gap == Fraction(1, 4)
    ...
    print("finite physical proportional branch count: 0")
    print("proportional endpoint exponent gap: 1/4")
    ...
    print("off-diagonal asymptotic reciprocal cyclotomic incidence: UNPROVED")
