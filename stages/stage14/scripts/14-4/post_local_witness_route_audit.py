...
def main() -> None:
    ...
    assert Fraction(1, 1) - local_exp == Fraction(1, 42)
    ...
    flags = summary["flags"]
    assert flags["S5U_LOCAL_METHOD_CLOSURE_IMPORTED"] is True
    assert flags["CURRENT_LOCAL_CLASS_B_EXPONENT"] == "41/42"
    assert flags["CURRENT_SQRT_REMAINING_POST_LOCAL_DELTA"] == "10/21"
    ...
    assert flags["SEPARATED_GLOBAL_HEIGHT_ROUTE_REJECTED_AS_PRIMARY"] is True
