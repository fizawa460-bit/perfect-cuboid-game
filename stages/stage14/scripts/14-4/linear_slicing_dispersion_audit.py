...
def main() -> None:
    ...
    # Primitive/opposite-parity edge-coordinate lock.
    primitive_checks = 0
    for m in range(1, 80):
        for n in range(1, m):
            if (m - n) % 2 == 0:
    ...
    # Remaining linear roots are distinct from the two endpoint roots.
    ...
    # Refined norm roots are also distinct from every linear endpoint root.
    norm_root_checks = 0
    for p in PRIMES:
        r = sqrt_minus_one(p)
        if p % 4 == 3:
    ...
    # Exponent ledger: discrepancy uses L^(2-kappa), bulk uses
    # L^(2-kappa/2), so the full interior mode is bulk-dominated.
    exponent_rows = []
