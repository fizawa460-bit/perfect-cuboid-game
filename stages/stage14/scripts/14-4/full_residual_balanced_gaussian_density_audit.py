...
def audit_balanced_linear_identities():
    checks = 0
    # Synthetic exact checks of the identities used after the endpoint-small peel.
    # g=1 is enough to verify the algebra; the theorem treats general g=B^o(1).
...
def audit_primitive_rootline_spacing():
    # Finite regression for the determinant divisibility behind the root-line lemma.
    checks = 0
        ...
        for rho in roots_minus_one(q):
            ...
            for i in range(min(len(pts), 80)):
                ...
                for j in range(i + 1, min(len(pts), 80)):
                    ...
                    assert det % q == 0
                    if det == 0:
                        # Primitive positive vectors with equal slope coincide.
                        assert (P1, Q1) == (P2, Q2)
                    checks += 1
    assert checks > 1000
    return checks
