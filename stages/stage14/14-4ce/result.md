...
4ce proves the exact dual theorem for the product-kernel allocation `xi=a*b`, then combines both sides.

No 14-4 H line is created and no unproved t/tH selector theorem is imported.
...
Take two states with the same `xi,k,a,b`.  Cross multiplication of (1.1) gives
...
```text
a^2 | y1^4*h2^2-y2^4*h1^2,
b^2 | x1^4*h2^2-x2^4*h1^2.
```

The absolute values are at most
...
forces both brackets to vanish.  Positivity then gives `x1/y1=x2/y2`; primitivity gives `(x1,y1)=(x2,y2)` and therefore the physical coordinates agree.
...
On the old critical shell `xi~B^(3/4)`, `k>=B^(3/4-o(1))`, `X<=B^(1/2+o(1))`, the exponent margin in (2.1) is `1/4`.
...
Therefore `A^2` divides the first bracket and `D^2` the second.  Their sizes are at most
...
If `(A*D)^2*k>2X^4`, both brackets vanish.  A prime divisor of `B` or `C` then contradicts `gcd(xi,k*h)=1`, so `B=C=1`; the splits coincide, and Section 2 forces equality.

Hence every off-diagonal same-`(xi,k)` collision satisfies

```text
boxed:
(Xi_agree)^2*k <= 2X^4.                              (3.1)
```
...
```text
boxed:
Xi_switch >= xi*sqrt(k)/(sqrt(2)*X^2).               (3.2)
```
...
Merged s7-18 proves on the same pair space

```text
K_switch >= k*sqrt(xi)/(sqrt(32)*X^2).
```

Therefore every 4cd-endpoint off-diagonal collision satisfies simultaneously
...
Thus every odd `3 mod 4` prime is allocation-frozen throughout a same-`(xi,k)` collision fiber.  All nontrivial switch support lies on primes split in `Q(i)`.

The same argument preserves the statewise coupled residue graph:
...
This is exact.  It is not interpreted as independent local half-density.

## 6. Why 7/8 is not yet improved

For fixed squarefree `xi` or `k`, the number of coprime divisor allocations is `2^omega(.)=B^o(1)`.  A large switching divisor therefore does not itself give a fixed-power loss.

Likewise support on primes `1 mod 4` supplies only logarithmic sparsity at this level.  We do not convert it into a power of `B`.

Hence the current unconditional whole-family bound remains
...
The next genuinely transverse receiver is

```text
CoupledDoubleSwitchQuadraticResidueIncidence
```

on the same physical pair, retaining

```text
Xi_agree, Xi_switch,
K_agree, K_switch,
xi,k,
branch/orientation/reconstruction masks.
```

Forbidden shortcuts:

- multiplying local quadratic-residue densities;
- claiming fixed-power sparsity from `1 mod 4` support;
- collapsing to raw `(xi,k)` before the two switch products are used;
- importing tH15/t55 selector energy into the 14-4 mainline without an exact bridge.
...
The t-route tH15/t55 work is independent and is not a prerequisite for 14-4ce or 14-4cf.
