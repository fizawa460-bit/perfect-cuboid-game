...
- `CommonPhysicalCenteredPrimePairKernel` is the maximal licensed common core;
- the core preserves exact labels, masks, signed aggregation, shared `U/V`, the
  divisor hyperbola, the single state-diagonal subtraction, and operation order;
- positive `(xi,k)` collision, signed Gaussian dispersion, fixed-U bipartite
...
- toolbox main remains nonblocking and owns no new Stage14 arithmetic theorem.
...
SIGNED_COMMON_REFINEMENT_REQUIRED=true
STATE_DIAGONAL_SUBTRACTED_EXACTLY_ONCE=true
RESIDUE_DIAGONAL_DISTINCT_FROM_PRINCIPAL_COHERENCE=true
SHARED_UV_AND_DIVISOR_HYPERBOLA_REQUIRED=true
COMPLETION_BEFORE_PAIR_COLLAPSE=true
...
NEW_WHOLE_FAMILY_POWER_SAVING_OWNED_BY_TOOLBOX_AQ=false
TOOLBOX_H0_CONSUMED=true
TOOLBOX_H_CONTINUATION_NEEDED=false
TOOLBOX_MAIN_BLOCKED_BY_H=false
TOOLBOX_OWNS_NEW_STAGE14_THEOREM=false
NEXT=Stage14-toolbox-ar receiver-ready import certificate and stale-contract guard
```
