...
def main():
    ...
    base=load(BASE,BASE_BLOB,"base_quick"); fast=load(FAST,FAST_BLOB,"fast_quick")
    cells=fast.load_h21_cells(a.h21_dir,base); ctx=base.source_context(); bc=ctx[0]; b0,b1=base.PLANNED[0]
    hits=list(a.old_dir.rglob("row-003.json")); req(len(hits)==1,"old row 3"); old=json.loads(hits[0].read_text())
    ...
    t=time.perf_counter(); new=fast.compute_row_cell_fast(ctx,pref,0,3,cells); fast_s=time.perf_counter()-t
    req(new==old,"exact dictionary mismatch")
    out={"schema":"HPADJ22_FAST_QUICK_BAND0_V1","status":"EXACT_EQUIVALENCE_PASS","row_id":new["row"]["row_id"],"old_seconds":OLD_SECONDS,"joint_seconds":joint_s,"prefix_seconds":pref_s,"fast_seconds":fast_s,"row_speedup":OLD_SECONDS/fast_s,"canonical":new["canonical_sha256_without_this_field"]}
