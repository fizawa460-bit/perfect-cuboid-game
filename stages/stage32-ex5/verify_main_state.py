...
def main():
    ...
    req(s["schema"]=="STAGE32EX5_MAIN_COMPACT_STATE_V1_EX5_07_EHS_BLOCKED","schema drift")
    ...
    req(r["primary_route_blocker_code"]=="XSTAGE_MISSING_EXACT_NONEMPTY_MB_SUBSET_ADAPTER","XSTAGE blocker drift")
    req(r["backup_1_route_id"]=="EX5R-EFC-001" and r["backup_1_route_status"]=="BLOCKED","EFC status drift")
    req(r["backup_1_blocker_code"]=="EFC_MISSING_SOURCE_BOUND_POPULATION_COMPLETE_FIXED_COMPONENT_THEOREM","EFC blocker drift")
    req(r["backup_2_route_id"]=="EX5R-EHS-001" and r["backup_2_route_status"]=="BLOCKED","EHS status drift")
    req(r["backup_2_blocker_code"]=="EHS_MISSING_SOURCE_BOUND_EXACT_LINEAR_SYSTEM_DIMENSION_ADAPTER","EHS blocker drift")
    req(r["backup_2_preflight_artifact"]=="stages/stage32-ex5/ex5-07-ehs-preflight-01.json","EHS artifact drift")
    req(r["backup_2_preflight_verifier"]=="stages/stage32-ex5/verify_ex5_07_ehs_preflight_01.py","EHS verifier drift")
    req(r["reserve_route_ids"]==["EX5R-LGS-001","EX5R-MOD-001"],"reserve route drift")
    req(r["next_selected_route_id"] is None and r["reserve_review_required"] is True,"reserve review routing drift")
    ...
    cur=s["current"]
    req(cur["status"]=="EX5_07_EHS_PREFLIGHT_BLOCKED_RETAINED","current status drift")
    req(cur["leaf"]=="EX5-10_RESIDUAL_PACKAGE_STATUS_OR_RESERVE_ROUTE_REVIEW","leaf drift")
    req(cur["subroute"]=="REVIEW_LGS_AND_MOD_RESERVES_AFTER_XSTAGE_EFC_EHS_TYPED_BLOCKERS","subroute drift")
    req(cur["next_route_on_block"]=="EX5-11_BOUNDED_BREADTH_EXHAUSTION_CLASSIFICATION","reserve block routing drift")
    req(s["current_leaf_working_set"]==["stages/stage32-ex5/ex5-07-ehs-preflight-01.json","stages/stage32-ex5/verify_ex5_07_ehs_preflight_01.py","stages/stage32-ex5/ex5-05-route-scorecard.json","stages/stage32-ex5/ex5-04-repository-asset-dedup.json","stages/stage32-ex5/ex5-02-current-coverage-dependency-graph.json"],"working set drift")
    req(s["credit"]["level"]=="EX5_07_PRIMARY_AND_BACKUP_PREFLIGHTS_TYPED_BLOCKERS_NO_RECEIVER_EFFECT","credit level drift")
    ...
    print("PASS: Stage32EX5 state through XSTAGE+EFC+EHS typed blockers; reserve review next")
