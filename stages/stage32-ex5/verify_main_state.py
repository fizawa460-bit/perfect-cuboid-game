...
def main() -> None:
    ...
    req(state["schema"] == "STAGE32EX5_MAIN_COMPACT_STATE_V2_BC2_RETAINED_PROVISIONAL", "schema drift")
    ...
    req(bootstrap["merge_authorized"] is False, "merge authorization must remain false before re-audit")

    auth = state["authority"]
    req(auth["current_bc2_authority"] == "PROVISIONAL", "BC2 authority drift")
    ...
    req(current["status"] == "BC2_RETAINED_PROVISIONAL_REAUDIT_REQUIRED", "current status drift")
    ...
    req(sync["current_provisional_claim_id"] == BC2_CLAIM_ID, "claim-sync current claim drift")
    ...
    credit = state["credit"]
    req(credit["level"] == "BC2_PROVISIONAL_INFRASTRUCTURE_NO_CREDIT", "BC2 credit ceiling drift")
    ...
    audit = state["audit"]
    req(audit["status"] == "FAIL_REPAIR_APPLIED_REAUDIT_REQUIRED", "audit repair status drift")
    req(audit["candidate_pr"] == 1726, "audit PR drift")
    req(audit["failed_exact_head"] == FAILED_BC2_HEAD and audit["failed_review_id"] == FAILED_BC2_REVIEW, "failed audit identity drift")
    req(audit["failed_result"] == "FAIL" and audit["reaudit_required"] is True, "re-audit gate lost")
    req(audit["mathematical_blocking_contradiction_found"] is False, "audit failure misclassified as mathematical contradiction")
    ...
    # Preserve the exact prior Cycle1 hostile-audit evidence; BC2 routing must not rewrite it.
    ...
    # Retained BC2 checkpoint must agree with current routing and remain fail-closed.
    ...
    req(handoff["current_routing"]["provisional_claim_id"] == BC2_CLAIM_ID, "handoff claim id drift")
    ...
    print("PASS: Stage32EX5 BC2 retained routing/claim-sync projection repaired; Cycle1 AUDITED authority preserved")
    print(f"current_bc2_claim={BC2_CLAIM_ID}:PROVISIONAL")
    print(f"next_leaf={NEXT_LEAF}")
    print("bc2_credit=NO_CREDIT")
    print("stage32_main_credit=NO")
    print("hostile_reaudit=REQUIRED")
