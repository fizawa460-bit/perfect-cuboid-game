...
BC2_27_FAILED_AUDIT_HEAD = "43fce7733be95b47b2d2a4e4568300add7694224"
BC2_27_FAILED_AUDIT_REVIEW = 5179261378
PERFECT_CUBOID_FIREWALL_KEYS = {
...
def main() -> None:
    ...
    req(a["full178_goal_claim_id_observed"] == FULL178_GOAL_CLAIM, "Stage32 FULL178 goal projection drift")
    req(git_blob(STAGE32_MAIN) == STAGE32_MAIN_BLOB, "Stage32 MAIN authority file blob drift")
    main_authority = json.loads(STAGE32_MAIN.read_text())
    req(main_authority["current_exact_frontier"]["full178_goal_claim_id"] == FULL178_GOAL_CLAIM, "authoritative FULL178 goal claim-id drift")
    ...
    claim_sync = s["claim_sync"]
    req(claim_sync["existing_active_goal"] == FULL178_GOAL_CLAIM, "EX5 claim-sync active goal mismatch")
    req(claim_sync["lane_role"] == "ATTACKS" and claim_sync["active_frontier_remapped"] is False and claim_sync["main_promotion"] is False, "claim-sync promotion leak")
    ...
    req(audit["last_hostile_audit_status"] == "FAIL", "BC2-27 failed-audit receipt missing")
    req(audit["last_hostile_audit_exact_head"] == BC2_27_FAILED_AUDIT_HEAD and audit["last_hostile_audit_review_id"] == BC2_27_FAILED_AUDIT_REVIEW, "BC2-27 failed-audit exact-head/review drift")
    req(audit["repair_required_before_merge"] is True and audit["re_audit_required"] is True and audit["freeze_active"] is True, "BC2-27 repaired boundary not frozen for re-audit")
    ...
    req(fw["perfect_cuboid_existence_claim"] is False, "Perfect Cuboid existence firewall leak")
    req(fw["perfect_cuboid_nonexistence_claim"] is False, "Perfect Cuboid nonexistence firewall leak")
    ...
    print("PASS: Stage32EX5 BC2-27 retained hostile-audit repair boundary is coherent")
    print("authority_claim=S32.FULL178.NUMERICAL_CENSUS.V1;perfect_cuboid_firewalls=EXACT_FALSE")
