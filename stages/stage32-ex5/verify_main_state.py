...
SRC37_BLOB='35a1644c197fd32aec845332452cc918f3bc75d9'; PF37_BLOB='15a008cd4557fe4e81d31591c3d80b50bc595a03'; PF37_CANON='30fa122941ee6b75e6e5004aa43ab121b216af93831cff0e9e22922f2438ddba'
...
def main():
    ...
    req(blob(src)==SRC37_BLOB and blob(pf)==PF37_BLOB,'BC2-37 static identity'); p=checked(pf,PF37_CANON); k=json.loads(rk.read_text())
    ...
    kt=k['target']; req(kt['fresh_unknown_parent_count']==41 and kt['fresh_unknown_parent_indices_sha256']==UNKNOWN_SHA and kt['prior_audited_unsat_count']==7295 and kt['targeted_replay_only'] is True,'runkey target')
    req(k['schema']=='STAGE32EX5_BC2_37_FRESH_UNKNOWN41_REPLAY_RUNKEY_V1' and k['generation'] in (0,1) and ((k['generation']==0 and k['armed'] is False) or (k['generation']==1 and k['armed'] is True)) and k['consumed_run'] is None,'runkey cold/armed semantics')
