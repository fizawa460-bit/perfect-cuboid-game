...
CURRENT_LEAF = "BC2_13_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT"
CURRENT_MAIN = "5ca6acba4b591d9e2d40057241c850598c1fa1df"
STAGE32_MAIN_HEAD = "58a25f59c3f82738c4666c76233568a038d151df"
...
def verify_current_docs() -> None:
    ...
    for name, text in docs.items():
        ...
        req("0..531" in text, f"{name} lost BC2-12 exact prefix")
    ...
    req(CURRENT_LEAF in docs["README"] and CURRENT_LEAF in docs["ROADMAP"], "BC2-13 current next step missing")
...
def main() -> None:
    ...
    req(current["status"] == "BC2_12_EXACT_UNSAT_PASS_BC2_13_ACTIVE", "current status drift")
    ...
    req(frontier["next_structural_block_exactly_rederived"] is False, "future structural block prematurely credited")
    ...
    req(canonical_without_field(bc2_11) == BC2_11_CANONICAL, "BC2-11 canonical replay drift")
    ...
    req(checkpoint["next_exact_unit"]["id"] == CURRENT_LEAF, "BC2-12 next-unit drift")
    ...
    req(canonical_without_field(cert) == CYCLE1_CERT, "Cycle1 certificate canonical replay drift")

    verify_current_docs()
    print("PASS: Stage32EX5 BC2-12 exact UNSAT retained; BC2-13 routed")
    ...
    print("bc2_12_parent_partition=20/20:UNSAT")
