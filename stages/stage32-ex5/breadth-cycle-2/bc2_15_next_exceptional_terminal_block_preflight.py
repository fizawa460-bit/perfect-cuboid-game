...
SCHEMA = "STAGE32EX5_BC2_15_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT_V1"
EXPECTED_BC2_14_CHECKPOINT_CANONICAL = "fa603024cbf3b89fb097401f4e5e9b3f0cad04a0a0b6dd0b919b3a37b2781c0e"
ROW_ID = "g1-d008"
DEGREE = 8
EXCEPTIONAL_MASS = 4
NORMAL_BUDGET = 132
...
def main() -> None:
    ...
    ap.add_argument("--bc2-14-checkpoint", type=Path, required=True)
    ap.add_argument("--output", type=Path, required=True)
    args = ap.parse_args()

    cp = load_json(args.bc2_14_checkpoint)
    verify_canonical(cp, EXPECTED_BC2_14_CHECKPOINT_CANONICAL, "BC2-14 checkpoint")
    if cp["exact_result"]["rank_532_to_664_block_exact_unsat_authorized"] is not True:
        raise ValueError("BC2-14 whole-block UNSAT authority regression")
    if cp["next_exact_unit"]["id"] != "BC2_15_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT":
        raise ValueError("BC2-14 next-unit routing regression")
    for key in (
        "whole_rank_0_to_132_block_closed",
        "whole_rank_133_to_265_block_closed",
        "whole_rank_266_to_398_block_closed",
        "whole_rank_399_to_531_block_closed",
        "whole_rank_532_to_664_block_closed",
    ):
        if cp["scope"][key] is not True:
            raise ValueError(f"predecessor closure regression: {key}")
    if cp["scope"]["local_exact_unsat_prefix"] != [0, 664]:
        raise ValueError("BC2-14 local exact UNSAT prefix regression")

    previous_block = [int(q) for q in cp["proof_partition"]["terminal_rank_block"]]
    if previous_block != [532, 664]:
        raise ValueError("BC2-14 retained block boundary regression")
    previous_outer_rank = int(cp["proof_partition"]["outer_exceptional_rank"])
    if previous_outer_rank != 4:
        raise ValueError("BC2-14 outer exceptional rank regression")
    ...
    payload = {
        ...
        "stage": "32EX5",
        "leaf": "BC2-15",
        "unit": "BC2_15_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT",
        "status": "PASS_NEXT_EXCEPTIONAL_BLOCK_EXACTLY_REDERIVED_NO_SOLVER_CREDIT",
        "source_locks": {
            "bc2_14_checkpoint_canonical_sha256": EXPECTED_BC2_14_CHECKPOINT_CANONICAL,
        ...
        "next_exact_unit": {
            "id": f"BC2_16_OUTER_RANK{outer_rank_start}_SYMBOLIC_X4_PARENT_PREFLIGHT",
    ...
    print(json.dumps({
        ...
        "next_exact_unit": payload["next_exact_unit"]["id"],
    }, sort_keys=True))
