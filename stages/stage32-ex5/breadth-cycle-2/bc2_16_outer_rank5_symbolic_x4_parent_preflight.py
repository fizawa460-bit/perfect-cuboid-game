...
SCHEMA = "STAGE32EX5_BC2_16_OUTER_RANK5_SYMBOLIC_X4_PARENT_PREFLIGHT_V1"
NORMAL_COUNT = 92
EXCEPTIONAL_COUNT = 48
BLOCK_EXCEPTIONAL_RANK = 5
BLOCK_START = 665
BLOCK_END = 797
...
EXPECTED_BC2_15_CANONICAL = "f88ebe53c4300c01fc0a4870cc0dca75a0a091b60c95c8da7e00926bcee43182"
...
EXPECTED_BASE_TERMINAL = (0, 1, 0, 0, 0, 0, 2, 0, 0, 0, 1)
EXPECTED_EXCEPTIONAL_SIGNATURE = (0, 1, 0, 0, 0, 2, 0, 0, 0, 1)
EXPECTED_FIXED_EXCEPTIONAL_MASS = 4
EXPECTED_RESIDUAL_EXCEPTIONAL_MASS = 0
...
def main() -> None:
    ...
    ap.add_argument("--bc2-15-checkpoint", type=Path, required=True)
    ap.add_argument("--retained", type=Path, required=True)
    ap.add_argument("--marking", type=Path, required=True)
    ap.add_argument("--parent-timeout-ms", type=int, default=5000)
    ...
    bc2_15, bc2_15_canonical = v1.load_canonical_json(args.bc2_15_checkpoint)
    for got, expected, name in (
        ...
        (bc2_15_canonical, EXPECTED_BC2_15_CANONICAL, "BC2-15 checkpoint"),
    ):
        if got != expected:
            raise ValueError(f"{name} canonical regression")

    if bc2_15.get("next_exact_unit", {}).get("id") != "BC2_16_OUTER_RANK5_SYMBOLIC_X4_PARENT_PREFLIGHT":
        raise ValueError("BC2-15 next-unit routing regression")
    f15 = bc2_15.get("firewalls", {})
    for key in (
        ...
        "rank_399_to_531_block_exact_unsat_retained",
        "rank_532_to_664_block_exact_unsat_retained",
    ):
        if f15.get(key) is not True:
            raise ValueError(f"BC2-15 predecessor authority regression: {key}")
    if f15.get("next_block_picard64_closed") is not False:
        raise ValueError("BC2-15 next-block credit firewall regression")

    replay = bc2_15["next_block"]
    if replay.get("terminal_rank_start") != BLOCK_START or replay.get("terminal_rank_end") != BLOCK_END:
        raise ValueError("BC2-15 rank-block regression")
    if replay.get("outer_exceptional_rank") != BLOCK_EXCEPTIONAL_RANK:
        raise ValueError("BC2-15 exceptional outer-rank regression")
    if tuple(replay.get("base_terminal_x4_zero", [])) != EXPECTED_BASE_TERMINAL:
        raise ValueError("BC2-15 base-terminal regression")
    if tuple(replay.get("exceptional_signature", [])) != EXPECTED_EXCEPTIONAL_SIGNATURE:
        raise ValueError("BC2-15 exceptional-signature regression")
    for key in (
        ...
        if replay.get(key) is not True:
            raise ValueError(f"BC2-15 replay authority regression: {key}")
    ...
    if block_width != EXPECTED_BLOCK_WIDTH or BLOCK_EXCEPTIONAL_RANK * block_width != BLOCK_START:
        raise ValueError("outer-rank/block-width identity regression")
    ...
    if base_terminal != EXPECTED_BASE_TERMINAL:
        raise ValueError("outer-rank5 base terminal changed")
    if base_exceptional_signature != EXPECTED_EXCEPTIONAL_SIGNATURE:
        raise ValueError("outer-rank5 exceptional signature changed")
    ...
    bundle = v1.load_retained(args.retained, "s32ex5_bc2_16_outer_rank5_picard")
    marking = v1.load_retained(args.marking, "s32ex5_bc2_16_outer_rank5_marking")
    ...
    if (fixed_exceptional_mass, residual_exceptional_mass) != (
        EXPECTED_FIXED_EXCEPTIONAL_MASS,
        EXPECTED_RESIDUAL_EXCEPTIONAL_MASS,
    ):
        raise ValueError("outer-rank5 exceptional mass split regression")
    ...
    if len(parent_assignments) != expected_parent_count or expected_parent_count != EXPECTED_PARENT_COUNT:
        raise ValueError("outer-rank5 selected-exceptional parent partition regression")
    ...
    whole_block_unsat = aggregate == "UNSAT" and exact_unsat_count == len(parent_assignments)

    payload = {
        "schema": SCHEMA,
        "stage": "32EX5",
        "leaf": "BC2-16",
        "unit": "BC2_16_OUTER_RANK5_SYMBOLIC_X4_PARENT_PREFLIGHT",
        "status": (
            "PASS_OUTER_RANK5_BLOCK_EXACT_UNSAT"
            if whole_block_unsat
            else "PASS_OUTER_RANK5_BLOCK_HAS_EXACT_PICARD64_COMPLETION"
            if aggregate == "SAT"
            else "PASS_OUTER_RANK5_PARENT_PREFLIGHT_PARTIAL_UNKNOWN"
        ),
        "source_locks": {
            "bc2_15_checkpoint_canonical_sha256": EXPECTED_BC2_15_CANONICAL,
        ...
        "block_semantics": {
            ...
            "bc2_15_exact_replay_consumed": True,
        ...
        "adaptive_exceptional_partition": {
            ...
            "mass_split_derived_from_bc2_15_terminal_and_locked_labels": True,
        ...
        "result": {
            ...
            "rank_665_to_797_block_exact_unsat_authorized": whole_block_unsat,
        ...
        "next_exact_unit": {
            "id": (
                ...
                if whole_block_unsat
                else "BC2_16_SAT_NODE_SUPPORT_CONSUMPTION"
                if aggregate == "SAT"
                else "BC2_16_OUTER_RANK5_EXACT_UNKNOWN_PARENT_REFINEMENT"
            ),
        ...
        "firewalls": {
            ...
            "rank_266_to_398_block_exact_unsat_retained": True,
            "rank_399_to_531_block_exact_unsat_retained": True,
            "rank_532_to_664_block_exact_unsat_retained": True,
            "rank_665_to_797_block_picard64_closed": whole_block_unsat,
