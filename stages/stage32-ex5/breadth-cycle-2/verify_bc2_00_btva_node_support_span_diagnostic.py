...
def main():
    ...
    if schema.endswith('BC2_38_TARGETED_REPLAY_AUDIT_BOUNDARY'):
        ...
        req(a['last_hostile_audit_exact_head']=='9852fcec959962607da3290100291a60185e7104' and a['last_hostile_audit_review_id']==5189412496,'BC2-37 audit receipt drift'); req(a['freeze_active'] is False and a['new_audit_boundary_exists'] is False and a['re_audit_required'] is False and a['bc2_38_execution_authorized'] is True,'BC2-38 execution authority drift'); req(cur['next_route']=='BC2_38_REFINE_REMAINING_FRESH_UNKNOWN_SET','BC2-38 route drift'); req(f['e8_bc2_37_audited'] is True and f['e8_bc2_38_executed'] is False and f['e8_bc2_38_target_unknown_count']==34 and f['e8_known_parent_unsat_count_lower_bound']==7302,'BC2-38 frontier drift')
    ...
    spec=importlib.util.spec_from_file_location('bc2_00_v4_frozen',FROZEN); req(spec is not None and spec.loader is not None,'cannot load frozen verifier'); module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
    compat=json.loads(json.dumps(state)); compat['schema']='STAGE32EX5_MAIN_COMPACT_STATE_V4_FULL178_FINAL_CHAIN_SYNC'; prior=compat.setdefault('prior_audited_authority',{}); c=prior.setdefault('cycle1',{}); c.setdefault('breadth_cycle','EX5_BREADTH_CYCLE_1'); c.setdefault('authority_status','AUDITED'); c.setdefault('scope_firewall','EX5_BREADTH_CYCLE_1_ONLY'); e=prior.setdefault('early_bc2',{}); e.setdefault('claim_id','S32.EX5.BC2_NODE_SUPPORT_SPAN_CHECKPOINT.V1'); compat.setdefault('frontier',{}).setdefault('runtime_exceptional_index_to_projective_node_bridge_complete',True)
    class P:
