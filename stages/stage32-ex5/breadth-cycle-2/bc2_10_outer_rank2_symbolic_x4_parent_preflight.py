...
SCHEMA = "STAGE32EX5_BC2_10_OUTER_RANK2_SYMBOLIC_X4_PARENT_PREFLIGHT_V1"
NORMAL_COUNT = 92
EXCEPTIONAL_COUNT = 48
BLOCK_EXCEPTIONAL_RANK = 2
BLOCK_START = 266
BLOCK_END = 398
...
EXPECTED_BC2_09_CANONICAL = "db1cb79d564904192bf5cd6450d54decacf15ef3711d686e573e92f3b8f56cd7"
...
EXPECTED_BASE_TERMINAL = (0, 1, 0, 0, 0, 0, 0, 2, 0, 0, 1)
EXPECTED_EXCEPTIONAL_SIGNATURE = (0, 1, 0, 0, 0, 0, 2, 0, 0, 1)
...
def main() -> None:
    ...
    ap.add_argument("--bc2-09-checkpoint", type=Path, required=True)
    ...
    bc2_09, bc2_09_canonical = v1.load_canonical_json(args.bc2_09_checkpoint)
    ...
    for got, expected, name in (
        ...
        (bc2_09_canonical, EXPECTED_BC2_09_CANONICAL, "BC2-09 checkpoint"),
    ):
        if got != expected:
            raise ValueError(f"{name} canonical regression")

    replay = bc2_09["exact_replay"]
    if bc2_09.get("next_exact_unit", {}).get("id") != "BC2_10_OUTER_RANK2_SYMBOLIC_X4_PARENT_PREFLIGHT":
        raise ValueError("BC2-09 next-unit routing regression")
    if bc2_09.get("predecessor", {}).get("rank_0_to_132_block_exact_unsat") is not True:
        raise ValueError("BC2-09 rank0..132 predecessor authority regression")
    if bc2_09.get("predecessor", {}).get("rank_133_to_265_block_exact_unsat") is not True:
        raise ValueError("BC2-09 rank133..265 predecessor authority regression")
    if bc2_09.get("firewalls", {}).get("rank_266_to_398_block_picard64_closed") is not False:
        raise ValueError("BC2-09 next-block credit firewall regression")
    if replay.get("terminal_rank_start") != BLOCK_START or replay.get("terminal_rank_end") != BLOCK_END:
        raise ValueError("BC2-09 rank-block regression")
    if replay.get("outer_exceptional_rank") != BLOCK_EXCEPTIONAL_RANK:
        raise ValueError("BC2-09 exceptional outer-rank regression")
    if tuple(replay.get("base_terminal_x4_zero", [])) != EXPECTED_BASE_TERMINAL:
        raise ValueError("BC2-09 base-terminal regression")
    if tuple(replay.get("exceptional_signature", [])) != EXPECTED_EXCEPTIONAL_SIGNATURE:
        raise ValueError("BC2-09 exceptional-signature regression")
    for key in (
        ...
        if replay.get(key) is not True:
            raise ValueError(f"BC2-09 replay authority regression: {key}")
    ...
    if base_terminal != EXPECTED_BASE_TERMINAL:
        raise ValueError("outer-rank2 base terminal changed")
    if base_exceptional_signature != EXPECTED_EXCEPTIONAL_SIGNATURE:
        raise ValueError("outer-rank2 exceptional signature changed")
    ...
    bundle = v1.load_retained(args.retained, "s32ex5_bc2_10_outer_rank2_picard")
    marking = v1.load_retained(args.marking, "s32ex5_bc2_10_outer_rank2_marking")
    ...
    if fixed_exceptional_mass != 4 or residual_exceptional_mass != 0:
        raise ValueError("outer-rank2 exceptional mass split regression")
    ...
    if len(parent_assignments) != expected_parent_count or expected_parent_count != 1:
        raise ValueError("outer-rank2 selected-exceptional parent partition regression")
    ...
    whole_block_unsat = aggregate == "UNSAT" and exact_unsat_count == 1
    payload = {
        "schema": SCHEMA,
        "stage": "32EX5",
        "leaf": "BC2-10",
        "unit": "BC2_10_OUTER_RANK2_SYMBOLIC_X4_PARENT_PREFLIGHT",
        "status": (
            "PASS_OUTER_RANK2_BLOCK_EXACT_UNSAT"
            if whole_block_unsat
            else "PASS_OUTER_RANK2_BLOCK_HAS_EXACT_PICARD64_COMPLETION"
            if aggregate == "SAT"
            else "PASS_OUTER_RANK2_SINGLE_PARENT_REFINEMENT_REQUIRED"
        ),
        "source_locks": {
            "bc2_09_checkpoint_canonical_sha256": EXPECTED_BC2_09_CANONICAL,
        ...
        "block_semantics": {
            ...
            "bc2_09_exact_replay_consumed": True,
        ...
        "result": {
            ...
            "rank_266_to_398_block_exact_unsat_authorized": whole_block_unsat,
        ...
        "next_exact_unit": {
            "id": (
                ...
                if whole_block_unsat
                else "BC2_10_SAT_NODE_SUPPORT_CONSUMPTION"
                if aggregate == "SAT"
                else "BC2_11_OUTER_RANK2_SINGLE_SUBCASE_EXACT_SOLVER_REFINEMENT"
            ),
        ...
        "firewalls": {
            ...
            "rank_266_to_398_block_picard64_closed": whole_block_unsat,
