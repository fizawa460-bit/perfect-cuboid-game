...
def main() -> None:
    ...
    # Immutable source providers remain byte-locked.  Arsenal index/catalog
    # are generated mutable views, so retain their historical identities in
    # the evidence while validating the live views semantically below.
    ...
    assert p["basis"]["bounded_human_registry_blob"] == HISTORICAL_CATALOG_BLOB
    ...
    assert "Generated by sync_arsenal_catalog.py; edit index.json, not this file." in catalog_text
    assert "the sole machine-readable registry" in catalog_text
    for card_id in ("S30-W01", "S30-W02", "S32-PW01", "S34-W03", "S36-PW01"):
        assert f"`{card_id}`" in catalog_text
    ...
    print(json.dumps({
        ...
        "historical_arsenal_catalog_blob": HISTORICAL_CATALOG_BLOB,
        "live_arsenal_views_revalidated_semantically": True,
