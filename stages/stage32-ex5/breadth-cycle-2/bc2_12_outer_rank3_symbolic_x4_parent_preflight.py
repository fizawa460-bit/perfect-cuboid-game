...
EXPECTED_FIXED_EXCEPTIONAL_MASS = 3
EXPECTED_RESIDUAL_EXCEPTIONAL_MASS = 1
...
def main() -> None:
    ...
    for got, expected, name in (
        ...
        (bc2_11_canonical, EXPECTED_BC2_11_CANONICAL, "BC2-11 checkpoint"),
    ):
        if got != expected:
    ...
    if f11.get("next_block_picard64_closed") is not False:
        raise ValueError("BC2-11 next-block credit firewall regression")

    replay = bc2_11["next_block"]
    ...
    if replay.get("outer_exceptional_rank") != BLOCK_EXCEPTIONAL_RANK:
        raise ValueError("BC2-11 exceptional outer-rank regression")
    ...
    if base_terminal != EXPECTED_BASE_TERMINAL:
        raise ValueError("outer-rank3 base terminal changed")
    if base_exceptional_signature != EXPECTED_EXCEPTIONAL_SIGNATURE:
        raise ValueError("outer-rank3 exceptional signature changed")
    ...
    bundle = v1.load_retained(args.retained, "s32ex5_bc2_12_outer_rank3_picard")
    marking = v1.load_retained(args.marking, "s32ex5_bc2_12_outer_rank3_marking")
    ...
    # This split is derived from the exact BC2-11 base terminal and the locked label map.
    ...
    if (fixed_exceptional_mass, residual_exceptional_mass) != (
        ...
        raise ValueError("outer-rank3 exceptional mass split regression")
    ...
    if len(parent_assignments) != expected_parent_count or expected_parent_count != EXPECTED_PARENT_COUNT:
        raise ValueError("outer-rank3 selected-exceptional parent partition regression")
    ...
    payload = {
        ...
        "stage": "32EX5",
        "leaf": "BC2-12",
        "unit": "BC2_12_OUTER_RANK3_SYMBOLIC_X4_PARENT_PREFLIGHT",
        "status": (
            "PASS_OUTER_RANK3_BLOCK_EXACT_UNSAT"
            if whole_block_unsat
            else "PASS_OUTER_RANK3_BLOCK_HAS_EXACT_PICARD64_COMPLETION"
            if aggregate == "SAT"
            else "PASS_OUTER_RANK3_PARENT_PREFLIGHT_PARTIAL_UNKNOWN"
        ),
        "source_locks": {
            "bc2_11_checkpoint_canonical_sha256": EXPECTED_BC2_11_CANONICAL,
        ...
        "block_semantics": {
            ...
            "bc2_11_exact_replay_consumed": True,
        ...
        "adaptive_exceptional_partition": {
            ...
            "mass_split_derived_from_bc2_11_terminal_and_locked_labels": True,
        ...
        "result": {
            ...
            "rank_399_to_531_block_exact_unsat_authorized": whole_block_unsat,
        ...
        "next_exact_unit": {
            "id": (
                ...
                if whole_block_unsat
                else "BC2_12_SAT_NODE_SUPPORT_CONSUMPTION"
                if aggregate == "SAT"
                else "BC2_12_OUTER_RANK3_EXACT_UNKNOWN_PARENT_REFINEMENT"
            ),
        ...
        "firewalls": {
            ...
            "rank_399_to_531_block_picard64_closed": whole_block_unsat,
