...
SCHEMA = "STAGE32EX5_BC2_13_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT_V1"
EXPECTED_BC2_12_CHECKPOINT_CANONICAL = "2fdb506c601ece59ef107fa62fc4e8948b6ea5145dae413e63ccc2d11691ecd9"
ROW_ID = "g1-d008"
DEGREE = 8
EXCEPTIONAL_MASS = 4
NORMAL_BUDGET = 132
...
def main() -> None:
    ...
    ap.add_argument("--bc2-12-checkpoint", type=Path, required=True)
    ap.add_argument("--output", type=Path, required=True)
    args = ap.parse_args()

    cp = load_json(args.bc2_12_checkpoint)
    verify_canonical(cp, EXPECTED_BC2_12_CHECKPOINT_CANONICAL, "BC2-12 checkpoint")
    if cp["exact_result"]["rank_399_to_531_block_exact_unsat_authorized"] is not True:
        raise ValueError("BC2-12 whole-block UNSAT authority regression")
    if cp["next_exact_unit"]["id"] != "BC2_13_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT":
        raise ValueError("BC2-12 next-unit routing regression")
    for key in (
    ...
    if cp["scope"]["local_exact_unsat_prefix"] != [0, 531]:
        raise ValueError("BC2-12 local exact UNSAT prefix regression")

    previous_block = [int(q) for q in cp["proof_partition"]["terminal_rank_block"]]
    if previous_block != [399, 531]:
        raise ValueError("BC2-12 retained block boundary regression")
    previous_outer_rank = int(cp["proof_partition"]["outer_exceptional_rank"])
    if previous_outer_rank != 3:
        raise ValueError("BC2-12 outer exceptional rank regression")
    ...
    payload = {
        ...
        "stage": "32EX5",
        "leaf": "BC2-13",
        "unit": "BC2_13_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT",
        "status": "PASS_NEXT_EXCEPTIONAL_BLOCK_EXACTLY_REDERIVED_NO_SOLVER_CREDIT",
        "source_locks": {
            "bc2_12_checkpoint_canonical_sha256": EXPECTED_BC2_12_CHECKPOINT_CANONICAL,
        ...
        "next_exact_unit": {
            "id": f"BC2_14_OUTER_RANK{outer_rank_start}_SYMBOLIC_X4_PARENT_PREFLIGHT",
