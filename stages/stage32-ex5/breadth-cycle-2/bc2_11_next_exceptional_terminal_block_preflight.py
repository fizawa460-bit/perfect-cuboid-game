...
SCHEMA = "STAGE32EX5_BC2_11_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT_V1"
EXPECTED_BC2_10_CHECKPOINT_CANONICAL = "1abeb2bd5299ed217840d028eb99ed961d98238cdec75b5286b05b7f066a752b"
ROW_ID = "g1-d008"
DEGREE = 8
EXCEPTIONAL_MASS = 4
NORMAL_BUDGET = 132
...
def main() -> None:
    ...
    ap.add_argument("--bc2-10-checkpoint", type=Path, required=True)
    ap.add_argument("--output", type=Path, required=True)
    args = ap.parse_args()

    cp = load_json(args.bc2_10_checkpoint)
    verify_canonical(cp, EXPECTED_BC2_10_CHECKPOINT_CANONICAL, "BC2-10 checkpoint")
    if cp["exact_result"]["rank_266_to_398_block_exact_unsat_authorized"] is not True:
        raise ValueError("BC2-10 whole-block UNSAT authority regression")
    if cp["next_exact_unit"]["id"] != "BC2_11_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT":
        raise ValueError("BC2-10 next-unit routing regression")
    ...
    if cp["scope"]["whole_rank_266_to_398_block_closed"] is not True:
        raise ValueError("rank 266..398 predecessor closure regression")

    previous_block = [int(q) for q in cp["proof_partition"]["terminal_rank_block"]]
    if previous_block != [266, 398]:
        raise ValueError("BC2-10 retained block boundary regression")
    ...
    payload = {
        ...
        "stage": "32EX5",
        "leaf": "BC2-11",
        "unit": "BC2_11_NEXT_EXCEPTIONAL_TERMINAL_BLOCK_PREFLIGHT",
        "status": "PASS_NEXT_EXCEPTIONAL_BLOCK_EXACTLY_REDERIVED_NO_SOLVER_CREDIT",
        "source_locks": {
            "bc2_10_checkpoint_canonical_sha256": EXPECTED_BC2_10_CHECKPOINT_CANONICAL,
        ...
        "next_exact_unit": {
            "id": f"BC2_12_OUTER_RANK{outer_rank_start}_SYMBOLIC_X4_PARENT_PREFLIGHT",
