...
def main() -> None:
    ...
    for lock in data["source_locks"]:
        ...
        if lock["path"] == HISTORICAL_INDEX_PATH:
            ...
            continue
        if lock["path"] == HISTORICAL_CATALOG_PATH:
    ...
    require(CATALOG.is_file(), "live Arsenal catalog missing")
    ...
    require("Generated by sync_arsenal_catalog.py; edit index.json, not this file." in catalog_text,
            "Arsenal catalog generated-view marker drift")
    require("the sole machine-readable registry" in catalog_text,
            "Arsenal catalog registry-authority marker drift")
    ...
    for expected_entry in data["selected_index_entries"]:
        ...
        require(f"`{cid}`" in catalog_text, f"selected Arsenal ID missing from live catalog: {cid}")
        actual = by_id[cid]
