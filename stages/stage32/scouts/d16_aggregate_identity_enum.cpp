...
static constexpr int RANK = 63;
static constexpr int BASE_CONSTRAINTS = 140;
static constexpr int AGG_IDENTITY_ROW = 140;
static constexpr int TOTAL_CONSTRAINTS = 141;
...
static Problem load_problem(const std::string& path) {
    ...
    std::string magic;
    std::getline(f, magic);
    if (magic != "S32_D16_CONSTRAINED_HPERP_AGG_V1") throw std::runtime_error("bad magic");
    Problem p;
    std::getline(f, p.core_sha);
    std::getline(f, p.source_blob);
    std::getline(f, p.input_sha);
    f >> p.n >> p.m;
    if (p.n != RANK || p.m != TOTAL_CONSTRAINTS) throw std::runtime_error("unexpected dimensions");
    ...
    if (p.cap[AGG_IDENTITY_ROW] != 0) throw std::runtime_error("aggregate identity is not equality");
    return p;
}
...
struct TierResult {
    ...
    uint64_t aggregate_identity_prunes = 0;
...
public:
    explicit Enumerator(const Problem& p): p_(p), n_(p.n), m_(p.m) {
        ...
        // lin.z = a.t for t=L^T z. remdual is the exact ellipsoid-dual
        // radius formula evaluated in long double, with conservative guards.
        ...
        // Exact aggregate equality first, then tighter exceptional caps, then normals.
        order_.reserve(m_);
        order_.push_back(AGG_IDENTITY_ROW);
        for (int r=92;r<140;r++) order_.push_back(r);
...
private:
    ...
    std::vector<long double> d_, assigned_;
    ...
    bool constraints_possible(int last_remaining, long double budget) {
        ...
        for (int rr: order_) {
            ...
            if (dist > reach + guard) {
                if (rr == AGG_IDENTITY_ROW) result_.aggregate_identity_prunes++;
                return false;
            }
        }
        return true;
    ...
    void exact_leaf() {
        ...
        for (long long v: z_) {
            ...
            for (int b=0;b<8;b++) {
                result_.checksum ^= (x>>(8*b))&255ULL;
                result_.checksum *= 1099511628211ULL;
            }
        }
    }
};
...
int main(int argc, char** argv) {
    try {
        std::string input, output, bounds_s = "2,4,6,8,10,12,16,20,26,34";
        double per_bound = 90.0;
        uint64_t node_cap = 200000000ULL, survivor_cap = 5000000ULL;
        for (int i=1;i<argc;i++) {
            std::string arg = argv[i];
            auto need=[&](){ if (++i>=argc) throw std::runtime_error("missing argument"); return std::string(argv[i]); };
            if (arg=="--input") input=need();
            else if (arg=="--output") output=need();
            else if (arg=="--bounds") bounds_s=need();
            else if (arg=="--per-bound-seconds") per_bound=std::stod(need());
            else if (arg=="--node-cap") node_cap=std::stoull(need());
            else if (arg=="--survivor-cap") survivor_cap=std::stoull(need());
        ...
        for (int b: bounds) {
            ...
            std::cerr << "bound="<<b<<" status="<<r.status<<" nodes="<<r.nodes
                      <<" agg_prunes="<<r.aggregate_identity_prunes
        ...
        f << "{\n";
        f << "  \"schema\": \"STAGE32_SCOUT_D16_CONSTRAINED_HPERP_AGG_ID_V1\",\n";
        ...
        f << "  \"architecture\": {\"direction\": \"H_PERP_PLUS_140_CAPS_PLUS_POSITIVE_DUAL_IDENTITY_PLUS_NORM_ONE_TREE\", \"enumerator\": \"CUSTOM_LLL_LDL_FINCKE_POHST_WITH_ELLIPSOID_LINEAR_REACH_PRUNING\", \"floating_pruning_is_scout_only\": true, \"exact_integer_leaf_recheck\": true, \"materialized_branch_count_constructed\": 0},\n";
        f << "  \"tiers\": [\n";
        for (size_t k=0;k<tiers.size();k++) {
            ...
            f << "    {\"bound\": "<<r.bound
              <<", \"status\": \""<<r.status<<"\""
              <<", \"nodes\": "<<r.nodes
              <<", \"coordinate_trials\": "<<r.coordinate_trials
              <<", \"constraint_prunes\": "<<r.constraint_prunes
              <<", \"aggregate_identity_prunes\": "<<r.aggregate_identity_prunes
              <<", \"exact_leaves\": "<<r.leaves
              <<", \"exact_survivors_including_zero\": "<<r.exact_survivors
              <<", \"nonzero_survivors\": "<<r.nonzero_survivors
              <<", \"elapsed_seconds\": "<<std::fixed<<std::setprecision(6)<<r.seconds
              <<", \"survivor_checksum_fnv64\": \""<<std::hex<<r.checksum<<std::dec<<"\""
              <<", \"norm_histogram\": {";
            bool first=true;
            for (int n=0;n<35;n++) if (r.norm_hist[n]) {
                if (!first) f << ",";
                first=false;
                f << "\""<<n<<"\":"<<r.norm_hist[n];
            }
