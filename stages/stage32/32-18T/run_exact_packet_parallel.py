...
def main():
    ...
    a=ap.parse_args(); manifest=json.loads(a.manifest.read_text()); packet=manifest['packets'][a.packet_id]; residues=[int(x) for x in packet['residues']]
    ...
    workers=max(1,min(a.workers,len(residues)))
    done={}
    ...
    if walls:
        out={'schema':'STAGE32_18O_D16_B14_PACKET_PILOT_V1','status':'RESOURCE_WALL_NODE_CAP','bound':14,'packet_id':a.packet_id,'tier':packet['tier'],'residues':residues,'completed_residues':[x['residue'] for x in per],'wall_residues':walls,'per_residue_node_cap':a.node_cap,'manifest_sha256':manifest_sha,'parallel_residue_workers':workers,'D16_B14_NUMERICAL_CREDIT':False,'GLOBAL_B14_AGGREGATION_COMPLETE':False,'THEOREM_CREDIT':False,'RECEIVER_CREDIT':False}
    ...
    out={'schema':'STAGE32_18O_D16_B14_PACKET_PILOT_V1','status':'COMPLETE','bound':14,'packet_id':a.packet_id,'tier':packet['tier'],'residues':residues,'residue_count':len(residues),'manifest_sha256':manifest_sha,'hybrid_risk_sum':packet['hybrid_risk_sum'],'p50_probe_prefix_sum':packet['p50_probe_prefix_sum'],'p48_probe_prefix_sum':packet['p48_probe_prefix_sum'],'canonical_survivors_including_zero':len(all_records),'canonical_nonzero_survivors':len(all_records)-hist.get('0',0),'canonical_norm_histogram':hist,'canonical_dump_sha256':hashlib.sha256(dump.read_bytes()).hexdigest(),'packet_residue_partition_complete':True,'TRAVERSAL_COMPLETENESS_CERTIFICATE':True,'all_symmetry_branch_rejections_exact_rational_cauchy_schwarz':True,'execution_work_counters_are_sum_of_independent_residue_runs_with_repeated_presplit_work':True,'execution_work_totals':totals,'per_residue':per,'parallel_residue_workers':workers,**locks,'D16_B14_NUMERICAL_CREDIT':False,'GLOBAL_B14_AGGREGATION_COMPLETE':False,'FULL_D16_G0_ROW_COMPLETE':False,'THEOREM_CREDIT':False,'RECEIVER_CREDIT':False}
    (a.output/'packet-certificate.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(json.dumps({'packet_id':a.packet_id,'status':'COMPLETE','residues':residues,'canonical':len(all_records)},sort_keys=True))
