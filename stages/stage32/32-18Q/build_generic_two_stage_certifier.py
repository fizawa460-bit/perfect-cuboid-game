...
NEW_SCHEMA='STAGE32_18Q_D16_B14_EXACT_TWO_STAGE_SHARDED_TRAVERSAL_CERT_V1'
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--source',type=pathlib.Path,required=True); ap.add_argument('--output',type=pathlib.Path,required=True); ap.add_argument('--primary-id',type=int,required=True)
    a=ap.parse_args(); pid=a.primary_id
    if not 0<=pid<1024: raise RuntimeError('primary id outside 0..1023')
    text=a.source.read_text(); text=once(text,OLD_SCHEMA,NEW_SCHEMA,'schema')
    ...
    text=once(text,oldm,newm,'metadata'); a.output.write_text(text); print({'primary_id':pid,'schema':NEW_SCHEMA})
