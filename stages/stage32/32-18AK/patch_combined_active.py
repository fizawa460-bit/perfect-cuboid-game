...
fn=r'''    cpp_rational cached_sym_crossgram(int r,int q,int last_remaining){
    ...
    bool pairwise_symmetry_possible_cached(int last_remaining,const cpp_rational& budget) {
        ...
        for(int r=0;r<s_.k;r++){
            ...
            long double sev=(-center)/reach; if(sev>0.50L) cand.push_back({r,sev});
        }
        ...
        for(size_t x=0;x<cand.size();x++) for(size_t y=x+1;y<cand.size();y++){
            ...
            cpp_rational cr=exact_center(s_.c0[r],sym_a_[r],last_remaining),cq=exact_center(s_.c0[q],sym_a_[q],last_remaining);
            if(cr>=0 || cq>=0) continue;
            cpp_rational d1=-cr,d2=-cq,g11=sym_dual_[r][last_remaining],g22=sym_dual_[q][last_remaining],g12=cached_sym_crossgram(r,q,last_remaining);
            cpp_rational det=g11*g22-g12*g12; if(det<=0) continue;
            cpp_rational lam1=(d1*g22-d2*g12)/det,lam2=(d2*g11-d1*g12)/det; if(lam1<=0 || lam2<=0) continue;
            if(d1*lam1+d2*lam2>budget){ ++pairwise_symmetry_prunes_; return false; }
        }
        return true;
    }

    bool cap_sym_active_possible(int last_remaining,const cpp_rational& budget) {
        if(last_remaining<0 || budget<=0 || last_remaining>18) return true;
        const long double bf=std::max<long double>(0,budget.convert_to<long double>());
        struct CapCand{ int r; int sign; long double severity; }; struct SymCand{ int r; long double severity; };
        std::vector<CapCand> caps; std::vector<SymCand> syms;
        for(int r=0;r<m_;r++){
            long double center=static_cast<long double>(p_.p0[r])+cap_assignedf_[r],dist=0; int sign=0;
            ...
            long double reach=std::sqrt(std::max<long double>(0,bf*dual)); if(reach<=0) continue;
            long double sev=dist/reach; if(sev>0.65L) caps.push_back({r,sign,sev});
        }
        for(int r=0;r<s_.k;r++){
            long double center=static_cast<long double>(s_.c0[r])+sym_assignedf_[r]; if(center>=0) continue;
            long double dual=sym_dualf_[r][last_remaining]; if(dual<=0) continue;
            long double reach=std::sqrt(std::max<long double>(0,bf*dual)); if(reach<=0) continue;
            long double sev=(-center)/reach; if(sev>0.65L) syms.push_back({r,sev});
        }
        ...
        for(const auto& c:caps) for(const auto& y:syms){
            ...
            cpp_rational cc=exact_center(p_.p0[c.r],cap_a_[c.r],last_remaining),hc=(c.sign>0)?cc:(cpp_rational(p_.cap[c.r])-cc),hs=exact_center(s_.c0[y.r],sym_a_[y.r],last_remaining);
            if(hc>=0 || hs>=0) continue;
            cpp_rational d1=-hc,d2=-hs,g11=cap_dual_[c.r][last_remaining],g22=sym_dual_[y.r][last_remaining],g12=0;
            for(int j=0;j<=last_remaining;j++) g12+=cpp_rational(c.sign)*cap_a_[c.r][j]*sym_a_[y.r][j]/D_[j];
            cpp_rational det=g11*g22-g12*g12; if(det<=0) continue;
            cpp_rational lam1=(d1*g22-d2*g12)/det,lam2=(d2*g11-d1*g12)/det; if(lam1<=0 || lam2<=0) continue;
            if(d1*lam1+d2*lam2>budget){ ++cap_sym_active_prunes_; return false; }
        }
        return true;
    }

    bool combined_active_possible(int last_remaining,const cpp_rational& budget){
        return pairwise_symmetry_possible_cached(last_remaining,budget) && cap_sym_active_possible(last_remaining,budget);
    }

'''
assert s.count(anchor)==1
s=s.replace(anchor,fn+anchor,1)
s=s.replace('''if(caps_possible(n_-1,cpp_rational(bound_)) && symmetry_possible(n_-1,cpp_rational(bound_))) dfs(n_-1,cpp_rational(0));''','''if(caps_possible(n_-1,cpp_rational(bound_)) && symmetry_possible(n_-1,cpp_rational(bound_)) && combined_active_possible(n_-1,cpp_rational(bound_))) dfs(n_-1,cpp_rational(0));''',1)
s=s.replace('''if(caps_possible(i-1,newrem) && symmetry_possible(i-1,newrem)){''','''if(caps_possible(i-1,newrem) && symmetry_possible(i-1,newrem) && combined_active_possible(i-1,newrem)){''',1)
meta='''        f<<"  \\\"exact_symmetry_prunes\\\": "<<symmetry_prunes_<<",\\n";\n'''
add=meta+'''        f<<"  \\\"combined_active_set_deep_only\\\": true,\\n";\n        f<<"  \\\"pairwise_symmetry_checks\\\": "<<pairwise_symmetry_checks_<<",\\n";\n        f<<"  \\\"pairwise_symmetry_prunes\\\": "<<pairwise_symmetry_prunes_<<",\\n";\n        f<<"  \\\"pairwise_crossgram_cache_hits\\\": "<<pairwise_crossgram_cache_hits_<<",\\n";\n        f<<"  \\\"cap_sym_active_checks\\\": "<<cap_sym_active_checks_<<",\\n";\n        f<<"  \\\"cap_sym_active_prunes\\\": "<<cap_sym_active_prunes_<<",\\n";\n'''
