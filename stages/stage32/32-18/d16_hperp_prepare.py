...
def pari_rank(matrix: list[list[int]]) -> int:
    code = f"M={gp_matrix(matrix)};print(matrank(M));quit;\n"
    proc = subprocess.run(["gp", "-q"], input=code, text=True, capture_output=True, check=False)
    if proc.returncode != 0:
        raise RuntimeError(proc.stderr[-4000:])
...
def main() -> None:
    ...
    caps = [8] * 92 + [4] * 48
    pairing_map_rank = pari_rank(lin)
    assert pairing_map_rank == RANK
    ...
    print(json.dumps({
        "schema": "STAGE32_18_D16_HPERP_PREP_V1",
        "rank": RANK,
        "constraints": M,
        "pairing_map_rank": pairing_map_rank,
        "pairing_map_injective": True,
