...
WAVE1_RETAINED=ROOT/"stages/stage32/management/btva-compressed-lift/BTVA-D8-E12-RATIO-DIRECTED-WAVE1-RETAINED.json"
WAVE1_RETAINED_BLOB="09c2955347b849445a2be1fa4d2b955767d43af2"
WAVE1_RETAINED_CANON="a56df6d9cae23937e53b5a3b6ab4bc1544ea4831fc6817b5e47d5b54f87fcf2b"
WAVE2_RETAINED=ROOT/"stages/stage32/management/btva-compressed-lift/BTVA-D8-E12-RATIO-DIRECTED-WAVE2-RETAINED.json"
WAVE2_RETAINED_BLOB="22d21ba084057a498be30be4371c0fa488fe9443"
WAVE2_RETAINED_CANON="2049545a7d61caa1ca5a854de40bc99a3a0a379349600cda91b2a10e80dafa6d"
WAVE2={
    (4,3,2,4,),
    (2,4,3,5,),
    (3,3,3,5,),
...
def main() -> None:
    ...
    wave1=lock_json(WAVE1_RETAINED,WAVE1_RETAINED_BLOB,WAVE1_RETAINED_CANON)
    wave2=lock_json(WAVE2_RETAINED,WAVE2_RETAINED_BLOB,WAVE2_RETAINED_CANON)
    ...
    req(wave1["result"]["all_selected_unsat"] is True and wave1["result"]["exact_wave1_joint_row_floor"]==444329,"wave1 retained boundary")
    req(set(tuple(x) for x in wave1["target"].get("selected_base4_in_order",[])) in (set(),WAVE1),"wave1 key metadata drift") if "selected_base4_in_order" in wave1["target"] else None
    req(len(WAVE1)==16 and not (WAVE1 & TOP16),"wave1 key set drift")
    req(wave2["result"]["all_selected_unsat"] is True and wave2["result"]["exact_wave2_joint_row_floor"]==438674,"wave2 retained boundary")
    req(len(WAVE2)==16 and not (WAVE2 & TOP16) and not (WAVE2 & WAVE1),"wave2 key set drift")
    ...
    req(sum(sum(v.values()) for v in proved_caps.values())==1124463,"proved48 overlap replay")

    caps_after_proved=subtract_key(full_caps,proved_caps)
    base_obj,base_floor,base_post=objective(h18,p15,cells,caps_after_proved)
    req(base_floor==438674,"post-wave2 exact qbin floor replay")

    remaining={k for k in pre_caps_by_key if k not in proved and sum(sum(c.values()) for c in pre_caps_by_key[k].values())>0}
    selected=[]
    ...
    req(len(selected_keys)==SELECT_COUNT and not (set(selected_keys)&proved),"selector overlap proved32")
    out={
      "schema":"STAGE32_MAIN_BTVA_D8_E12_RATIO_DIRECTED_SELECTOR_WAVE3_V1",
      "stage":32,
      "status":"EXACT_HPADJ21_RATIO_DIRECTED_BTVA_WAVE3_SELECTOR_ZERO_CREDIT",
      "target":{"row_id":ROW_ID,"row_index":ROW_INDEX,"g":G,"d":D,"e":E,"selected_count":SELECT_COUNT},
      "baseline":{"top16_wave1_wave2_already_unsat":True,"proved_base4_count":48,"wave2_exact_qbin_row_floor":base_floor,"wave2_row_tightening_candidate":9757},
      ...
      "hypothetical_if_all_selected_unsat":{
        ...
        "additional_tightening_beyond_wave2":base_floor-current_floor,
      ...
      "population":{"e12_base4_key_count":len(raw_blocks),"hpadj21_predomain_base4_key_count":len(pre_caps_by_key),"proved_key_count":48},
      "source_locks":{
        ...
        "top16_key_stream_sha256":TOP16_SHA256,"wave1_retained_blob_sha1":WAVE1_RETAINED_BLOB,"wave1_retained_canonical_sha256":WAVE1_RETAINED_CANON,"wave2_retained_blob_sha1":WAVE2_RETAINED_BLOB,"wave2_retained_canonical_sha256":WAVE2_RETAINED_CANON,
      },
      "semantics":{
