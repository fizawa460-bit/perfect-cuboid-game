...
BAND=7
FULL_INTERVAL=(84,96)
SCHEMA="STAGE32_MAIN_V46_GRF04_HPADJ22_HIGHD_BCHUNK_V1"
...
def count_intersection(counter,xs,*,d,g,tint,q):
    if tint is None or not xs:return 0
    rem=3*d*d+48*d+96-96*g-24*q
    if rem<0:return 0
    ...
    hi=(d//2-tlo+rad)//2
    if lo>hi:return 0
    return bisect.bisect_right(xs,hi)-bisect.bisect_left(xs,lo)
...
def main():
    ...
    a=ap.parse_args()
    req(FULL_INTERVAL[0]<=a.b_start<=a.b_stop<=FULL_INTERVAL[1],"invalid band7 b-chunk")

    direct=load(DIRECT,DIRECT_BLOB,"v46_h22_direct_highd_chunk")
    proj=load(PROJECTION,PROJECTION_BLOB,"v46_grf04_proj_highd_chunk")
    ...
    req(tuple(old.PLANNED[BAND])==FULL_INTERVAL,"band7 interval drift")
    ...
    joint=bc.build_joint_bc_shard(old.HMAX,a.b_start,a.b_stop)
    ...
    for b in range(a.b_start,min(a.b_stop,h)+1):
      for c in range(h+1):
        jbc=joint[b][c]
        active=[];qkeys=set()
        for sbc in range(8):
          for r in (0,1):
            ...
            if clean:
                active.append((sbc,r,clean));qkeys.update(q for q in clean if q<=tq)
        if not active:continue
        c3=counter.component3(d,b,c)
        if c3<0:continue

        tp=time.perf_counter()
        intervals=interval_table(proj,b,c,qkeys);interval_keys+=len(intervals)
        points=x4_points(h16,h,g,b,c)
        eligible={};picard={}
        for r in (0,1):
          for qa in qa_values:
            xs=[x4 for x4,cap in points[r] if cap>=qa]
            eligible[(r,qa)]=xs;picard[(r,qa)]=len(xs)
        weighted={}
        for sbc,r,hist in active:
          for qa in qa_values:
            cut=tq-qa
            if cut<0:
                weighted[(sbc,r,qa)]=(0,0);continue
            xs=eligible[(r,qa)];pcount=picard[(r,qa)]
            survive=jweight=0
            ...
            weighted[(sbc,r,qa)]=(survive,jweight);weighted_cells+=1
        ...
        for aa in range(h+1):
          ...
          for support in range(11):
            ...
            if es:
                req(all(19*d-5*int(e)>=4*d for e in es),f"eligible e lower n regression {(b,c,aa,support)}")
                ec[support]=len(es)
          if not ec:continue
          for sbc,r,hist in active:
            for sa in range(4):
              ...
              for qa,va in tiers:
                survive,jweight=weighted[(sbc,r,qa)]
                baseline+=survive*va*picard[(r,qa)]*ne
    ...
    out={
      "schema":SCHEMA,"stage":32,"status":"HIGHD_EXACT_BCHUNK_COMPLETE_ZERO_CREDIT",
      "target":{"row_index":ROW_INDEX,"row_id":row_id,"g":g,"d":d,"band_position":BAND,
      ...
      "result":{"partial_hpadj22_exact_survivors":baseline,"partial_joint_exact_survivors":joint_exact,
        "partial_exact_improvement":baseline-joint_exact,"strict":joint_exact<baseline,
      ...
      "timing_seconds":{"joint_bc_build":round(t1-t0,6),"joint_precompute":round(precompute,6),
        "joint_accumulate":round(accumulate,6),"total":round(t2-t0,6)},
      "firewalls":{"main_pruning_credit":False,"full178_complete":False,"row177_full_band_complete":False,
    ...
    print("GRF04_HPADJ22_HIGHD_BCHUNK="+json.dumps({"b_chunk":[a.b_start,a.b_stop],
      "baseline":baseline,"joint":joint_exact,"improvement":baseline-joint_exact,
      "timing":out["timing_seconds"],"canonical":out["canonical_sha256_without_this_field"]},sort_keys=True))
