...
def run_one(task,artifact,out,cap,timeout,proof):
    ...
    done=subprocess.run(cmd,capture_output=True,text=True,check=False); cp=out/f'coord-{task.label}'/'checkpoint.json'
