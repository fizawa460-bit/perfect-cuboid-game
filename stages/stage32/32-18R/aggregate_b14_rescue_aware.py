...
def main():
    ...
    for pid in BULK_IDS:
        d,rs,rids=load_packet(a.bulk,pid,manifest,lock); records.extend(rs); residues.extend(rids); packet_sources[str(pid)]='18P_bulk251'; packet_counts[str(pid)]=len(rs)
    ...
    for pid,primary in HOT_PACKET_TO_PRIMARY.items():
        d,rs,rids=load_hot(a.hot,pid,primary,manifest,lock); records.extend(rs); residues.extend(rids); packet_sources[str(pid)]=f'18Q_logical_primary_{primary}'; packet_counts[str(pid)]=len(rs)
