...
def main():
    ...
    ap.add_argument("--fast-json",type=pathlib.Path,required=True)
    ap.add_argument("--fast-dump",type=pathlib.Path,required=True)
    ...
    if fast.get("bound")!=args.bound or fast.get("status")!="COMPLETE":
        raise RuntimeError("fast b10 incomplete")
    fast_records=sorted(read_records(args.fast_dump))
    if records!=fast_records:
        raise RuntimeError("exact-sharded and fast canonical sets differ")

    lower={k:v for k,v in hist.items() if int(k)<=8}
    if lower!=AUDITED_B8_HIST:
        raise RuntimeError(f"audited b8 histogram regression mismatch {lower}")
    if sum(lower.values())!=260:
        raise RuntimeError("audited b8 canonical count regression mismatch")

    args.output_dump.write_bytes(MAGIC+b"".join(records))
    dump_sha=hashlib.sha256(args.output_dump.read_bytes()).hexdigest()

    exact_summary={
      "schema":"STAGE32_18C_D16_B10_SHARDED_EXACT_AGGREGATE_V1",
      "status":"COMPLETE",
      "bound":args.bound,
      "aut_group_order":1536,
      "stable_aut_content_sha256":EXPECTED_AUT,
      "prepared_input_sha256":EXPECTED_HPERP,
    ...
    cert={
      "schema":"STAGE32_18C_D16_B10_RESOURCE_SAFE_PRODUCTION_CERTIFICATE_V1",
      "verdict":"PASS_EXACT_SHARDED_CROSS_CERTIFIED_D16_B10_PRODUCTION",
      "AUDIT_STATUS":"PENDING",
      "bound":args.bound,
      "shard_count":args.shard_count,
      "split_coordinate":split_coordinate,
      "aggregate_shard_nodes":totals["nodes"],
      ...
      "canonical_norm_histogram":hist,
      "new_norm10_canonical_survivors":hist.get("10",0),
