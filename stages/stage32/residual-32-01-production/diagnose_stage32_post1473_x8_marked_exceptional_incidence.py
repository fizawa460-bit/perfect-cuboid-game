#!/usr/bin/env python3
"""Replay the retained full 140-curve intersection and mark exceptional X(8) nodes."""
...
def main() -> None:
    ...
    # Load both retained payloads so their nonexpiring canonical/source locks are
    # checked. The full 140x140 form is intentionally reconstructed directly
    # from the Hperp degree+linear records before the adapter projects it to the
    # ordinary 140x64 retained-basis pairing matrix.
    load_module_payload(args.retained, "stage32_post1473_support_picard")
    ...
    full_sha256 = csha(matrix_list(full))
    ...
    for exceptional_label in EXCEPTIONAL:
        ...
        c2_hits = [label for label in C2_ALL if int(full[label - 1, j]) != 0]
        factor1_hits = [label for label in FIRST_FACTOR if int(full[label - 1, j]) != 0]
        factor2_hits = [label for label in SECOND_FACTOR if int(full[label - 1, j]) != 0]
        values = {str(label): int(full[label - 1, j]) for label in c2_hits}
    ...
    out = {
        "schema": "STAGE32_POST1473_X8_MARKED_EXCEPTIONAL_INCIDENCE_REPLAY_V2",
        "status": "PASS",
        "hperp_text_sha256": hmeta["hperp_text_sha256"],
        "upstream_git_blob_sha1": hmeta["upstream_git_blob_sha1"],
        "full_intersection_sha256": full_sha256,
        "full_intersection_symmetric": True,
