...
def main() -> None:
    ...
    assert int(x["production_generation"]) >= 34
    assert x["source_mode"] in {"BOOTSTRAP_LOCKED_OLD_FRONTIER_V1", "RESUMABLE_FRONTIER_ARTIFACT_V1"}
    assert int(x["source_frontier_count"]) > 0
    assert 0 < int(x["dispatch_count"]) <= 128
    assert int(x["carry_count"]) >= 0
    assert int(x["dispatch_count"]) + int(x["carry_count"]) == int(x["source_frontier_count"])
    if x["source_mode"] == "BOOTSTRAP_LOCKED_OLD_FRONTIER_V1":
        assert isinstance(x.get("source_slices"), list) and x["source_slices"]
    else:
        src=x["source_frontier_artifact"]
        assert int(src["generation"]) == int(x["production_generation"])-1
        assert int(src["run_id"]) > 0 and int(src["artifact_id"]) > 0
        assert len(src["artifact_zip_sha256"]) == 64
        assert len(src["manifest_canonical_sha256"]) == 64
    ...
    raw = {"kind":"STRATUM_PARTITION","row_id":"g0-d012","e":14,"prefix":[0],"next_min":0,"next_max":1}
    first = decode_input(raw, emin=8)
    assert first["kind"] == CURSOR_KIND and first["row_id"] == "g0-d012" and first["e_current"] == 14
    ...
    print(json.dumps({
        ...
        "production_generation":int(x["production_generation"]),
        "source_mode":x["source_mode"],
        "source_frontier_count":int(x["source_frontier_count"]),
        "dispatch_count":int(x["dispatch_count"]),
        "carry_count":int(x["carry_count"]),
