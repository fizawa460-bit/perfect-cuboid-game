#!/usr/bin/env python3
"""Certify Stage32 post1566 orbit-sum / correspondence-commutator batch."""
...
EXPECTED_CERT_CANONICAL = "5b20d05953342c03229a866758816c97a478ad7d24a6dd8797758a1de336d9cb"
EXPECTED_SOURCE_NOTE_BLOB = "2d1e81ac3a23dff248c8feca085519b990d8a1f9"
EXPECTED_DIAGNOSTIC_BLOB = "18506ba6958d561ee104732488cd46fc43fb4e10"
...
def main() -> None:
    ...
    assert cert["schema"] == "STAGE32_POST1566_ORBIT_SUM_CORRESPONDENCE_COMMUTATOR_BATCH_V1"
    ...
    # Recompute the finite core instead of trusting a copied result.
    diag = run_diagnostic()
    assert diag["schema"] == "STAGE32_POST1566_ORBIT_SUM_COMMUTATOR_DIAGNOSTIC_V1"
    assert diag["retained_stoll_group_order"] == 1536
    assert diag["h_deck_group_order"] == 4
    assert diag["h_orbit_sum_stabilizer_count"] == 4
    assert diag["h_orbit_sum_stabilizer_outside_h_count"] == 0
    assert {row["word"] for row in diag["h_orbit_sum_stabilizer_elements"]} == {
        "1", "g7*g8", "g7*g9", "g8*g9"
    }
    assert all(row["is_H_deck_element"] for row in diag["h_orbit_sum_stabilizer_elements"])
    assert diag["beta_B_in_full_stoll"] is True
    assert diag["beta_B_in_H"] is False
    assert diag["beta_B_fixes_h_orbit_sum"] is False
    assert diag["q602_residues"] == [73, 97, 235]
    assert diag["all_q602_residues_noncommuting_mod2"] is True

    fr = cert["finite_result"]
    assert fr["retained_stoll_group_order"] == diag["retained_stoll_group_order"]
    assert fr["h_deck_group_order"] == diag["h_deck_group_order"]
    assert fr["h_orbit_sum_stabilizer_count"] == diag["h_orbit_sum_stabilizer_count"]
    assert fr["h_orbit_sum_stabilizer_outside_h_count"] == 0
    assert set(fr["h_orbit_sum_stabilizer_words"]) == {
        row["word"] for row in diag["h_orbit_sum_stabilizer_elements"]
    }
    assert fr["beta_B_in_full_stoll"] is True
    assert fr["beta_B_in_H"] is False
    assert fr["beta_B_fixes_h_orbit_sum"] is False
    assert fr["q602_residues"] == [73, 97, 235]
    assert fr["all_q602_residues_noncommuting_mod2"] is True

    # Source-bound quotient geometry used for the only reverse pullback step.
    cover = json.loads((ROOT / "stages/stage32/residual-32-01-production/post1484-o210-q4-common-double-cover-cartesian-identity.json").read_text())
    ...
    post1566 = json.loads((ROOT / "stages/stage32/residual-32-01-production/post1563-ambient-symmetry-exhaustion-batch.json").read_text())
    ...
    h_adapter = json.loads((ROOT / "stages/stage32/residual-32-01-production/post1490-o210-q4-equivariant-beauville-deck-cross-exclusion.json").read_text())
    assert h_adapter["equivariant_adapter"]["modular_to_stoll"] == {
        "u": "g7*g9", "uv": "g8*g9", "v": "g7*g8"
    }

    post1532 = json.loads((ROOT / "stages/stage32/residual-32-01-production/post1532-q602-single-b3-commutator.json").read_text())
    ...
    routes = cert["routes"]
    a = routes["A_current_main_orbit_sum"]
    assert a["result"] == "PASS_EXACT_H_ORBIT_SUM_STABILIZER_EQUALS_H"
    assert a["historical_PR1547_used_as_authority"] is False
    assert a["current_main_recomputation_required"] is True
    ...
    assert c["resolved_component_identity"] == "Dtilde=pi_tilde^*C"
    ...
    assert d["transported_Gamma_same_bidegree"] is True
    assert d["difference_bidegree"] == [0, 0]
    assert d["fiber_classes_fixed_by_diagonal_b3"] is True
    assert d["nonzero_NS_difference_survives_fiber_quotient"] is True
    assert d["diagonal_transport_is_conjugation_up_to_inverse_convention"] is True
    assert d["actual_T_b3_noncommutation_proved"] is True
    assert d["conditional_on_exact_carrier_correspondence_existence"] is True

    # A fiber combination a F1 + b F2 has bidegree (b,a) under the retained
    # convention, so bidegree (0,0) forces a=b=0. This is the quotient firewall.
    for acoef, bcoef in ((1, 0), (0, 1), (2, -3), (-5, 7)):
        if (acoef, bcoef) != (0, 0):
            assert (bcoef, acoef) != (0, 0)
    ...
    dec = cert["decision"]
    assert dec["result"] == "PASS_EXACT_ORBIT_SUM_CORRESPONDENCE_NONINVARIANCE_AND_B3_NONCOMMUTATION"
    assert dec["h_orbit_sum_stabilizer_exactly_H"] is True
    assert dec["beta_B_h_orbit_sum_noninvariance_proved"] is True
    ...
    assert dec["valence_refuted"] is True
    assert dec["scalarity_refuted"] is True
    ...
    print(json.dumps({
        ...
        "result": dec["result"],
        "stabilizer_equals_H": dec["h_orbit_sum_stabilizer_exactly_H"],
        "conditional_T_b3_noncommutation": dec["actual_T_b3_noncommutation_proved"],
        "valence_refuted": dec["valence_refuted"],
