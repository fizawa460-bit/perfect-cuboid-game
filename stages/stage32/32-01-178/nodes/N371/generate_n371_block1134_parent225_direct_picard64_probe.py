...
def preflight(cut_root: Path, comp_root: Path):
    ...
    req(n370["attempt"]["parent_ordinal"] == PARENT_ORDINAL and n370["attempt"]["exact_result"] == "unknown" and n370["attempt"]["reason_unknown"] == "timeout", "N370 timeout drift")
    req(state["target"]["parent_ordinal"] == PARENT_ORDINAL and state["method"]["per_parent_timeout_ms"] == 60000, "N371 state drift")
...
def main() -> None:
    ...
    if r == sat:
        ...
        numerator = scale * scale * 8 * 8 - 16 * scale * scale * self_square
    ...
    status = "SAT_CURRENT_V15_WITNESS_CANDIDATE" if witness is not None else ("UNSAT_PARENT225_DIRECT_PICARD64" if r != unknown else "UNKNOWN_PARENT225_DIRECT_PICARD64")
    body = {
