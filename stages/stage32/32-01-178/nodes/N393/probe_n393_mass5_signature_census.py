...
def main() -> None:
    ...
    req(audit391["status"] == "HOSTILE_AUDIT_PASS" and audit391["audited_exact_head"] == EXPECTED["n391_audit_head"] and int(audit391["review_id"]) == EXPECTED["n391_audit_review"], "N391 hostile audit receipt drift")

    audit392 = json.loads((repo / "stages/stage32/32-01-178/nodes/N392/AUDIT-PASS.json").read_text())
    req(audit392["status"] == "HOSTILE_AUDIT_PASS_RETAINED", "N392 audit status drift")
    req(audit392["audited_exact_head"] == EXPECTED["n392_audit_head"], "N392 audit head drift")
    req(int(audit392["hostile_audit_review_id"]) == EXPECTED["n392_audit_review"], "N392 audit review drift")
    ...
    n357 = load_module(n357_path, "n393_n357")
    ...
    req(len(entries) == 30, "mass5 count drift")
    payload = {
        ...
        "mass5_blocks": [e["block"] for e in entries],
        "mass5_offsets": [e["survivor_offset"] for e in entries],
        "reduced_abc_signature_counts": {"%d,%d,%d" % k:v for k,v in sorted(reduced.items())},
        "entries": entries,
    }
    print("PASS_N393_MASS5_SIGNATURE_PROBE")
    print("N393_PROBE_JSON=" + json.dumps(payload, sort_keys=True, separators=(",",":")))
