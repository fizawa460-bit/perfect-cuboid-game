...
def lock_extra_sources() -> None:
    for name, (path, expected) in EXTRA_LOCKS.items():
        req(path.is_file(), f"missing source lock {name}: {path}")
        req(git_blob(path) == expected, f"source drift {name}")
    ex6_text = EX6.read_text()
    req(STOLL_TESTA_SECTION5_LOG_BLOB in ex6_text, "Stoll-Testa Section5 source-lock regression")
...
def build_kernel(vf) -> Kernel:
    ...
    residual_supports = [tuple(sorted(set(block) - set(observed))) for block in blocks]
    req([list(x) for x in residual_supports[:5]] == EXPECTED_SUPPORTS, "five residual support regression")
    req(len(residual_supports) == 6 and all(x for x in residual_supports), "six residual supports must be nonempty")
    ...
    q, degree, linear, _caps, _hmeta = _parse_hperp(marking["hperp_text"])
    ...
    req(penalty == penalty.T and all(sympy.factor(penalty[:k, :k].det()) > 0 for k in range(1, 6)), "penalty positive-definite regression")
    ...
    # Exact bridge to retained GRF04.  This is symbolic, not a sample check:
    # min_Hperp(x) = q/2 + (d/2 - 2*x4 - t)^2/12.
    xs = sympy.symbols("x0:11")
    d = sympy.symbols("d")
    zx = Matrix([xs[i] - Rational(d * x_degrees[i], 16) for i in range(11)])
    matrix_rho = sympy.expand((zx.T * old_norm * zx)[0])
    qexc = sum(xs[i] ** 2 for i in range(11) if i != 4)
    t = xs[0] + xs[1] + xs[6] + xs[9]
    closed_rho = Rational(1, 2) * qexc + Rational(1, 12) * (Rational(d, 2) - 2 * xs[4] - t) ** 2
    ...
    return Kernel(
        blocks=tuple(tuple(x) for x in blocks),
        observed_exceptionals=observed,
        residual_supports=tuple(residual_supports),
        x_degrees=x_degrees,
        r_degrees=r_degrees,
        old_norm_matrix=old_norm,
        center=center,
        penalty=penalty,
    )
...
def quadratic_lower(prefix: tuple[int, ...], mu: Matrix, lower_mats: list[Matrix]) -> sympy.Expr:
    depth = len(prefix)
    if depth == 0:
        return Rational(0)
    delta = Matrix([Rational(prefix[i]) - mu[i, 0] for i in range(depth)])
    return eval_quad(lower_mats[depth], delta)
...
def residual_problem(kernel: Kernel, *, g: int, d: int, e: int, x: tuple[int, ...]) -> dict:
    ...
    req(tuple(assignment) == tuple(load_module(RECOVERABILITY, "bnb_assignment_check").ASSIGNMENT_LABELS), "assignment order regression")
    ...
    for block in kernel.blocks:
        known.append(sum(x[pos[lab]] for lab in block if lab in observed))
    ...
    if penalty_budget < 0:
        return {"structurally_infeasible": True, "reason": "already_rejected_by_GRF04", "old_rho": old_rho, "genus_budget": genus_budget}

    mu = Matrix([
        Rational(d * kernel.r_degrees[i], 16) + (kernel.center * zx)[i, 0]
        for i in range(5)
    ])
    return {
        ...
        "m10": m10,
        "remaining": remaining,
        "known_block_masses": tuple(known),
        "caps": tuple(caps),
        "sum_lo": sum_lo,
        "sum_hi": sum_hi,
        "old_rho": old_rho,
...
def bnb_count(kernel: Kernel, problem: dict, *, stop_after_one: bool = False) -> tuple[int, tuple[int, ...] | None, Stats]:
    if problem.get("structurally_infeasible"):
        return 0, None, Stats()
    caps = problem["caps"]
    sum_lo = int(problem["sum_lo"])
    sum_hi = int(problem["sum_hi"])
    mu = problem["mu"]
    budget = problem["penalty_budget"]
    lower_mats = prefix_lower_matrices(kernel.penalty)
    suffix_caps = [0] * 6
    ...
    def rec(prefix: tuple[int, ...], total: int) -> bool:
        ...
        if total > sum_hi or total + suffix_caps[depth] < sum_lo:
            stats.sum_prunes += 1
            return False
        if quadratic_lower(prefix, mu, lower_mats) > budget:
        ...
        if lo > hi:
            stats.sum_prunes += 1
            return False
        # Try values near the unconstrained center first so survivor queries can
        # stop early; every value remains covered for exact rejection/counting.
        center = mu[depth, 0]
        values = list(range(lo, hi + 1))
        values.sort(key=lambda v: (abs(Rational(v) - center), v))
        for v in values:
...
def brute_count(kernel: Kernel, problem: dict) -> tuple[int, tuple[int, ...] | None]:
    if problem.get("structurally_infeasible"):
        return 0, None
    count = 0
    first = None
    for r in itertools.product(*(range(c + 1) for c in problem["caps"])):
        s = sum(r)
        if not (problem["sum_lo"] <= s <= problem["sum_hi"]):
            continue
...
def main() -> None:
    ...
    # Exact finite-domain regression against independent brute force.  These are
    # algebraic smoke panels only, not Stage32 bounded pruning/census credit.
    regressions = []
    xzero = (0,) * 11
    for e in (0, 2, 4, 6, 8):
        p = residual_problem(kernel, g=0, d=8, e=e, x=xzero)
        bcount, bwitness, stats = bnb_count(kernel, p)
        rcount, rwitness = brute_count(kernel, p)
        req(bcount == rcount, f"B&B/brute count mismatch e={e}: {bcount} != {rcount}")
        req((bwitness is None) == (rwitness is None), f"B&B/brute feasibility mismatch e={e}")
        one_count, one_witness, _ = bnb_count(kernel, p, stop_after_one=True)
        req((one_count > 0) == (bcount > 0), f"B&B stop-after-one mismatch e={e}")
        req((one_witness is None) == (bcount == 0), f"B&B witness mismatch e={e}")
        regressions.append({
            "g": 0,
            "d": 8,
            "e": e,
    ...
    out = {
        ...
        "purpose": "exact five-residual aggregate feasibility engine combining N220 mass, six rank-3 fibration-nef inequalities, and the retained GRF04/Hperp completion budget; no production census",
        "source_authority": {
        ...
        "regression": {
            "panels": regressions,
            "all_bnb_counts_match_independent_bruteforce": True,
        },
