...
s=p.read_text()
perm=list(reversed(range(48)))+list(range(48,63))
assert sorted(perm)==list(range(63))
assert perm[48:]==list(range(48,63))
plist=','.join(map(str,perm)); lower=','.join(map(str,perm[:48]))
anchor='static long long floor_rat(const cpp_rational& x){\n'
assert s.count(anchor)==1
helper=f'''static void apply_lower48_reverse_permutation(Problem& p, Bundle& s){{
...
new='Problem p=load_problem(input); Bundle s=load_bundle(bundle,p); apply_lower48_reverse_permutation(p,s); ExactEnumerator e(p,s); e.run(bound,node_cap,dump,shard_id,shard_count,split_i); e.write_json(output); return 0;'
assert s.count(old)==1
s=s.replace(old,new,1)
meta='        f<<"  \\"split_coordinate\\": "<<split_i_<<",\\n";\n'
assert s.count(meta)==1
extra=meta+'        f<<"  \\"coordinate_order_redirection\\": \\"lower48_reverse_v1\\",\\n";\n'+'        f<<"  \\"coordinate_relabeling_semantics_only\\": true,\\n";\n'+'        f<<"  \\"partition_coordinates_48_62_fixed\\": true,\\n";\n'+f'        f<<"  \\"lower48_permutation\\": [{lower}],\\n";\n'
