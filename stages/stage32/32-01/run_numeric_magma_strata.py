...
phi:=hom<Z64 -> Z2 | [{imgs}]>;
t:=Z2![{d},{e}];
if t notin Image(phi) then
  printf "STAGE32_STRATUM d={d} g={g} e={e} feasible=0 raw=0 kept=0\\n";
else
  c0z:=t @@ phi; C0:=Pic!Eltseq(c0z); K:=Kernel(phi);
  ...
  printf "STAGE32_STRATUM d={d} g={g} e={e} feasible=1 raw=%o kept=%o\\n",raw,kept;
end if;
...
'''
    ...
    try:
      ...
      out='\n'.join(lines); ok='STAGE32_STRATUM_END' in out and not any(x in out for x in ('Runtime error','User error','Internal error'))
      return {'e':e,'ok':ok,'seconds':round(time.time()-t0,3),'stdout':out}
    except Exception as exc:
      return {'e':e,'ok':False,'seconds':round(time.time()-t0,3),'error':repr(exc)}
