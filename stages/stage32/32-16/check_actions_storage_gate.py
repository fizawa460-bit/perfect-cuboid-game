...
SCHEMA = "STAGE32_16_ACTIONS_RETAINED_STORAGE_GATE_V1"
CONSERVATIVE_INCREMENTAL_CEILING_BYTES = 100_000_000
NEW_COMPACT_CERTIFICATE_BOUND_BYTES = 287 * 20_000
NEW_BUNDLE_RECEIPT_BOUND_BYTES = 48 * 100_000
...
REQUIRED = {
    ...
    (32726279718, "stage32-15-e20-a0-le16384-exact-tier"),
}
...
def main() -> None:
    ...
    # Pagination completeness is checked against the endpoint's declared total.
    ...
    projected_peak = retained_bytes + NEW_RUN_OWN_BOUND_BYTES
    ...
    report = {
        ...
        "retained_stage32_artifact_bytes": sum(
            int(row["size_in_bytes"]) for row in stage32_rows
        ),
        "required_predecessor_artifacts": required_rows,
        "required_predecessor_artifact_bytes_already_in_retained_total": sum(
            int(row["retained_bytes"]) for row in required_rows
        ),
        ...
        "new_run_worst_case": {
            ...
            "below_100mb_fallback_requirement": NEW_RUN_OWN_BOUND_BYTES
            < 100_000_000,
            "does_not_depend_on_deletion": True,
        },
        "projected_simultaneous_retained_peak_bytes": projected_peak,
        "new_run_incremental_headroom_bytes": CONSERVATIVE_INCREMENTAL_CEILING_BYTES
        - NEW_RUN_OWN_BOUND_BYTES,
        "safe_to_launch_pilot_or_fanout": safe,
        "stop_reason": (
            None
            if safe
            else "NEW_RUN_WORST_CASE_INCREMENTAL_STORAGE_EXCEEDS_100MB_GATE"
        ),
    ...
    if args.enforce and not safe:
        raise SystemExit(
            "STOP: new-run incremental Actions storage violates the conservative 100 MB gate"
        )
