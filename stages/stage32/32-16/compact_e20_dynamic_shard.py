...
OUT_SCHEMA = "STAGE32_D8_E20_A0_DYNAMIC_SHARD_COMPACT_CERT_V1"
...
def compact_row(row: dict[str, Any]) -> dict[str, Any]:
    """Verify a raw exact shard completely before returning its compact certificate."""
    ...
    for branch in branches:
        ...
        leaf = {
            ...
            "survivor_basis_shas": [
                s["basis_coordinates_sha256"] for s in search["survivors"]
            ],
        }
    ...
    raw_survivors = sorted(
        row["numerical_survivors"], key=lambda r: tuple(r["basis_coordinates"])
    )
    ...
    report = {
        ...
        "last_branch_index": (
            shard_index + (want - 1) * shard_count if want else None
        ),
...
def main() -> None:
    ...
    args = ap.parse_args()
    report = compact_row(json.loads(args.input.read_text()))
        ...
        json.dumps(
            {
                ...
                "shard": int(p["shard_index"]),
                "shard_count": int(p["shard_count"]),
                "branches": report["executed_shard_branch_count"],
                "survivors": report["exact_numerical_survivor_count_in_shard"],
                "raw_sha": report["source_raw_deterministic_sha256"],
