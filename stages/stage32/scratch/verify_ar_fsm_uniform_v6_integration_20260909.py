...
EXPECTED_OUT="66fa8a9122b518ef3bd39c853806e89765a4c0a4ac03a277d93d3ab62dbea1c0"
...
assert sum(min(x,9) for x in m)==232
assert sum(min(x,10) for x in m)==244
labels10=[i+1 for i,x in enumerate(m) if x>=10]
assert labels10==[16,18,23,27,28,34,37,38,39,42,44,48]
excess=sorted((x-1 for x in m if x>0), reverse=True)
assert sum(excess[:23])==190
assert sum(excess[:24])==194
assert 47-28==19

# Minimal-branch node-load ladder from sum x_i >= 186 and x_i <= m_i.
expected={2:14,3:10,4:7,5:5,6:2}
for k,qmin in expected.items():
    base=sum(min(x,k-1) for x in m)
    gains=sorted((max(0,x-(k-1)) for x in m), reverse=True)
    s=base
    q=0
    while s<186:
        s+=gains[q]
...
print(json.dumps({
    "verdict":"PASS_STAGE32_MAIN_SCRATCH_AR_FSM_UNIFORM_V6_INTEGRATION_REPAIRED",
    "minimum_node_branches":238,
    "minimum_multibranch_nodes":24,
    "minimum_contact_saturated_nodes":19,
    "high_branch_threshold":10,
    "high_branch_candidate_labels":labels10,
    "minimal_branch_node_ladder":expected,
    "D_node":"-(1488+8E)k",
