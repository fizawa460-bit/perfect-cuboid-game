...
OUT_SCHEMA = "STAGE32_D8_E20_A0_EXACT_TIER_LE114186_V1"
...
def main() -> None:
    ...
    target_rows = [
        ...
        if int(r["materialized_branch_count"]) <= TARGET_THRESHOLD
    ]
    delta_rows = [
    ...
    planned = {
        (int(c["cell_index"]), str(c["cell_id"])): (
            int(c["total_branches"]), int(c["shard_count"])
        )
    ...
    expected_prev_keys = {
        (int(r["cell_index"]), str(r["cell_id"]))
    ...
    for bundle_id, bundle in expected_bundles.items():
        ...
        got_keys = [
            (int(i["cell_index"]), int(i["shard_index"])) for i in receipt["certificates"]
        ]
    ...
    assert summary_keys == target_keys and len(all_summaries) == EXPECTED_CUMULATIVE_CELLS
    total_branches = sum(int(c["total_branch_count"]) for c in all_summaries)
    assert total_branches == EXPECTED_CUMULATIVE_BRANCHES
