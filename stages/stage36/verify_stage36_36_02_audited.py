...
def main():
 ...
 req(schema in {V5,V6,V7,V8,V9},"36-02 audited successor schema moved")
 req(s.get("stage36_36_02_authority")=={
  "pr":1541,"hostile_audit_review":AUDIT_REVIEW,"audited_head":AUDITED_HEAD,
  "merged_main_sha":AUDITED_PR_MERGE,"exact_head_ci_run":AUDIT_CI_RUN,
  "exact_head_ci_job":AUDIT_CI_JOB,"inventory_blob_sha":AUDITED_INVENTORY_BLOB,"verdict":"PASS",
 },"36-02 authority block moved")
 req(s.get("completed_units",{}).get("36-02")=={
 ...
 if schema==V5:
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==AUDITED_PR_MERGE,"V5 lifecycle moved")
  req("36-03" not in s.get("completed_units",{}),"36-03 started in V5")
 elif schema==V6:
  req(s.get("status")=="ACTIVE_PENDING_HOSTILE_AUDIT" and s.get("base_main_sha")==V6_BASE,"V6 lifecycle moved")
  req(g.get("physical_open_push_and_boundary_complete") is False and s.get("current",{}).get("unit")=="36-03","V6 boundary moved")
 elif schema==V7:
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==V7_BASE,"V7 lifecycle moved")
  req(g.get("physical_open_push_and_boundary_complete") is True and s.get("current",{}).get("unit")=="36-04","V7 successor moved")
  req("36-04" not in s.get("completed_units",{}),"36-04 started in V7")
 elif schema==V8:
  ...
  req(g.get("physical_open_push_and_boundary_complete") is True and g.get("pointwise_H_torsor_class_explicit") is False,"V8 gate boundary moved")
  req(s.get("current",{}).get("unit")=="36-04","V8 current unit moved")
  req(s.get("completed_units",{}).get("36-04",{}).get("promotion_status")=="PROVISIONAL_NOT_AUDITED","36-04 prematurely audited")
  req("36-05" not in s.get("completed_units",{}),"36-05 started in V8")
 else:
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==V9_BASE,"V9 lifecycle moved")
  req(g.get("physical_open_push_and_boundary_complete") is True and g.get("pointwise_H_torsor_class_explicit") is True,"V9 audited gate lost")
  req(s.get("completed_units",{}).get("36-04",{}).get("promotion_status")=="AUDITED","36-04 audit promotion lost")
  req(s.get("current",{}).get("unit")=="36-05","V9 successor moved")
  req("36-05" not in s.get("completed_units",{}),"36-05 started in V9")
 if schema!=V5:
  p=s.get("stage36_36_02_promotion",{})
  req(p.get("pr")==1548 and p.get("merged_main_sha")==PROMOTION_MERGE and p.get("NEW_THEOREM_CREDIT") is False,"36-02 promotion provenance moved")
