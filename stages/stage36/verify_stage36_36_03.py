...
BASE = "26fb608cb2551ab2102ae36ad3b57c063959df58"
...
def canonical_point(p: tuple[int, int, int]) -> tuple[int, int, int]:
    g = 0
    for x in p:
        g = gcd(g, abs(x))
    require(g > 0, "parallel/duplicate projective lines")
    p = tuple(x // g for x in p)
    for x in p:
        if x:
            if x < 0:
                p = tuple(-y for y in p)
            break
    return p


def dot(a: tuple[int, int, int], p: tuple[int, int, int]) -> int:
    return sum(x * y for x, y in zip(a, p))


def reconstruct_arrangement(lines: dict[str, tuple[int, int, int]]):
    points: dict[tuple[int, int, int], tuple[str, ...]] = {}
    for left, right in itertools.combinations(lines, 2):
        p = canonical_point(cross(lines[left], lines[right]))
        incidence = tuple(sorted(name for name, coeff in lines.items() if dot(coeff, p) == 0))
        points[p] = incidence
    triples = {(p, inc) for p, inc in points.items() if len(inc) == 3}
    doubles = {(p, inc) for p, inc in points.items() if len(inc) == 2}
    require(all(len(inc) <= 3 for inc in points.values()), "fourfold concurrence appeared")
...
def main() -> None:
    ...
    # S30-WF02: bind the audited 36-02 inventory exactly; do not trust mutable state for its mathematical identity.
    ...
    source_auth = cert.get("source_authority", {})
    require(source_auth.get("stage36_36_02_hostile_audit_review") == 5113379283, "36-02 audit review moved")
    require(source_auth.get("stage36_36_02_audited_head") == "3a78f9ff156b53f509625d353df48d1b3e02b836", "36-02 audited head moved")
    require(source_auth.get("stage36_36_02_promotion_merge") == BASE, "36-02 promotion authority moved")
    require(source_auth.get("stage36_36_02_inventory") == {"path": "stages/stage36/36-02/representative-inventory.json", "blob_sha": AUDITED_36_02_INVENTORY_BLOB}, "36-02 inventory authority declaration moved")

    declared_sources = cert.get("source_locks", {})
    for key, (rel, sha) in SOURCES.items():
        require(declared_sources.get(key) == {"path": rel, "blob_sha": sha}, f"36-03 source declaration moved: {key}")
        require(blob_sha(ROOT / rel) == sha, f"36-03 source blob drift: {key}")

    declared_arsenal = cert.get("arsenal_locks", {})
    for key, (rel, sha) in ARSENAL.items():
        row = declared_arsenal.get(key, {})
        require(row.get("path") == rel and row.get("blob_sha") == sha, f"36-03 Arsenal declaration moved: {key}")
        require(blob_sha(ROOT / rel) == sha, f"36-03 Arsenal blob drift: {key}")
    ...
    for needle in [
        ...
        require(needle in roadmap, f"Stage36 roadmap anchor missing: {needle}")

    hub = json.loads((ROOT / SOURCES["stage29_endpoint_hub_graph"][0]).read_text())
    nodes = {row["id"]: row for row in hub.get("nodes", [])}
    edges = {row["id"]: row for row in hub.get("edges", [])}
    require(nodes.get("U_PHYS", {}).get("base_field") == "Q", "U_PHYS field moved")
    require("a1*a2*a3 != 0" in nodes.get("U_PHYS", {}).get("data", ""), "U_PHYS side-open definition moved")
    require("resolution is an isomorphism here" in nodes.get("U_PHYS", {}).get("data", ""), "U_PHYS resolution scope moved")
    require(nodes.get("CAMPEDELLI_BAR_H", {}).get("data", "").startswith("Cbar_H=Sbar/H; six A1 points"), "canonical Campedelli node moved")
    require(nodes.get("CAMPEDELLI_H", {}).get("base_field") == "Q", "resolved Campedelli Q-form moved")

    e03 = edges.get("E03", {})
    require(e03.get("relation") == "CREPANT_RESOLUTION_TO_CANONICAL_MODEL", "E03 relation moved")
    ...
    require(e15.get("proof_status") == "AUDITED_EXACT_FOR_ALL_TEN_KERNELS", "E15 ten-kernel scope moved")
    require(e15.get("rational_point_pushforward") is True and e15.get("rational_point_lift") == "requires H-torsor descent", "E15 rational transfer moved")
    e15r = edges.get("E15R", {})
    ...
    e16r = edges.get("E16R", {})
    require(e16r.get("relation") == "CREPANT_RESOLUTION_TO_CANONICAL_MODEL", "E16R relation moved")
    ...
    require("U=Sbar∩D_+(a1a2a3)" in crosswalk, "crosswalk U definition moved")
    require("smooth and resolution-isomorphic" in crosswalk, "crosswalk open resolution scope moved")
    ...
    for needle in [
        "H ∩ I_x = 0",
        "H`-action on the entire singular canonical model `Sbar` is free",
        "finite etale `H`-torsor, even at the rational-double-point locus",
        "There are exactly six such `A1` points.",
        "S  --etale degree 8-->  C_H",
        "On the physical endpoint open, all seven branch forms are nonzero",
        "U_endpoint(Q) -> C_H(Q)",
    ...
    for needle in [
        ...
        "exact physical-open image and deleted boundary",
        "three certified Q-symmetry representatives",
    ]:
        require(needle in routing, f"arithmetic-routing anchor missing: {needle}")

    line_section = cert.get("seven_line_base", {})
    declared_lines = {name: tuple(v) for name, v in line_section.get("line_coefficients", {}).items()}
    require(declared_lines == EXPECTED_LINES, "seven-line coefficient system moved")
    triples, doubles = reconstruct_arrangement(declared_lines)
    require(triples == EXPECTED_TRIPLES, f"triple-point reconstruction mismatch: {triples}")
    require(doubles == EXPECTED_DOUBLES, f"double-point reconstruction mismatch: {doubles}")
    require(len(triples) == 6 and len(doubles) == 3, "arrangement intersection count moved")
    ...
    declared_triples = {(tuple(row["point"]), tuple(sorted(row["lines"]))) for row in line_section.get("triple_points", [])}
    declared_doubles = {(tuple(row["point"]), tuple(sorted(row["lines"]))) for row in line_section.get("remaining_double_points", [])}
    require(declared_triples == EXPECTED_TRIPLES, "certificate triple-point ledger moved")
    require(declared_doubles == EXPECTED_DOUBLES, "certificate double-point ledger moved")
    require(all(row.get("xyz_zero") is True for row in line_section.get("triple_points", [])), "triple xyz boundary flag moved")
    require(all(row.get("xyz_nonzero") is True for row in line_section.get("remaining_double_points", [])), "double xyz-open flag moved")

    physical = cert.get("physical_open", {})
    require(physical.get("smooth") is True and physical.get("resolution_isomorphism_on_open") is True, "physical open smooth/resolution fact moved")
    ...
    require(chain.get("H_group") == "(Z/2)^3", "H group moved")
    require(chain.get("H_action_free_on_all_Sbar") is True, "global H freeness moved")
    require(chain.get("canonical_H_torsor_finite_etale") is True and chain.get("resolved_H_torsor_finite_etale") is True, "H etale quotient moved")
    require(chain.get("H_torsor_degree") == 8 and chain.get("Q_defined") is True, "H quotient degree/Q-form moved")
    ...
    require(boundary.get("Cbar_H_A1_count") == 6, "Campedelli A1 count moved")
    require(boundary.get("all_Cbar_H_A1_points_outside_physical_side_nonzero_open") is True, "Campedelli A1 physical-boundary fact moved")
    require(boundary.get("Sbar_A1_count") == 48 and boundary.get("Sbar_A1_points_over_each_triple_point") == 8, "Sbar A1 ledger moved")
    require(boundary.get("all_48_Sbar_A1_points_outside_Ubar") is True, "Sbar A1 open exclusion moved")
    require(boundary.get("C_H_exceptional_minus2_curves_over_A1_count") == 6, "C_H exceptional count moved")
    require(boundary.get("physical_image_avoids_C_H_exceptional_curves") is True, "physical image exceptional avoidance moved")
    require(boundary.get("canonical_and_resolved_targets_identical_on_physical_image") is True, "canonical/resolved physical-image identification moved")

    image = cert.get("physical_rational_image", {})
    require(image.get("base_branch_free_open") == "A = P2 minus D7", "branch-free base open moved")
    ...
    require(image.get("endpoint_Q_point_to_each_representative_branch_free_quotient_open_Q") is True, "endpoint branch-free quotient image moved")
    require(image.get("converse_lift_claim") is False, "converse lift overclaim")

    firewall = cert.get("scheme_vs_rational_firewall", {})
    require(firewall.get("Ubar_Qbar_subset_of_pi_inverse_A") is False, "scheme/rational open firewall lost")
    ...
    require(behavior.get("H_torsor_degenerates_on_boundary") is False, "H-torsor degeneration incorrectly asserted")
    require(behavior.get("H_torsor_is_global_etale_even_at_RDP") is True, "global H-torsor scope lost")
    require(behavior.get("beta_H_to_P2_branched_on_D7") is True, "beta_H branch locus lost")
    require(behavior.get("radical_squareclass_chart_requires_nonzero_line_values") is True, "radical chart domain moved")
    require(behavior.get("radical_zero_branch_cases_require_separate_treatment") is True, "zero/branch cases silently discarded")
    require(behavior.get("physical_rational_endpoint_image_avoids_those_zero_cases") is True, "physical rational branch avoidance lost")
    require(behavior.get("global_C_H_Q_points_on_branch_are_not_ruled_out") is True, "global quotient branch points prematurely removed")
    require(behavior.get("global_C_H_Q_emptiness_claim") is False, "global quotient emptiness overclaim")

    restricted = cert.get("restricted_receiver_preparation", {})
    require(restricted.get("S34_W03_prepared") is True, "S34-W03 receiver preparation missing")
    require(restricted.get("receiver_intersection_exclusion_executed") is False, "S34-W03 executed prematurely")
    require(restricted.get("receiver_closed") is False, "receiver prematurely closed")
    ...
    require(promotion.get("hostile_audit_required") is True, "36-03 hostile audit gate missing")
    require(promotion.get("promoted_to_audited_authority") is False, "36-03 prematurely promoted")
    require(promotion.get("next_leaf_before_audit_allowed") is False, "36-04 prematurely allowed")
    ...
    require(state.get("status") == "ACTIVE_PENDING_HOSTILE_AUDIT", "Stage36 36-03 audit status moved")
    require(state.get("base_main_sha") == BASE, "Stage36 36-03 state base moved")
    gates = state.get("promotion_gates", {})
    require(gates.get("source_authority_lock_complete") is True, "36-01 audited gate lost")
    require(gates.get("three_Q_representatives_exact") is True, "36-02 audited gate lost")
    require(gates.get("physical_open_push_and_boundary_complete") is False, "36-03 gate prematurely promoted")
    ...
    require(unit.get("status") == "EXACT_PHYSICAL_OPEN_PUSH_BOUNDARY_PENDING_HOSTILE_AUDIT", "36-03 provisional unit status moved")
    require(unit.get("ENDPOINT_TO_EACH_Q_REPRESENTATIVE_PUSH_EXACT") is True, "36-03 exact push result missing")
    require(unit.get("CONVERSE_LIFT_CLAIM") is False, "36-03 converse overclaim")
    require(unit.get("NEW_THEOREM_CREDIT") is False, "36-03 theorem credit leaked")
    require(unit.get("promotion_status") == "PROVISIONAL_NOT_AUDITED", "36-03 prematurely promoted")
    current = state.get("current", {})
    require(current.get("unit") == "36-03", "current unit advanced past 36-03")
    require(current.get("next_exact_leaf") == "36-03_PHYSICAL_OPEN_PUSH_AND_BOUNDARY", "36-04 started before audit")
