...
LOCKS={
 BV:'11b2b927f04c6a7ad151d2456fc79d27329b8dad',
 BVV:'345fdf7c8133883c3eb261e1313ee56a7b9d7f7f',
...
def main():
    ...
    c=json.loads(CERT.read_text()); bv=json.loads(BV.read_text()); bu=json.loads(BU.read_text()); ae=json.loads(AE.read_text()); bi=json.loads(BI.read_text()); bj=json.loads(BJ.read_text())
    ...
    for a0 in range(-15,16):
      for b0 in range(-15,16):
        ...
        Q=a0*a0+b0*b0; D0=a0*b0*(a0-b0)*(a0+b0)
        sigma=v2int(Q)[0]; delta=v2int(D0)[0]
        assert sigma in (0,1)
    ...
    for A in (1,7):
      for B in (1,7):
        for u in range(1,16,2):
          for v in range(1,16,2):
            minus=(A*u*u-B*v*v)%8; plus=(A*u*u+B*v*v)%8
            if A==B:
        ...
        for mu0 in (1,3,5,7):
          for X in range(0,5):
            for Y in range(0,5):
              for xu in (1,3,5,7):
                for yu in (1,3,5,7):
                  x=Fraction((2**X)*xu,1); y=Fraction((2**Y)*yu,1)
                  F=(x*x-(2**g)*mu0*y*y)/a
                  pred=predict_generic(a,g,mu0,x,y)
                  if pred is None:
                      assert F==0
                  else:
                      assert pred==q2square(F),(a,g,mu0,X,Y,xu,yu,F,pred,q2square(F))

    n=c['notation']; src=c['primitive_source_Q2_cases']; rule=c['generic_two_term_Q2_square_rule']; filt=c['branch_only_prime2_filters']; rem=c['remaining_same_AB_taxonomy']; ay=c['AY_specialization_check']; rr=c['route_result']
    assert n['primitive_parameter_bound']=='delta>=2*sigma+1'
    assert n['Lminus']=='v2(Tminus)=(2+delta+f-e-g)/2'
    assert n['Lplus']=='v2(Tplus)=(2+delta+e-f-g)/2'
    ...
    for a0,b0,expected in [(1,2,(14,8,6,4)),(2,11,(158,77,52,30))]:
        rows=mod.ae_outer(a0,b0); Q=a0*a0+b0*b0; D0=a0*b0*(a0-b0)*(a0+b0)
        sigma=v2int(Q)[0]; delta=v2int(D0)[0]
        qrows=[r for r in rows if r[-1]]
        same=[r for r in qrows if r[0]%8==r[1]%8]
        final=[r for r in same if not (r[6]==1 and delta==2*sigma+2-r[5])]
        assert (len(rows),len(qrows),len(same),len(final))==expected,(a0,b0,len(rows),len(qrows),len(same),len(final))
    d=c['exact_fixed_p_diagnostics']
    assert (d['p_1_over_2']['AW_AE_outer'],d['p_1_over_2']['after_Q_row'],d['p_1_over_2']['after_AB_residue_filter'],d['p_1_over_2']['after_critical_deep_filter'])==(14,8,6,4)
    assert (d['p_2_over_11']['AW_AE_outer'],d['p_2_over_11']['after_Q_row'],d['p_2_over_11']['after_AB_residue_filter'],d['p_2_over_11']['after_critical_deep_filter'])==(158,77,52,30)
    ...
    assert st['schema']=='STAGE36_CAMPEDELLI_UNIFORM_TORSOR_MAIN_STATE_V112_36_09BW_GENERAL_Q2_TAXONOMY'
    bw=st['authority_frontier']['36-09BW']
    assert bw['certificate_blob_sha']==CERT_BLOB
    assert bw['GENERAL_PRIME2_BRANCH_TAXONOMY_COMPLETE'] is True
    assert bw['AB_MOD8_BRANCH_FILTER'] is True and bw['CRITICAL_DEEP_BRANCH_FILTER'] is True
    ...
    print('36-09BW verified: general AW Q2 valuation/unit taxonomy is exact. Full branches require A=B mod8, and the critical deep row delta=2*sigma+2-e is impossible; diagnostics prune 8->4 and 77->30 after the BU Q row. BX selected.')
