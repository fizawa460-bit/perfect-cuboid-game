...
def main():
 ...
 req(cert.get("pass_condition")=={"UNIFORM_FINITE_RAMIFICATION_SUPPORT_PROVED":False,"FINITE_EXHAUSTIVE_H_TWIST_FAMILY":False,"ARBITRARY_PRIME_PHYSICAL_RECEIVER_POINT_CLAIM":False},"36-05 audited pass-condition moved")
 geo=cert.get("fixed_geometric_support",{}); nonzero_abs=sorted({abs(det(*(LINES[k] for k in comb))) for comb in itertools.combinations(LINES,3) if det(*(LINES[k] for k in comb))})
 req(nonzero_abs==[1,2] and geo.get("absolute_nonzero_triple_determinants")==[1,2],"36-05 determinant spectrum moved")
 req(geo.get("seven_line_arrangement_combinatorial_bad_prime_candidates")==[2] and geo.get("FULL_C_H_GOOD_REDUCTION_OUTSIDE_2_PROVED") is False,"36-05 geometric firewall moved")
 gap=cert.get("arithmetic_specialization_gap",{})
 for key in ["UNIFORM_Q_PRIME_SUPPORT_PROVED","numerator_denominator_support_controlled_uniformly","primitive_receiver_gcd_resultant_support_theorem_available","S_integrality_theorem_available","ARBITRARY_PRIME_PHYSICAL_RECEIVER_POINT_CLAIM","PHYSICAL_RECEIVER_POINT_FAMILY_EXHIBITED"]: req(gap.get(key) is False,f"36-05 arithmetic firewall moved: {key}")
