...
def main():
 s=json.loads(STATE_PATH.read_text()); schema=s.get("schema"); req(schema in {V1,V2,V3,V4,V5,V6,V7,V8,V9,V10,V11,V12},"unrecognized Stage36 successor schema"); req(s.get("stage")=="36","Stage36 number moved")
 ...
 if schema==V10:
  req(s.get("status")=="ACTIVE_PENDING_HOSTILE_AUDIT" and s.get("base_main_sha")==PENDING_36_05_BASE,"V10 lifecycle moved"); u5=s.get("completed_units",{}).get("36-05",{}); req(u5.get("legal_outcome")=="BLOCKED_MOVING_RAMIFICATION_SUPPORT" and u5.get("promotion_status")=="PROVISIONAL_NOT_AUDITED","V10 blocked outcome moved"); return
 a5=s.get("stage36_36_05_authority",{}); req(a5.get("pr")==1569 and a5.get("hostile_audit_review")==5118563918 and a5.get("audited_head")=="cf430199171c98ed5f9eaaadeb8d2d40268ca6ba","36-05 authority moved"); req(a5.get("merged_main_sha")==AUDITED_36_05_MERGE and a5.get("exact_head_ci_run")==33928640974 and a5.get("exact_head_ci_job")==101202500740,"36-05 merge/CI authority moved"); req(a5.get("certificate_blob_sha")=="193d0165b242d799bc981774783a5160c1ac58dc" and a5.get("legal_outcome")=="BLOCKED_MOVING_RAMIFICATION_SUPPORT" and a5.get("verdict")=="PASS","36-05 authority semantics moved")
 ...
 if schema==V11:
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==V11_BASE,"V11 lifecycle moved"); req(s.get("current",{}).get("unit")=="36-09" and s.get("current",{}).get("36_06_entry_allowed") is False and s.get("current",{}).get("36_09_entry_allowed") is True,"V11 route moved"); req("36-09" not in s.get("completed_units",{}),"36-09 started in V11"); return
 req(s.get("status")=="ACTIVE_PENDING_HOSTILE_AUDIT" and s.get("base_main_sha")==V12_BASE,"V12 lifecycle moved"); req(s.get("stage36_36_05_promotion")==PROMO_36_05,"36-05 promotion provenance moved in V12")
 u9=s.get("completed_units",{}).get("36-09",{}); req(u9.get("promotion_status")=="PROVISIONAL_NOT_AUDITED" and u9.get("SELECTED_NEXT_ROUTE")=="36-09A_CAMP4_BRAUER_COMPATIBILITY_PREFLIGHT","V12 36-09 boundary moved"); req(u9.get("RECEIVER_MATCHED_REPLACEMENT_THEOREM_PROVED") is False,"V12 replacement theorem credit leaked")
 cur=s.get("current",{}); req(cur.get("unit")=="36-09" and cur.get("36_06_entry_allowed") is False and cur.get("36_09A_entry_allowed") is False,"V12 successor gate moved"); req("36-09A" not in s.get("completed_units",{}),"36-09A started before hostile audit")
