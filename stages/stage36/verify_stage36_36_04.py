...
ORDER7 = ["A1", "A2", "A3", "B3", "B2", "B1", "C"]
ORDER_INDEX = {name:i for i,name in enumerate(ORDER7)}
FORMS = {"A1":"x", "A2":"y", "A3":"z", "B3":"x+y", "B2":"x+z", "B1":"y+z", "C":"x+y+z"}
STRATA = {"NONE":[],"B3":["B3"],"B2":["B2"],"B1":["B1"],"C":["C"],"B3_B2":["B3","B2"],"B3_B1":["B3","B1"],"B2_B1":["B2","B1"]}
...
def canonical_dual(H,Z,target):
    ...
    for c in itertools.product((0,1),repeat=6):
        if pairing_vector(H,c)!=target: continue
        sup=support(c)
        if set(sup)&set(Z): continue
        order_tuple=tuple(ORDER_INDEX[x] for x in sup)
        candidates.append((len(sup),order_tuple,''.join(map(str,c)),c))
...
def main() -> None:
    cert=json.loads(CERT_PATH.read_text()); state=json.loads(STATE_PATH.read_text()); inv=json.loads(INV_PATH.read_text())
    ...
    require(cert.get("freshness_sync")==FRESHNESS,"36-04 certificate freshness moved")
    ...
    require(auth.get("stage36_36_03_promotion_merge")=="efe25f4ef74dc776da7ccad3f5cd786b0b2906e4","36-03 promotion merge moved")
    require(auth.get("stage36_36_03_certificate")=={"path":"stages/stage36/36-03/physical-open-boundary.json","blob_sha":PREV_BLOB},"36-03 cert authority moved")
    ...
    for key,(rel,sha) in SOURCES.items():
        require(cert.get("source_locks",{}).get(key)=={"path":rel,"blob_sha":sha},f"source declaration moved: {key}")
    ...
    require(al.get("S30-W02")=="NOT_TRIGGERED_Q_FORM_ALREADY_EXACT","S30-W02 activation moved")
    require(al.get("S34-W01")=="NOT_TRIGGERED_FINITE_SQUARECLASS_FAMILY_NOT_YET_PROVED","S34-W01 activated prematurely")
    require(al.get("S34-W03")=="PREPARED_IN_36_03_NOT_EXECUTED","S34-W03 credit moved")
    ...
    for needle in ["[x:y:z]=[a_1^2:a_2^2:a_3^2]","L_{a1}=x","L_{b3}=x+y","L_c=x+y+z"]:
    ...
    for needle in ["finite etale `H`-torsor","S  --etale degree 8-->  C_H","No converse is asserted."]:
    ...
    for left,right in [("B3","B2"),("B3","B1"),("B2","B1")]:
        p=cross(LINE_COEFF[left],LINE_COEFF[right]); require(all(x!=0 for x in p),f"{left}+{right} left physical coordinate open")
    require(det3(LINE_COEFF["B3"],LINE_COEFF["B2"],LINE_COEFF["B1"])!=0,"three B-lines acquired projective intersection")

    reps=cert.get("representatives",{}); inv_reps=inv.get("representatives",{})
    ...
    for rep,row in reps.items():
        ...
        require(row.get("H_perp_supports")==src.get("character_supports"),f"{rep}: H-perp not inventory-derived")
        H=[bits(s) for s in row["H_basis"]]; Hperp=[support_to_c6(s) for s in row["H_perp_supports"]]
        for hp in Hperp: require(pairing_vector(H,hp)==(0,0,0),f"{rep}: H-perp vector not orthogonal")
        span=set()
        for mask in range(8):
            w=(0,0,0,0,0,0)
            for i,b in enumerate(Hperp):
                if mask>>i&1: w=xor(w,b)
            span.add(w)
        require(len(span)==8,f"{rep}: H-perp rank moved")
        chart=row.get("dual_chart_c6",{}); require(set(chart)==set(STRATA),f"{rep}: chart coverage moved")
        per_character=[set(),set(),set()]
        for stratum,Z in STRATA.items():
            chosen=chart[stratum]; require(len(chosen)==3,f"{rep}/{stratum}: not three chars")
            for j,s in enumerate(chosen):
                c=bits(s); selection_count+=1
                ...
                sup=support(c); require(len(sup)%2==0,f"{rep}/{stratum}: odd support")
                require(not(set(sup)&set(Z)),f"{rep}/{stratum}: selected G vanishes")
                per_character[j].add(c)
        for j,choices in enumerate(per_character):
            for c,cprime in itertools.combinations(choices,2):
                d=xor(c,cprime)
                require(pairing_vector(H,d)==(0,0,0),f"{rep}/chi{j+1}: transition not H-invariant")
                require(in_span3(d,Hperp),f"{rep}/chi{j+1}: transition absent from H-perp span")
    require(selection_count==72,"dual-character selection coverage moved")

    pc=cert.get("pointwise_class",{})
    require(pc.get("degree")==8,"pointwise H-torsor degree moved")
    require("T1^2=G_c1" in pc.get("fiber_equations",""),"explicit fiber equations missing")
    require("delta_H(P)" in pc.get("definition",""),"pointwise H1 class missing")
    require("iff" in pc.get("rational_lift_iff","") and "rational squares" in pc.get("rational_lift_iff",""),"rational lift iff missing")
    ...
    require(state.get("freshness_sync_36_04")==FRESHNESS,"36-04 state freshness moved")
    ...
    for key in ["source_authority_lock_complete","three_Q_representatives_exact","physical_open_push_and_boundary_complete"]: require(gates.get(key) is True,f"audited predecessor gate lost: {key}")
    for key,value in gates.items():
        if key not in {"source_authority_lock_complete","three_Q_representatives_exact","physical_open_push_and_boundary_complete"}: require(value is False,f"later gate prematurely promoted: {key}")
