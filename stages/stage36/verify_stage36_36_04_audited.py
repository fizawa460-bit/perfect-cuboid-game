...
def main():
 ...
 for k in ["source_authority_lock_complete","three_Q_representatives_exact","physical_open_push_and_boundary_complete","pointwise_H_torsor_class_explicit"]: req(g.get(k) is True,f"audited predecessor gate lost: {k}")
 ...
 if schema==V9:
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==AUTH["merged_main_sha"],"V9 lifecycle moved"); req(s.get("current",{}).get("unit")=="36-05" and "36-05" not in s.get("completed_units",{}),"V9 successor moved")
 elif schema==V10:
  req(s.get("status")=="ACTIVE_PENDING_HOSTILE_AUDIT" and s.get("base_main_sha")==V10_BASE,"V10 lifecycle moved"); req(s.get("stage36_36_04_promotion")==PROMO,"36-04 promotion provenance moved")
  ...
  req(s.get("status")=="ACTIVE" and s.get("base_main_sha")==V11_BASE,"V11 lifecycle moved"); req(s.get("stage36_36_04_promotion")==PROMO,"36-04 promotion provenance moved in V11")
