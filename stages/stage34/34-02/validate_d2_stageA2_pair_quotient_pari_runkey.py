...
KEY="stages/stage34/runkeys/stage34-02-d2-stageA2-pair-quotient-pari.json"
def emit(ok,reason):
...
checks=[
(new.get("schema")=="STAGE34_02_D2_STAGEA2_PAIR_QUOTIENT_PARI_RUNKEY_V1","schema"),(new.get("armed") is True,"armed"),(isinstance(new.get("generation"),int) and new["generation"]>int(old.get("generation",0)),"generation"),(new.get("expected_parent_branches")==92,"branches"),(new.get("expected_distinct_models")==48,"models"),(new.get("pari_effort")==0,"effort"),(new.get("per_model_timeout_seconds")==120,"timeout"),(new.get("planned_heavy_jobs")==1,"jobs"),(new.get("effective_heavy_concurrency")==1,"concurrency"),(new.get("artifact_max_bytes")==4194304,"artifact_cap"),(new.get("retention_days")==1,"retention")]
