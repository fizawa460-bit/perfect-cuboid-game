...
KEY="stages/stage34/runkeys/stage34-02-d2-stageA2-reconstruction-rank.json"
def emit(ok,reason):
...
old=json.loads(oldshow.stdout) if oldshow.returncode==0 else {"generation":0,"armed":False}
checks=[
(new.get("schema")=="STAGE34_02_D2_STAGEA2_RECONSTRUCTION_RANK_RUNKEY_V1","schema"),
(new.get("armed") is True,"armed"),
(isinstance(new.get("generation"),int) and new["generation"]>int(old.get("generation",0)),"generation"),
(new.get("expected_twist_species")==18,"species"),
(new.get("expected_input_survivors")==1214,"input"),
