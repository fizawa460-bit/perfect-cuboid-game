...
KEY="stages/stage34/runkeys/stage34-02-d2-stageA2-rankzero-AB-complete-pullback.json"
def emit(ok,reason):
...
checks=[
(new.get("schema")=="STAGE34_02_D2_STAGEA2_RANKZERO_AB_COMPLETE_PULLBACK_RUNKEY_V1","schema"),(new.get("armed") is True,"armed"),(isinstance(new.get("generation"),int) and new["generation"]>int(old.get("generation",0)),"generation"),(new.get("expected_input_d1_branches")==92,"input"),(new.get("expected_target_branches")==16,"target"),(new.get("expected_remaining_d1_branches")==76,"remaining"),(new.get("planned_heavy_jobs")==1,"jobs"),(new.get("effective_heavy_concurrency")==1,"concurrency"),(new.get("artifact_max_bytes")==1048576,"artifact_cap"),(new.get("retention_days")==1,"retention")]
