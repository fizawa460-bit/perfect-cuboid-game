...
for i,c in enumerate(lock["targets"],1):
    ...
    if status=="PASS_RATIONALPOINTS_RETURN":
        ...
        rec.update({"complete":complete,"height_bound":val("HEIGHT_BOUND:",out),"point_count":cnt,"receiver_degenerate_count":deg,"full_parent_lift_count":par,"nondegenerate_full_parent_lift_count":nondeg,"closure_candidate":cand,"point_lines":[x for x in out.splitlines() if x.startswith("POINT:")],"stdout_sha256":hashlib.sha256(out.encode()).hexdigest()})
