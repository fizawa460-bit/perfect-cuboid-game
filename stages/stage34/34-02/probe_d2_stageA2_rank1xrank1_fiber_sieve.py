...
gens2={int(x["model_id"]):ns["parse_gen"](x["mwrank_o_line"]) for x in mw2["models"]};brmap={x["branch_id"]:x for x in sel["branches"]}
CAP=300000;MAX_PRIMES=12;BOUND=401
...
def bd_pair(br,mid,pair,G):
 ...
 assert len(tors)==4 and G[1]**2==G[0]**3+a4*G[0]+a6
...
def local_tables(br,bd,p):
 ...
 a4=int(bd["a4"])%p;a6=int(bd["a6"])%p;G=ns["reduce_point"](bd["G"],p);m=ns["point_order"](G,p,a4,a6);tors=[ns["reduce_point"](T,p) for T in bd["tors"]];out=[]
 for Ti in tors:
