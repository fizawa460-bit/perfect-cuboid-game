...
def submit(code):
    ...
    req=urllib.request.Request(URL,data=data,headers={
        ...
        "Referer":REFERER,
        "User-Agent":"perfect-cuboid-stage34-q8413-twocover-descent/1.0"
    },method="POST")
...
S,sm,elts,smap,loc:=TwoSelmerGroup(E); print "TWO_SELMER_INVARIANTS:",Invariants(S); assert #S eq 8;
...
rank_zero_candidate=(not bad and complete and cc==1 and fc==0)
payload={
  "schema":"STAGE34_02C_D2_STAGEA2_Q8413_MODEL38_RESIDUAL_TWOCOVER_DESCENT_PROBE_V1",
  ...
  "two_selmer_invariants":val("TWO_SELMER_INVARIANTS:",out),
  ...
  "credit":"Diagnostic only. Empty fake 2-Selmer set on the sole residual cover is an exact rank-zero proof candidate under the locked theorem adapter, but deterministic replay and hostile audit are required before any authority change.",
...
print(json.dumps({"status":payload["status"],"residual_cover_count":cc,"fake_two_selmer_cardinality":fc,"rank_zero_candidate":rank_zero_candidate},sort_keys=True))
