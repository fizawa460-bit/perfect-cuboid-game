...
assert f eq fexp;
fint:=3025*f; fintexp:=-990*x^5-400*x^4+1980*x^3+400*x^2-990*x;
assert fint eq fintexp; print "QUOTIENT_IDENTITY: true"; print "INTEGRAL_MODEL_IDENTITY: true";
...
payload={
  ...
  "branch_id":"6b3bcb70c4fda8e6f1e0","q":"20/99","delta":[-1,-55,-5,-11],"triple":"U*V*B",
  "source_lock":LOCK.name,"source_lock_sha256":"sha256:"+hashlib.sha256(LOCK.read_bytes()).hexdigest(),
  "quotient_identity_verified":val("QUOTIENT_IDENTITY:",out)=="true","integral_model_identity_verified":val("INTEGRAL_MODEL_IDENTITY:",out)=="true",
  "rank_bounds":[int(x) for x in val("RANK_BOUNDS:",out).split()[:2]],"complete_qpoint_count":int(val("CHABAUTY0_COUNT:",out)),
  "receiver_degenerate_count":int(val("RECEIVER_DEGENERATE_COUNT:",out)),"full_parent_lift_point_count":int(val("FULL_PARENT_LIFT_POINT_COUNT:",out)),"nondegenerate_full_parent_lift_count":int(val("NONDEGENERATE_FULL_PARENT_LIFT_COUNT:",out)),
  "point_lines":point_lines,"raw_stdout_sha256":"sha256:"+hashlib.sha256(out.encode()).hexdigest(),
  "proof":"The literal U*V*B quotient is reconstructed from the parent square equations and replaced only by the Q-isomorphic integral model Y=55y. Exact RankBounds(J)=[0,0] makes Chabauty0 a complete enumeration of the quotient rational points. Every returned quotient point is checked against all four parent square conditions and receiver degeneracy; no nondegenerate full parent lift exists.",
...
assert payload["quotient_identity_verified"] and payload["integral_model_identity_verified"] and payload["rank_bounds"]==[0,0] and payload["nondegenerate_full_parent_lift_count"]==0
OUT.write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
