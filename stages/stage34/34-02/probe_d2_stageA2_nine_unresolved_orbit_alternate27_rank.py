...
LOCK=ROOT/"d2-stageA2-nine-unresolved-orbit-alternate27-lock.json"
OUT=ROOT/"d2-stageA2-nine-unresolved-orbit-alternate27-rank-probe.json"
RAW=ROOT/"d2-stageA2-nine-unresolved-orbit-alternate27-rank-probe-stdout.txt"
...
def submit(code):
    ...
    req=urllib.request.Request(URL,data=data,headers={"Content-Type":"application/x-www-form-urlencoded","Accept":"text/html, application/xml, application/xhtml+xml","Referer":REFERER,"User-Agent":"perfect-cuboid-stage34-alt27-rank/1.0"},method="POST")
...
assert lock["schema"]=="STAGE34_02B_D2_STAGEA2_NINE_UNRESOLVED_ORBIT_ALTERNATE27_LOCK_V1"
assert lock["status"]=="SOURCE_LOCKED_COLD_NOT_ARMED"
targets=lock["targets"]
assert len(targets)==27 and len({int(t["model_id"]) for t in targets})==27
...
payload={
  "schema":"STAGE34_02B_D2_STAGEA2_NINE_UNRESOLVED_ORBIT_ALTERNATE27_RANK_PROBE_V1",
  "status":"DIAGNOSTIC_NO_CREDIT_COMPLETE" if len(resolved)==27 else "DIAGNOSTIC_NO_CREDIT_PARTIAL",
  "source_lock":LOCK.name,"source_lock_sha256":"sha256:"+hashlib.sha256(LOCK.read_bytes()).hexdigest(),
  "input_targets":27,"resolved":len(resolved),"unresolved":27-len(resolved),
  ...
  "firewalls":{"rank_bound_is_complete_pointset":False,"diagnostic_is_parent_closure":False,"unresolved_is_math_failure":False,"remaining_22_closed":False,"D2_all_factor_branches_closed":False,"all_multiples_closed":False,"R29_EXT_CHANG_C_closed":False,"perfect_cuboid_nonexistence_claim":False}
}
OUT.write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n")
print("ALT27_RANK_PROBE="+json.dumps({k:payload[k] for k in ["status","resolved","unresolved","rank_bounds_histogram","best_alternate_rank_upper_by_branch","favorable_rank_upper_le_1_count"]},sort_keys=True))
