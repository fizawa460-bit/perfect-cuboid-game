...
def submit(code):
    ...
    req=urllib.request.Request(URL,data=data,headers={"Content-Type":"application/x-www-form-urlencoded","Accept":"text/html, application/xml, application/xhtml+xml","Referer":REFERER,"User-Agent":"perfect-cuboid-stage34-six-rank-adapter/2.0"},method="POST")
...
lock=json.loads(LOCK.read_text()); assert lock["schema"]=="STAGE34_02B_D2_STAGEA2_SIX_DIAGNOSTIC_RANKBOUND_ADAPTER_PROOF_LOCK_V2_REPAIRED" and len(lock["targets"])==6
records=[]; rawparts=[]
...
payload={"schema":"STAGE34_02B_D2_STAGEA2_SIX_DIAGNOSTIC_RANKBOUND_ADAPTER_PROOF_CERTIFICATE_V2_REPAIRED","status":"READY_FOR_HOSTILE_AUDIT_SIX_REPAIRED_ADAPTER_CANDIDATES" if len(closed)==6 else "EXACT_REPAIRED_ADAPTER_PROOF_PARTIAL_CANDIDATES_REQUIRE_HOSTILE_AUDIT","source_lock":LOCK.name,"source_lock_sha256":"sha256:"+hashlib.sha256(LOCK.read_bytes()).hexdigest(),"rank_evidence":lock["rank_evidence"],"generation1_revocation":lock["generation1_revocation"],"records":records,"direct_closure_candidate_count":len(closed),"direct_closure_candidate_branch_ids":[r["branch_id"] for r in closed],"sign_transfer_candidate_branch_ids":[r["partner"] for r in closed],"candidate_closed_branches_from_this_adapter_if_audited":2*len(closed),"candidate_remaining_from_22_if_only_this_adapter_audited":22-2*len(closed),"candidate_remaining_by_q_from_22_if_only_this_adapter_audited":rem,"raw_stdout_sha256":"sha256:"+hashlib.sha256(raw.encode()).hexdigest(),"credit":"Pre-audit repaired adapter proof only. The exact-model source-locked prior RankBounds supplies only the rigorous rank<=1 premise; this run supplies quotient completeness and exact parent pullback. No authoritative promotion without hostile audit.","firewalls":{"hostile_audit_passed":False,"candidate_is_authoritative":False,"authoritative_remaining_d1":22,"D2_all_factor_branches_closed":False,"all_multiples_closed":False,"R29_EXT_CHANG_C_closed":False,"perfect_cuboid_nonexistence_claim":False}}
